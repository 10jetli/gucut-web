...
const list = products
  ...
  .map((p) => ({
    ...
    /* 🔴 **รหัสของตัวเลือกสินค้า** (แก้ 19 ก.ย. 2569 — ท่านประธานสั่ง)
       เว็บเราขายหลายแบบในสินค้าเดียว (เช่น 00073 เลือก "เฉพาะเครื่อง" / "11.8 นิ้ว")
       และตัวเลือกแต่ละอันผูกรหัสไว้แล้ว (`v[].k`) ⇒ **ขายได้จริงและตัดสต็อกได้จริง**
       แต่ฟีดนี้เคยส่งแค่ `sku` ระดับสินค้า ⇒ ตัวตรวจ "ช่องทางที่ลงขาย" มองไม่เห็น
       ⇒ รายงานบอกว่าเว็บเราขาด **251 รหัส** ทั้งที่ขาดจริงแค่ 31
       ⇒ ถ้าเชื่อตัวเลขนั้นแล้วลงมือ จะไปสร้างสินค้าซ้ำ 220 รายการบนเว็บ
          ซึ่งแย่กว่าไม่ทำอะไรเลย (เกือบทำจริง 18 ก.ย.)

       🔑 **เพิ่มช่องใหม่ ไม่แตะ `sku` เดิม** — ไฟล์นี้มีคนใช้ 5 ที่
          (products-feed · catalog-feed · feed-health · status · marketplace-listings)
          เปลี่ยนความหมายของช่องเดิม = พังเงียบหลายที่พร้อมกัน */
    vsku: (() => {
      const own = String(p.sku || "");
      const out = [...new Set((p.v || [])
        .map((v) => String(v?.k || "").trim())
        .filter((k) => k && k !== own))];
