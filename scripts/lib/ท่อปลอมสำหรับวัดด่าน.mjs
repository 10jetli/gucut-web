  ...
  const server = createServer((req, res) => {
    ...
    if (ไล่หน้าจริง || ไล่หน้าเพี้ยน) {
      ...
      res.end(JSON.stringify({
        ...
        /* 🔴🔴 **20 ก.ย. 2569 — ท่อปลอมส่งแค่ `nextOffset` แต่ท่อจริงส่งสามคีย์**
           📏 ยิงท่อจริงยืนยัน: `?list=stock&limit=2` ⇒ `nextOffset` · `nextPage` · `pagingDone` ครบ
           ⇒ ⇒ ด่าน `check-paging-complete` **แดงว่า "stock ไม่บอกทางไปหน้าถัดไป"**
              ทั้งที่ท่อจริงบอกครบ ⇒ **แดงลวงจากตัวปลอม** (รูปเดียวกับ `สภาพตอนวัด` วันนี้ จุดที่สอง)
           🔑 และผมเกือบโทษการแก้ของตัวเอง — ถอยการเปลี่ยนแล้วด่าน **ยังแดง**
              ⇒ ⇒ **ด่านนี้แดงอยู่ก่อนรอบนี้** · เจอเพราะถอยแล้ววัดซ้ำ ไม่ใช่เพราะอ่านโค้ด
           ⚠️ `pagingDone` ต้องเป็น `true` **เฉพาะเมื่อไล่จบจริง** — ส่ง true ตลอดจะกลบอาการ
              "ไล่ไม่จบแต่บอกว่าจบ" ซึ่งเป็นสิ่งที่ด่านนี้มีไว้จับ */
        ...(ถัดไป === null
          ? { pagingDone: true }
          : { nextOffset: ถัดไป, nextPage: Math.floor(ถัดไป / Math.max(1, limit)) + 1, pagingDone: false }),
