  ...
  for (const q of [undefined, null, '', '   ']) {
    ...
    await withFetch(async (urls) => {
      ...
      /* 17 ก.ย. 2569: เส้นสดอ่านกระจกได้ **เฉพาะยอดรวม** (mirrorTotals) — แถวต้องมาจาก ZORT สดเท่านั้น
         🔴 แก้เกณฑ์ 18 ก.ย. 2569: เดิมยกเว้นด้วย **รูป SQL ตัวเดียวแบบ hardcode**
            ⇒ วันที่เพิ่มยอดรวมแบบแยกกลุ่ม (byStatus) ด่านร้องทั้งที่ **อยู่ในเจตนาเดิม** (เป็นยอดรวม ไม่ใช่แถว)
            ⇒ ผูกกับ **เจตนา** แทน: ห้ามดึง **คอลัมน์รายแถว** จากกระจก (number/id/return_date/vendor)
              ยอดรวมทุกแบบ (COUNT/SUM/GROUP BY) ยังอ่านได้ตามเดิม
            🔑 ด่านที่ผูกกับรูป ไม่ใช่เจตนา จะร้องใส่ของที่ถูก แล้วคนจะแก้ด่านให้หลวมทั้งใบ
               (ผมเกือบทำแบบนั้น — หยุดอ่านเจตนาของด่านก่อน) */
      const ดึงคอลัมน์รายแถว = /\b(number|return_date|vendor|contact)\b/;
      const อ่านแถวจากกระจก = sqls.filter(
        (x) => /return_orders_v2/.test(x.s) && ดึงคอลัมน์รายแถว.test(x.s)
      );
