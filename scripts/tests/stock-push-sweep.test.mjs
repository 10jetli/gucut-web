...
/* ⚠️ node:test mock โมดูลเดิมซ้ำไม่ได้ ⇒ แผนที่จะคืนต้องสลับผ่านตัวแปร (กติกาเดียวกับ `ผลยิงที่จะคืน`)
   null = แผนมาตรฐาน 2 รหัส (เทสต์เดิมทุกข้อพึ่งรูปนี้ **ห้ามเปลี่ยนค่าเริ่มต้น**) */
let แผนที่จะคืน = null;
mock.module('../../netlify/lib/stock-push.mjs', { namedExports: {
  stockPushDryRun: async ({ platform = 'lazada' } = {}) => ({ [platform]: {
    push: แผนที่จะคืน ?? [{ sku: 'A1', from: 1, to: 5, kind: 'up' }, { sku: 'B2', from: 2, to: 9, kind: 'up' }],
...
const { กวาดดันสต็อก, สถานะดันสต็อก, ยิงจริงของ, ยกก้อนไปรอบหน้าไหม } = await import('../../netlify/lib/stock-push-sweep.mjs');
...
/* ══════════════════════════════════════════════════════════════════════════════
   ด่านงบเวลา: **ห้ามเริ่มก้อนที่คาดว่าทำไม่ทัน** (เพิ่ม 19 ก.ย. 2569 · ใบ t_mu7qyolx)

   🔴 ที่มาจากการวัดของจริง ไม่ใช่จากการอ่านโค้ด (`?sweeptiming=1` ย้อน 12 ชม.):
      `lazada live` 19 รอบ · ค่ากลาง 14,083 ms · **ช้าสุด 30,021 ms = เกินเพดาน 30,000 ที่เราตั้งเอง**
      รายขั้น: แผน 15,380 · ยิง 14,161 · เขียนสมุด 849 ⇒ รวม ≈ 30.4 วิ **ตรงกับรอบที่ล้น**
      และ **7 จาก 19 รอบเกือบชนเพดาน** ⇒ เป็นเรื่องปกติของช่องทางนี้ ไม่ใช่รอบผิดปกติ
   🔑 ต้นเหตุ: ด่านเดิมถามว่า "เริ่มก้อนใหม่ได้ไหม" ที่วินาทีที่ 16
      แต่ **หนึ่งก้อนกินได้ถึง ~14 วิ** ⇒ ผ่านด่านที่ 15.9 แล้วล้นไปทั้งก้อน
      ⇒ **ด่านที่หน่วยใหญ่กว่างบที่เหลือ กันอะไรไม่ได้เลย**
   ══════════════════════════════════════════════════════════════════════════════ */

/* 🔴 **เทสรอบแรกของผมในไฟล์นี้เป็นข้อที่ผ่านได้ทั้งสองทาง** — ตัวปลอมยิงเสร็จทันที
   ⇒ ไม่มีก้อนช้าให้ด่านจับ ⇒ ยิงครบ 3 ก้อนทั้งกรณีที่ด่านมีและไม่มี ⇒ ข้อนั้นไม่ได้ทดสอบอะไร
   ⇒ ย้ายมาทดสอบ **คำตัดสิน** ตรง ๆ ด้วยเลขจริง (ฟังก์ชันบริสุทธิ์ที่ลูปเรียกใช้จริง)
   🔑 บทเรียน: **ตัวปลอมมองไม่เห็นมิติที่ตัวเองไม่มี** — ตัวปลอมไม่มีมิติ "เวลา" จึงทดสอบเรื่องเวลาไม่ได้ */
test('คำตัดสินยกก้อนไปรอบหน้า: ก้อนก่อนหน้าช้า + ใช้เวลาไปมาก ⇒ ต้องยก', () => {
  const เหตุ = ยกก้อนไปรอบหน้าไหม({ ใช้ไปแล้ว_ms: 15_500, ก้อนก่อนหน้าใช้_ms: 14_000 });
  assert.match(String(เหตุ), /คาดว่าเกินงบ/);
  assert.match(String(เหตุ), /14000/, 'ต้องบอกเลขที่ใช้ตัดสิน ไม่ใช่บอกแค่ว่ายก');
});

test('🔑 พลังแยกแยะ ①: ก้อนก่อนหน้าเร็ว ⇒ **ต้องไม่ยก** (ไม่งั้นของค้างสะสมเงียบ ๆ)', () => {
  assert.equal(ยกก้อนไปรอบหน้าไหม({ ใช้ไปแล้ว_ms: 15_500, ก้อนก่อนหน้าใช้_ms: 800 }), null);
  assert.equal(ยกก้อนไปรอบหน้าไหม({ ใช้ไปแล้ว_ms: 2_000, ก้อนก่อนหน้าใช้_ms: 14_000 }), null);
});

test('🔑 พลังแยกแยะ ②: ก้อนแรก (ยังไม่รู้เวลาก้อนก่อน) ⇒ ใช้กติกาเดิมอย่างเดียว ห้ามเดา', () => {
  /* ถ้าเดาเวลาก้อนแรกเป็นเลขคงที่ รอบที่แผนช้าจะ **ไม่ยิงเลยทั้งรอบ** ซึ่งเคยเกิดจริง 17 ก.ย. */
  assert.equal(ยกก้อนไปรอบหน้าไหม({ ใช้ไปแล้ว_ms: 15_900, ก้อนก่อนหน้าใช้_ms: null }), null);
  assert.equal(ยกก้อนไปรอบหน้าไหม({ ใช้ไปแล้ว_ms: 15_900, ก้อนก่อนหน้าใช้_ms: 0 }), null);
  assert.equal(ยกก้อนไปรอบหน้าไหม({ ใช้ไปแล้ว_ms: 15_900, ก้อนก่อนหน้าใช้_ms: NaN }), null);
});

test('กติกาเดิมยังอยู่: เลยเวลาเริ่มก้อนใหม่ ⇒ ยก แม้ไม่รู้เวลาก้อนก่อน', () => {
  assert.match(String(ยกก้อนไปรอบหน้าไหม({ ใช้ไปแล้ว_ms: 16_001, ก้อนก่อนหน้าใช้_ms: null })),
    /เลยเวลาเริ่มก้อนใหม่/);
});

test('อ่านเวลาไม่ได้ ⇒ ไม่ตัดสิน (ปล่อยผ่าน) — ห้ามหยุดยิงเพราะตัววัดพัง', () => {
  /* 🔑 ตัววัดพังไม่ควรทำให้ระบบหยุดทำงาน — ไม่งั้นตัวที่มีไว้ช่วยจะกลายเป็นตัวที่ทำให้พัง */
  for (const v of [null, undefined, NaN, 'ไม่ใช่เลข'])
    assert.equal(ยกก้อนไปรอบหน้าไหม({ ใช้ไปแล้ว_ms: v, ก้อนก่อนหน้าใช้_ms: 14_000 }), null);
});

test('🔑 พลังแยกแยะ: ก้อนเร็ว ⇒ **ต้องยิงครบทุกก้อน** ไม่ใช่ถอยเพราะกลัวเกินงบ', async () => {
  /* ถ้าข้อนี้แดง = ด่านใหม่ถอยมากเกินไป ⇒ ของค้างสะสมทุกรอบโดยไม่มีใครรู้
     (ด่านที่ระวังเกินไปทำให้ "สต็อกไม่ตรง" กลายเป็นอาการถาวรแบบเงียบ) */
  คำสั่ง.length = 0;
  แผนที่จะคืน = Array.from({ length: 250 }, (_, i) => ({ sku: `F${i}`, from: 1, to: 5, kind: 'up' }));
  ผลยิงที่จะคืน = { fired: 100, pushed: 100, rejected: 0, notSent: 0, results: [] };
  const เดิม = ส่งแผนมา.length;
  await กวาดดันสต็อก({ platform: 'lazada', force: true });
