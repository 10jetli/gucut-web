...
/* ── ⑥ จุดบอดของ `ยุบ`: มันเห็นแค่การซ้ำ "ภายในคำขอเดียว" ──────────────────
 * 🔴 ของจริง 19 ก.ย. 2569 (ยิงซิงก์รอบแรก): TikTok `รับมา 300 · ส่งไป 300 · ลงฐาน 100`
 *    แต่ `ยุบ` เป็น **0 ทุกหน้า** เพราะแต่ละหน้าไม่มีของซ้ำในตัวเอง —
 *    ที่ซ้ำคือ **หน้า 1/2/3 คืนแถวชุดเดียวกัน** (TikTok เมิน `page` เพราะใช้ `pageToken`)
 * 🔑 ตัวนับที่นับในขอบเขตแคบกว่าปัญหา จะเงียบตลอดกาล
 *    ⇒ ต้องส่งคู่กุญแจออกไปให้ผู้เรียกรวม **ข้ามหน้า** แล้วตัดสินจากยอดรวมทั้งรอบ
 * ⚠️ ด่านนี้เฝ้า "ของที่ต้องส่งออกมาให้รวมได้" ไม่ได้เฝ้าตัวเลข —
 *    เพราะการตัดสินอยู่ที่ผู้เรียก (core.mjs) ไม่ใช่ที่ตัวเขียน
 */
test("⑥ ต้องส่งคู่กุญแจของรอบนั้นออกมา เพื่อให้ผู้เรียกรวมข้ามหน้าได้", async () => {
  แถวในฐาน = 0;
  const r = await เขียนการเงิน("fee-line", [feeRow("900", "Commission", -1), feeRow("901", "Payment Fee", -2)]);
  assert.ok(r.คู่กุญแจชุดนี้ instanceof Set, "ต้องคืน Set ของคู่กุญแจ ไม่งั้นผู้เรียกจับการซ้ำข้ามหน้าไม่ได้");
  assert.equal(r.คู่กุญแจชุดนี้.size, 2);
});

test("⑥ ยิงชุดเดิมซ้ำสองรอบ ⇒ คู่กุญแจรวมต้องยังเป็น 2 (ผู้เรียกจะเห็นว่าส่งไป 4 แต่ไม่ซ้ำ 2)", async () => {
  แถวในฐาน = 0;
  const ชุด = [feeRow("910", "Commission", -1), feeRow("911", "Payment Fee", -2)];
  const a = await เขียนการเงิน("fee-line", ชุด);
  const b = await เขียนการเงิน("fee-line", ชุด);
  const รวม = new Set([...a.คู่กุญแจชุดนี้, ...b.คู่กุญแจชุดนี้]);
  assert.equal(a.ยุบ + b.ยุบ, 0, "แต่ละคำขอไม่มีของซ้ำในตัวเอง ⇒ ยุบ = 0 (นี่คือจุดบอด)");
  assert.equal(รวม.size, 2, "แต่รวมข้ามคำขอแล้วไม่ซ้ำเพียง 2 ⇒ ผู้เรียกต้องจับได้จากตรงนี้");
});
