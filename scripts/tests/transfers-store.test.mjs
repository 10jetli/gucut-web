...
test('core.mjs — เส้นที่ไม่รับ q ต้องประกาศ ignored ไม่ใช่เมินเงียบ', () => {
  ...
  for (const list of ['categories', 'deadstock']) {
    ...
    /* 🔄 เดิมบังคับรูป `ignored: { q` ตรงตัว ⇒ **เปลี่ยนรูปโค้ดไม่ได้เลยทั้งที่พฤติกรรมเดิม**
       (ตกจริง 19 ก.ย. 2569 ตอนเปลี่ยนเป็น spread เพื่อรวม ignored ของ offset/page เข้าไปด้วย)
       ⇒ ตรวจสองอย่างที่เป็นใจความจริง: มีคำว่า ignored และมี q อยู่ในนั้น
       🔑 เทสต์ที่ผูกกับ **รูปโค้ด** จะบล็อกการแก้ที่ถูกต้อง · ผูกกับ **ใจความ** ถึงจะทน */
    assert.match(body, /ignored:/, `${list}: ต้องมี ignored — เมินเงียบคือปุ่มหลอกบนจอ`);
    /* ⚠️ **ห้ามผูกกับชื่อตัวแปร** — deadstock ใช้ `qd` · categories ใช้ `qc`
       ผมเขียน regex เดาชื่อรอบแรกแล้วตกอีกรอบ ⇒ ตรวจสองข้อที่เป็นใจความ:
       บล็อกนั้น **อ่านค่า q** และ **มี ignored** ⇒ พอที่จะบอกว่าไม่ได้เมินเงียบ */
    assert.match(body, /searchParams\.get\("q"\)/, `${list}: ต้องอ่านค่า q มาตรวจ`);
    assert.match(body, /\{\s*q(:|\s*\})/, `${list}: ignored ต้องมีคีย์ q`);
  }
});
