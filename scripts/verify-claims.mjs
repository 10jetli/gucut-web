...
const CLAIMS = [
  ...
  },
  {
    ...
    async check() {
      ...
      /* 📌 **วัดฝั่ง ZORT ไว้แล้วครั้งหนึ่ง 7 ก.ย. 2569** (เปิดจอจริงที่ล็อกอินอยู่)
          `/Warehouse/Transferlist` เปิดช่วงกว้าง 10 ปี ⇒ **12,197 ใบ** · กระจกวันนั้น 12,002
          ⇒ **ส่วนต่าง 195 ใบ**
          ⚠️ **ต้องเปิดช่วงกว้างเท่านั้น** — ค่าเริ่มต้นของจอ ZORT แคบมาก (วัดที่ /Sell/list:
             ค่าเริ่มต้น 122 · เปิดกว้าง 26,030) ⇒ เทียบด้วยค่าเริ่มต้นจะได้ส่วนต่างที่ไม่มีความหมาย
          ⚠️ **เลขทั้งสองฝั่งโตทุกวัน** ⇒ ตัวที่ต้องเฝ้าคือ **ส่วนต่าง** ไม่ใช่เลขดิบ
             เอา 12,197 ไปเทียบกับกระจกวันหลังไม่ได้ — ต้องเปิด ZORT ดูใหม่ทุกครั้ง
          ⚠️ ยังไม่รู้ว่า 195 ใบนั้นคืออะไร — **นี่คือตัวเลข ไม่ใช่คำอธิบาย** */
      return {
        state: "ต้องดูด้วยตา",
        why: `กระจกมี ${t.toLocaleString()} ใบ · ใบเก่าสุด ${r.body?.oldest ?? "?"} ` +
          `⇒ เปิด /Warehouse/Transferlist ใน ZORT **โดยใส่ zfromdate=01/01/2560&zenddate=31/12/2569** ` +
          `แล้วเทียบยอดรวมวันนี้ (วัดไว้ 7 ก.ย. 2569: ZORT 12,197 · กระจก 12,002 ⇒ ต่าง 195 ใบ ` +
          `— เลขทั้งสองฝั่งโตทุกวัน ให้เฝ้า "ส่วนต่าง" ไม่ใช่เลขดิบ)`,
      };
    },
  },
  {
