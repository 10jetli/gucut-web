...
/* 🔴🔴 **ห้ามเปลี่ยนไปใช้ `เขียนถ้าเนื้อเปลี่ยน` กับไฟล์นี้** (เขียนไว้ 20 ก.ย. 2569)
   ไฟล์ที่ตัวนี้เขียน (`netlify/lib/arch-data.mjs`) เป็นที่มาของ **`x-core-build`**
   = **ตัวชี้ยุคของท่อ** ที่ทั้งสองบัญชีใช้ตอบคำถาม *"ของที่แก้ถึงระบบจริงหรือยัง"*
   ⇒ ถ้าไฟล์ไม่ถูกเขียนเมื่อเนื้อเหมือนเดิม ⇒ `generatedAt` ค้าง ⇒ **หัวนั้นโกหกโดยไม่มีอะไรฟ้อง**
   ⚠️ และเหตุผลที่จะพาคนมาแก้จุดนี้ **ฟังดูดีมาก**: "ไฟล์นี้ถูกเขียนทุก build ⇒ git ไม่สะอาด
      ⇒ อีกบัญชี pull ไม่ได้" — ซึ่งเป็นเหตุผลที่ผมใช้กับ `gen-zort-arch.mjs` วันเดียวกันนี้เอง
   🔑 คลาส: **การแก้ที่ถูกสำหรับไฟล์หนึ่ง เป็นการแก้ที่ผิดสำหรับไฟล์ที่ทำหน้าที่เป็นตัวชี้เวลา**
   ✅ มีเทสกันไว้: `scripts/tests/ตัวชี้ยุคของท่อ.test.mjs`
   📌 ถ้าวันหนึ่งจำเป็นจริง ⇒ **ย้ายที่มาของ `x-core-build` ก่อน** (เช่น env ที่ Netlify ใส่ตอน build)
      แล้วแก้เทสนั้นพร้อมกัน — ห้ามปิดเทสเฉย ๆ */
writeFileSync(
