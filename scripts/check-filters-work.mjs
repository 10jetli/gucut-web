...
/* ══ รอบสอง: ส่งค่า **ผิดรูป** แล้วต้องตีกลับ ไม่ใช่เมินเงียบ ══════════════
   🔴 ที่มา: คลาสนี้กัดทีมมาหลายรอบ และทุกรอบหน้าตาเหมือนสำเร็จ
     · `only=negative` (คำที่เดาก่อนเสมอ) ⇒ เดิมตกเงียบเป็น "ไม่กรอง" ⇒ **คืนทั้งคลัง 2,672 แถว**
       คนขอรายการติดลบได้ของทั้งคลังกลับไปโดยเชื่อว่านั่นคือรายการติดลบ (พลาดกันทั้งทีม 13 ก.ย.)
     · `sort=` ค่าที่ไม่รองรับ ⇒ กลายเป็นเรียงตาม qty เงียบ ๆ
   ⇒ ต่างจากรอบแรก: รอบแรกส่ง "ค่าถูกรูปแต่ไม่มีของ" (ทดสอบว่ากรองจริงไหม)
     รอบนี้ส่ง **"ค่าผิดรูป/ไม่รู้จัก"** (ทดสอบว่าปฏิเสธหรือเมิน)

   ⚠️ **ผลที่ยอมรับได้**: 400 ✅ · ประกาศใน `ignored` ✅ · **เมินเงียบแล้วคืนผลเท่าไม่ส่ง 🔴**
   ⚠️ ช่องที่รับข้อความอิสระ (`q` · `sku` · ชื่อคน) **ไม่มีค่าผิดรูป** ⇒ ข้าม ไม่ใช่ตก
      (ส่งอะไรไปก็เป็นคำค้นที่ถูกต้องเสมอ ⇒ เกณฑ์นี้ใช้กับมันไม่ได้) */
const ค่าผิดรูป = {
  from: "ไม่ใช่วันที่", to: "2026-99-99",
  status: "", // ว่าง = ไม่กรอง (ถูกต้อง) ⇒ ใช้ค่าที่ผิดชนิดแทนด้านล่าง
  only: "negative",          // ← คำที่เดาก่อนเสมอ · ของจริงคือ neg
  kind: "goods-ไม่มีจริง",
  by: "ไม่ใช่ month หรือ category",
  days: "ไม่ใช่เลข", quietDays: "abc", lookbackDays: "-5", minSold: "abc",
  store: "z99", warehouse: "คลัง!@#",
  withEmail: "maybe", withPhone: "maybe",
  includeCancelled: "maybe",
};
const ข้ามเพราะเป็นข้อความอิสระ = new Set(["q", "sku", "member", "channel", "carrier", "status", "source"]);
const เมินเงียบ = [];
for (const [เส้น, e] of Object.entries(catalogue.เส้น)) {
  const ช่อง = [...new Set(Object.keys(e.คีย์).filter((k) => k.startsWith("applied.")).map((k) => k.split(".")[1]))]
    .filter((f) => !ข้าม.has(f) && !ข้ามเพราะเป็นข้อความอิสระ.has(f) && ค่าผิดรูป[f] !== undefined && ค่าผิดรูป[f] !== "");
  if (!ช่อง.length) continue;
  const ฐาน = await ยิง(new URLSearchParams({ list: เส้น, limit: "5", ...(บังคับ[เส้น] || {}) }).toString());
  const ฐานN = นับแถว(ฐาน.data);
  for (const f of ช่อง) {
    const ชื่อส่ง = (m => m && m[f] ? String(m[f]).split(" ")[0] : (ชื่อพารามิเตอร์[f] || f))(ฐาน.data?.appliedParamNames);
    const r = await ยิง(new URLSearchParams({ list: เส้น, limit: "5", ...(บังคับ[เส้น] || {}), [ชื่อส่ง]: ค่าผิดรูป[f] }).toString());
    if (r.http === 400 || r.data?.error) { ผ่าน++; continue; }                 // ✅ ตีกลับ
    const ig = r.data?.ignored;
    if (ig && typeof ig === "object" && f in ig) { ผ่าน++; continue; }          // ✅ ประกาศว่าเมิน
    const n = นับแถว(r.data);
    if (n === null || ฐานN === null) continue;                                  // นับไม่ได้ ⇒ ไม่ตัดสิน
    if (n !== ฐานN) { ผ่าน++; continue; }                                       // ✅ มีผลบางอย่าง (ไม่เมิน)
    เมินเงียบ.push(`${เส้น}.${f}: ส่งค่าผิดรูป "${ค่าผิดรูป[f]}" ทางชื่อ "${ชื่อส่ง}" แล้วได้ผลเท่าไม่ส่ง (${ฐานN}) และไม่ประกาศว่าเมิน`);
  }
}
if (เมินเงียบ.length) {
  console.log(`\n🔴 ค่าผิดรูปถูกเมินเงียบ ${เมินเงียบ.length} ช่อง — คนจะได้ผลของ "ไม่กรอง" โดยเชื่อว่ากรองแล้ว`);
  for (const x of เมินเงียบ) console.log(`   · ${x}`);
  console.log("   ℹ️ แก้: ตอบ 400 พร้อมบอกค่าที่รับได้ **หรือ** ประกาศใน `ignored`");
  console.log("      🔑 เคสจริงที่เจ็บที่สุด: only=negative เคยคืนทั้งคลัง 2,672 แถวให้คนที่ขอรายการติดลบ");
}
...
process.exit(ปุ่มหลอก.length || เมินเงียบ.length ? 1 : 0);
