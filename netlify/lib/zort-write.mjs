...
//
// 📌 เส้นที่ยิงของจริงตรวจแล้ว 6 ก.ย. 2569 · **ยิงซ้ำยืนยัน 20 ก.ย. 2569 (ยังเหมือนเดิมทั้ง 3 เส้น · ตัวควบคุมผ่าน 4/4)** (ส่ง {} เปล่าไม่มีคีย์ ⇒ ไม่ได้สร้างอะไร)
...
/* 🔔 **ZORT ยิงเหตุการณ์กลับมาหาเราได้** — เจอ 6 ก.ย. 2569 ตอนกวาดทั้งแผง
   ...
   📏 **อ่านของจริงแล้ว 18 ก.ย. 2569** (`GET /api/core?zortwebhook=1` · อ่านอย่างเดียว):
      ZORT ตอบ `resCode 100` · `resDesc "Invalid Webhook"` · ช่องที่ได้: detail/resCode/resDesc/resDesc2/resDesc3
      (detail = null ⇒ **ไม่ได้ค่าที่ตั้งไว้กลับมาเลย**)
   ⚠️ **คำตอบนี้แยกไม่ออก** ระหว่าง ① ร้านยังไม่ได้ตั้ง webhook ② รูปคำขอของเราผิด (ขาดพารามิเตอร์)
      ยังไม่มีตัวควบคุมที่แยกสองกรณีนี้ได้ ⇒ **ห้ามสรุปว่าร้านไม่ได้ตั้งไว้** [[ข้อความปฏิเสธที่แยกสาเหตุไม่ออก]]
   ⚠️ ยังไม่รู้ว่ามันยิงเหตุการณ์อะไรได้บ้าง — เอกสาร V4 ไม่เขียนไว้ ⇒ ต้องได้รูปคำขอที่ถูกก่อน
...
export const ZORT_WEBHOOK = {
  ...
  /* ⚠️ ค่านี้ไปโผล่บนจอ ⇒ ต้องเป็นความจริงวันนี้ ไม่ใช่ความจริงวันที่เขียน
     18 ก.ย. 2569: อ่านแล้ว (ไม่ใช่ "ยังไม่เคยอ่าน") แต่ ZORT ตอบ Invalid Webhook ⇒ ยังไม่ได้ค่า */
  status: "อ่านแล้ว 18 ก.ย. 2569 — ZORT ตอบ 'Invalid Webhook' (resCode 100) จึงยังไม่ได้ค่าที่ตั้งไว้ " +
    "และแยกไม่ออกว่าไม่ได้ตั้งไว้ หรือรูปคำขอเราผิด",
  untested: true,
};
 ...
 *  ✅ 18 ก.ย. 2569 ยิงจริงแล้ว: เป็น **"เอกสารบัญชี"** (694 แถว · ใบเสร็จรับเงิน/ใบกำกับภาษี
 *     · referencetype+referencenumber โยงกลับใบขาย-ใบซื้อ) **ไม่ใช่ไฟล์แนบ/สลิป**
 *  ⚠️ ยังคง **ห้ามแกะฟิลด์ตามชื่อที่เดาเอง**
