 ...
 *
 *  📐 **ขอบเขตของกระจกนี้: เฉพาะสินค้าที่ยังไม่ถูกลบ (ZORT เรียกว่า archive)**
 *     วัดจริง 18 ก.ย. 2569 บนจอ ZORT: ปกติ **2,899 รายการ** · ติ๊ก "แสดงสินค้าที่ถูกลบ" ได้ **14,084**
 *     ⇒ ของที่ถูกลบมี ~11,185 รายการ และ **ไม่ได้อยู่ในกระจกนี้โดยตั้งใจ**
 *  ⚠️ ไม่มีป้ายนี้ วันหนึ่งจะมีคนเทียบ 2,899 กับ 14,084 แล้วสรุปว่า "กระจกขาด 11,185 รายการ"
 *     แล้วไปไล่แก้ตัวซิงก์ที่ไม่ได้พัง (กลไกเดียวกับผู้ติดต่อ: 28,344 ไม่รวม archive 3,170 ราย)
 *  ⚠️ วันไหนต้องการของที่ถูกลบด้วย ต้องหาพารามิเตอร์ของ ZORT เพิ่ม — **ยังไม่รู้ว่ามีไหม** (ยังไม่ได้ยิงหา) */
export async function syncProducts() {
  ...
  /* ⚠️ **สินค้าที่ไม่มีรหัสเก็บเข้ากระจกไม่ได้เลยโดยโครงสร้าง** (SKU เป็นกุญแจหลัก)
      ...
      ตอนตรวจวันนั้นทั้ง 226 ตัวสต็อกเป็นศูนย์หมด มูลค่ารวม ฿0
      🔴 **ห้ามอ่านประโยคข้างบนว่า "ไม่มีตัวเลขไหนของเราผิดเพราะเรื่องนี้"** (แก้ข้อความ 6 ก.ย. 2569)
         นั่นเป็นผลการวัด**ของวันนั้น** ไม่ใช่คุณสมบัติของระบบ — วันที่มีคนใส่ของเข้าสินค้า
         ที่ไม่มีรหัสสักตัว ประโยคนั้นจะกลายเป็นเท็จทันที **โดยไม่มีอะไรฟ้อง**
         และของที่หายไปคือ **สต็อกที่ระบบเราตามไม่ได้เลย** ซึ่งเป็นเรื่องเงินจริง
      ⇒ ตัวที่ตอบว่า "ยังจริงอยู่ไหม" คือ `noSkuWithStock` ที่ส่งออกไปกับผลทุกครั้ง
         (วัดจริง 6 ก.ย. 2569 ผ่าน /api/core?list=stock: **noSkuWithStock = 0** ⇒ ยังจริงอยู่)
         **มากกว่า 0 เมื่อไหร่ = ต้องขึ้นเตือน ห้ามปล่อยผ่าน** — ดู [[stale-state-comments]]
...
/* ══ สินค้าเป็นชุด (Bundle) ══════════════════════════════════════════
   ...
   ❌ **บันทึกเดิม 3–6 ก.ย. 2569 ว่า "GetBundleDetail คืน list ว่าง ⇒ ZORT ไม่เปิด API สูตรชุด" — ผิด**
      ...
      ⇒ สูตรชุดซิงก์ด้วย syncBundleRecipes() (งานตามเวลา bundle-recipe-sync — **รอบจริงอยู่ที่ `export const config` ของไฟล์นั้น** ห้ามเขียนเลขซ้ำที่นี่ · 19 ก.ย. 2569 แยกสต็อกชุดออกเป็น bundle-stock-sync แล้ว)
...
export async function probeBundleDetail(skuIn, whIn) {
  ...
  /* wh = รหัสคลัง (เอกสาร V4: GetBundleDetail รับ warehousecode) — ใช้ดูสต็อกชุดรายคลังให้จอรายละเอียดชุด (ใบ t_mu1dfe94)
     รับเฉพาะตัวอักษร/ตัวเลข ไม่งั้นตีกลับ ⇒ คืนค่าดิบให้ดู
     📏 วัด 18 ก.ย. 2569 (ชุด 00073-11.5-KK onhand 18 · 00073-18-KK onhand 13): ส่ง wh=NEW ได้ตัวเลข **เท่ากับตอนไม่ส่งทุกช่อง**
        ส่ง wh=KLD / ANJ (คลัง POS จริงของร้าน) → resCode 100 "Access Denied." ⇒ รหัสร้านอ่านคลัง POS ไม่ได้ (เรื่องสิทธิ์ ไม่ใช่ API ไม่มี)
     ⚠️ **ตัวควบคุม: ส่ง wh=ZZZ (รหัสที่ไม่มีอยู่จริง) ก็ได้ "Access Denied." เหมือนกันเป๊ะ**
        ⇒ ข้อความนี้ **แยกไม่ออก**ระหว่าง "คลังมีจริงแต่ถูกกั้นสิทธิ์" กับ "ไม่มีคลังนี้" [[field-answers-other-question]]
     ⇒ สรุปที่พิสูจน์ได้: NEW ไม่ทำให้เลขเปลี่ยน · คลังอื่นอ่านไม่ได้เลย ⇒ **ยังตอบไม่ได้ว่า warehousecode กรองจริงไหม**
        จะตอบได้ต้องมีสิทธิ์อ่านคลังที่สอง (ท่านประธานเปิดสิทธิ์ใน ZORT) ไม่ใช่แก้ที่โค้ดเรา */
...
export async function listBundles(o = {}) {
  ...
  /* คอลัมน์ "ราคาสินค้ารวม" แบบ ZORT = ผลรวม (ราคาขายของชิ้นส่วน × จำนวน)
     ...
     ⚠️ สูตรชุดซิงก์จาก ZORT เองตั้งแต่ 14 ก.ย. 2569 (เดิมเป็นภาพนิ่ง 3 ก.ย.) — ยังส่ง recipeAt ไปให้จอโชว์เสมอ
        🔴 **ไม่เขียนรอบเวลาไว้ที่นี่** — เคยเขียนว่า "ทุกชั่วโมง" แล้วกลายเป็นเท็จวันที่แยกงาน (19 ก.ย. 2569)
        รอบจริงมาจาก cron ของ bundle-recipe-sync · ค่าที่ส่งออกคือ `recipeExpectedEveryHours` */
  let recipeAt = null;
  if (rows.length) {
  ...
  return {
    ...
    /* 🔑 **ท่อบอกรอบเวลาที่คาดหวังไปเลย — จอต้องไม่ฝังเลขเกณฑ์เอง** (เพิ่ม 19 ก.ย. 2569)
        ที่มา: เรื่องเดียวกันมีสี่ตัวเลขที่ขัดกันเอง
          ...
          ข้อความบนจอ 5 จุด = ทุกชั่วโมง · **เกณฑ์เตือนในโค้ดจอ = 6 ชม.**
        ⇒ ถ้าตารางจริงคือวันละครั้ง 10:00 ไทย เกณฑ์ 6 ชม.จะทำให้จอ **แดงประมาณ 18 ชม./วัน
          ทั้งที่งานปกติทุกอย่าง** ⇒ คนเลิกอ่านจอสถานะ ซึ่งแย่กว่าไม่มีจอ
        ⇒ ท่อรู้ตารางจริงอยู่แล้ว (cron-table สร้างจาก `export const config` ตอน build)
          ⇒ **คิดเกณฑ์ให้แล้วส่งไป** ⇒ แก้ตารางเมื่อไหร่ จอปรับเอง ไม่มีเลขฝังให้ล้าสมัย
    ...
    /* 🔑 **สต็อกชุดกับสูตรชุดเป็นคนละงานแล้ว (19 ก.ย. 2569) ⇒ คนละรอบ ⇒ ต้องส่งสองเกณฑ์**
        ...
        ⇒ ถ้าส่งเกณฑ์เดียว จอจะใช้เกณฑ์ของงานหนึ่งไปตัดสินความสดของอีกงาน
        = อาการเดิมที่ทำให้แถบแดงขึ้น 22.5 ชม./วัน เพียงแต่กลับด้าน
        ⚠️ ชื่อคีย์ต่างกันชัด (`stockExpected*` vs `recipeExpected*`) **ห้ามใช้ชื่อเดียวกัน** */
    ...(await รอบที่คาดหวัง("bundle-stock-sync", "stock")),
    ...mk, // ⚠️ recipeAt = บรรทัดสูตรชุดที่เปลี่ยนล่าสุด · recipeCheckedAt = ตรวจกับ ZORT ล่าสุด (UTC) — ห้ามใช้แทนกัน
    note:
      /* 🔴 ห้ามเขียนรอบเวลาเป็นข้อความตรงนี้ (แก้ 19 ก.ย. 2569)
         เดิมเขียนว่า "ทุกชั่วโมง" ⇒ **กลายเป็นเท็จวันเดียวกับที่ผมแยกงานสูตรเป็นวันละครั้ง**
         และไม่มีอะไรฟ้อง เพราะมันเป็นแค่สตริง
         🔑 เกณฑ์เดียวกันต้องมีแหล่งเดียว — รอบจริงมาจาก `recipeExpectedEveryHours`
            ที่ `รอบที่คาดหวัง()` คิดจาก cron จริงและ spread อยู่ในคำตอบนี้แล้ว */
      "สูตรชุดซิงก์จาก ZORT เองตามรอบใน `recipeExpectedEveryHours` (รอบละ 90 ชุด) — " +
...
export async function listBundleItems(bundleSku = "", memberSku = "") {
  ...
  return {
    ...
    /* 🔴 **ส่งทั้ง sku และ member ⇒ โหมด "ชุดนี้มีอะไรบ้าง" ชนะ และ member ถูกเมิน**
        แต่ `applied.member` ยังบอกค่าที่ส่งมา ⇒ **อ่านได้ว่าใช้กรองแล้ว ทั้งที่ไม่ได้ใช้**
        (เจอด้วยด่าน check-filters-work.mjs 19 ก.ย. 2569: ส่งคู่กันแล้วแถวเท่าเดิม)
        ⇒ ประกาศว่าเมินตัวไหน ⇒ ปลายทางเขียนขอบเขตถูกและด่านไม่ต้องเดา
        🔑 `applied` ต้องหมายถึง "ใช้จริง" เท่านั้น — ถ้ามันรวมค่าที่ไม่ได้ใช้ มันก็เป็นแค่เสียงสะท้อน */
    ...(one && member ? { ignored: { member: "ส่ง sku มาด้วย ⇒ ใช้โหมด \"ชุดนี้มีอะไรบ้าง\" · member ไม่ถูกใช้กรองรอบนี้" } } : {}),
    ...
    /* 🔴 ห้ามเขียนรอบเวลาเป็นข้อความ — ดูเหตุผลที่ note อีกตัวข้างบน (เคยเป็นเท็จมาแล้ว) */
    note: "สูตรชุดซิงก์จาก ZORT เองตามรอบใน `recipeExpectedEveryHours` (รอบละ 90 ชุด) · collectedAt = สูตรเปลี่ยนล่าสุด · recipeCheckedAt = ตรวจกับ ZORT ล่าสุด (UTC)",
    sku: one || undefined,
    rows,
  };
}
...
export async function blockedByNegative() {
  ...
  return {
    ...
    roots: roots.length,
    blockedSkus,
    /* 📋 **ตรวจว่าโค้ดแบ่งกองครบ — ไม่ได้ตรวจข้อมูล** (เขียนคำใหม่ 11 ก.ย. 2569)
        ค่านี้ **เป็นจริงเสมอโดยโครงสร้าง**: services กับ goods เป็น filter คู่ตรงข้ามบน `neg`
        ตัวเดียวกัน (รวมกันได้ neg เสมอโดยนิยาม) แล้ว goods ถูกแยกเป็น sellable/roots
        ด้วย if/else ที่ไม่ทิ้งแถว ⇒ ทุกข้างของการเทียบคำนวณจาก `neg` ก้อนเดียว
        ⇒ ไม่มีค่าชุดไหนทำให้เป็นเท็จได้ **นอกจากมีคนเพิ่มกองใหม่แล้วลืมบวกในบรรทัดนี้**
        ซึ่งเป็นความผิดตอนแก้โค้ด ไม่ใช่ความผิดของข้อมูล

        🚫 **ห้ามเรียกค่านี้ว่า "ตาข่าย" หรือ "ตัวตรวจ"** (CEO กำชับ 11 ก.ย. 2569) —
           คนอ่านผ่าน ๆ จำจากคำ ไม่ได้จำจากคำอธิบาย แล้ววันหนึ่งจะมีคนพูดว่า
           "จุดนั้นมีตาข่ายอยู่แล้ว" ทั้งที่มันไม่เคยจับอะไรได้เลย
           (เกิดขึ้นจริงกับ shopee-stock.mjs มาแล้ว — คอมเมนต์อ้างถึงตาข่ายที่ไม่มีอยู่จริง)

        ❓ ทำไมไม่แก้ให้มีคม: ตัวหารจากนอกกองที่พอเป็นไปได้คือนับแถวติดลบจาก D1 อีกคิวรี
           ⇒ จับได้แค่ "ข้อมูลเปลี่ยนระหว่างสองคิวรี" ซึ่งเกิดยากมาก แลกกับการยิง D1
           เพิ่มหนึ่งรอบทุกครั้งที่เปิดจอ ทั้งที่ D1 ช้าอยู่แล้ว ⇒ จ่ายด้วยของจริงเพื่อซื้อ
           ความสบายใจ · **ตัดสินใจไม่แก้ (CEO 11 ก.ย. 2569)** ⇒ ค่านี้อยู่ต่อในฐานะ
           คู่มือสำหรับคนที่จะมาเพิ่มกอง ไม่ใช่ในฐานะสิ่งที่เฝ้าข้อมูลให้
        ⚠️ เพิ่มกองใหม่ต้องมาบวกในบรรทัดนี้ด้วยทุกครั้ง */
...
export async function reorderPlan(o = {}) {
  ...
  for (const { sku: parent, kids, isParent } of targets) {
    ...
    rows.push({
      ...
      /* ⚠️ **ใบขายของ ZORT นับเส้นไม่ได้ และจะนับไม่ได้ตลอดไป** (ยืนยัน 5 ก.ย. 2569)
          ร้านคีย์ขายโซ่ตัดด้วย **รหัสม้วน + จำนวนเป็นฟัน** (เช่น `00894` จำนวน 30)
          ไม่ได้คีย์รหัสความยาว (`00894-30T`) ⇒ ใบขายรู้แค่ "ตัดฟันไปเท่าไหร่"
          **ไม่รู้ว่าแตกเป็นกี่เส้น** และข้อต่อผูกกับ "จำนวนเส้น" ไม่ใช่ "จำนวนฟัน"
          ⇒ `chainsCut` เป็น 0 แทบทุกแถวโดยธรรมชาติ **ห้ามอ่านว่า "ไม่มีการตัด"**
             อ่านว่า **"นับจากใบขายไม่ได้"** — จึงมี `chainsCountable` กำกับไว้ให้จอไม่เข้าใจผิด
          ⚠️ ห้ามเอา avgT ที่เดาเองมาคูณให้ตัวเลขดูสวย
             5,060.5 ฟัน เป็นได้ทั้ง 169 เส้น (30T) และ 107 เส้น (47T) — ต่างกัน 62 คู่ */
      chainsCut: cut,
      chainsCountable: cut > 0,
      /* คำนวณย้อนกลับได้แค่ "ช่วง" ⇒ **คืนช่วง ห้ามคืนจุดกึ่งกลาง**
          สั้นสุดในบันได = ได้เส้นเยอะสุด = ใช้ข้อต่อเยอะสุด (และกลับกัน) */
      chainsEstMin: minT && used > 0 ? Math.floor(used / maxT) : null,
      chainsEstMax: minT && used > 0 ? Math.floor(used / minT) : null,
      ladderTeethMin: minT,
      ladderTeethMax: maxT,
  ...
  return {
    ...
    /* ⚠️ **ยอดใช้ยังไม่ได้หักของที่ลูกค้าคืน** — ต้องบอกคนกดสั่งของ ไม่ใช่เก็บไว้ในคอมเมนต์
        (ฝั่งจอเจอบั๊กเดียวกันในแผนของเขา 7 ก.ย. 2569 แล้วเลือกท่านี้: ไม่แตะวิธีคิดเลข
         แต่ติดป้ายให้เห็น — **แผนที่คำนวณไม่ได้เลย แย่กว่าแผนที่มีป้ายเตือน**)
        วัดขนาดแล้ว: 90 วันมีใบคืน 45 ใบ · ฿26,399 ⇒ เล็กเมื่อเทียบยอดขายรวม
     ⚠️ **แต่ยอดรวมไม่ใช่เกณฑ์ที่ถูก** (ฝั่งจอเสนอ) — ต้องดู **รายรหัสที่แย่ที่สุด**
        รหัสไหนที่ของคืนเกิน ~10% ของยอดขายตัวเอง แผนจะสั่งเกินสำหรับรหัสนั้นอย่างมีนัย
        **ยังไม่ได้วัดรายรหัส** — ต้องดึงรายการในใบคืนทีละใบ (682 ใบ) ซึ่งยังไม่ได้ทำ
        ⇒ อย่าเพิ่งสรุปว่า "ของคืนน้อย ไม่มีผล" จากยอดรวมอย่างเดียว */
    /* ⚠️ **ข้อความนี้เคยเขียนว่า "ยังไม่ได้ตรวจรายรหัส" — ตรวจแล้ว 7 ก.ย. 2569**
       ตรวจด้วย `/api/core?returnskus=1&days=90` (ดึงรายการในใบคืนทีละใบ อ่านครบ 45/45)
       ⚠️ ตัวเลขในนี้เป็น **ผลวัดของวันนั้น ไม่ได้อัปเดตตาม** — สงสัยเมื่อไหร่ยิงซ้ำได้
          ([[stale-state-comments]] · [[nets-expire-silently]]) */
    returnsNote:
      "ยอดใช้คิดจากใบขาย **ยังไม่ได้หักของที่ลูกค้าคืน** ⇒ ตัวเลขความต้องการสูงกว่าจริงเล็กน้อย · " +
      "**ตรวจรายรหัสแล้ว 7 ก.ย. 2569** (90 วัน · 45 ใบ · อ่านครบทุกใบ): รหัสที่ขายจริงเป็นพัน " +
      "อัตราคืน 1.1–4.5% ทุกตัว · ไม่มีรหัสไหนที่คืนเกิน 10% ของยอดขายตัวเองเลยในกลุ่มที่ขาย ≥20 ชิ้น " +
      "⇒ **แผนไม่ได้สั่งเกินอย่างมีนัย** · ยิงซ้ำได้ที่ /api/core?returnskus=1",
    chainsNote:
      "ร้านคีย์ขายโซ่ตัดด้วยรหัสม้วน + จำนวนเป็นฟัน ไม่ได้คีย์รหัสความยาว ⇒ ใบขายไม่รู้ว่าตัดกี่เส้น · " +
