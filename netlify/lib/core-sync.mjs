...
export async function zortOrderCountForMonthAll(ym) {
  ...
  return {
    ok: true,
    ym,
    /* 🔒 **`store:"all"` + `stores` เป็นหลักฐานที่ฝั่งจอใช้ตัดสิน ห้ามถอดสองช่องนี้**
       ท่อรุ่นก่อน ee196c1 ไม่รู้จัก `all` แล้ว **ตกกลับ z1 เงียบ ๆ ตอบ 200 หน้าตาปกติ**
       ⇒ ถ้า deploy เหลื่อม (จอขึ้นก่อนท่อ) หรือ rollback ท่อ บั๊กเทียบข้ามขอบเขตกลับมาทันที
       จอจึงเช็คว่า `store === "all"` และมี `stores` จริงไหม — ไม่ผ่าน = ถอยไปถามทีละร้าน
       (ฝั่งจอทดสอบทางนี้ด้วย fixture จำลองท่อรุ่นเก่าแล้ว 9 ก.ย. 2569 · จับได้จริง)
       ⚠️ ใครเห็นว่า `stores` ซ้ำซ้อนกับ `perStore` แล้วลบทิ้ง = จอจะถอยไปทางเก่า
          **ตลอดกาลโดยไม่มีอะไรฟ้อง** เพราะทางถอยก็ให้เลขถูกเหมือนกัน แค่ช้ากว่า */
    store: "all",
    stores: tags,
    from: parts[0].from,
    to: parts[0].to,
    ...
    ...(() => {
      ...
      return {
        zortCount: count,
        zortAmount: amount,
        /* 🔑 **ส่งชื่อคีย์เป็นอังกฤษคู่กับไทย — คีย์อังกฤษคือตัวที่เครื่องอ่าน** (18 ก.ย. 2569)
           ฝั่งจอมีด่าน check-pipe-keys ที่กวาดว่าชื่อคีย์ที่จอใช้มีอยู่ในซอร์สท่อจริงไหม
           แต่ด่านนั้น **มองคีย์ภาษาไทยไม่เห็น** ⇒ จอต้องลงทะเบียนยกเว้นเป็นราย ๆ
           ⇒ วันที่ฝั่งท่อเปลี่ยนชื่อคีย์ไทย ด่านจะไม่ฟ้อง และจอจะพังเงียบ
           ⇒ ตัดความเสี่ยงที่ราก: มีคีย์อังกฤษให้ด่านจับได้ · คีย์ไทยเก็บไว้ให้คนอ่าน log เข้าใจ
           ⚠️ ทั้งสองคีย์มาจาก **ตัวแปรเดียวกัน** (unknown) ไม่ได้คำนวณสองรอบ
              ⇒ ไม่มีทางที่สองคีย์ขัดกันเอง (ถ้าคำนวณแยก จะกลายเป็นสองแหล่งความจริง) */
        ...(unknown.length
          ? {
