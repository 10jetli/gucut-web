...
/* 🔴🔴 **20 ก.ย. 2569 — ด่านนี้คืน "ข้อความ" เดียว แล้วปลายทางไม่รู้ว่าเป็นนโยบายหรือของเสีย**
   ฝั่งจอกางของจริง: `?pushstuck=1&limit=200` ⇒ **109 จาก 200 แถวที่มี `last_error`
   คือข้อความของด่าน ⑧** ซึ่งเป็น **การปฏิเสธโดยนโยบาย ไม่ใช่ความล้มเหลวของแพลตฟอร์ม**
   ⇒ ตัวนับ `มีข้อผิดพลาด 113` อ่านตรง ๆ ไม่ได้ · ถ้าไม่กางจะรายงานว่า "ดันสต็อกพัง 113 รหัส"
   🔑 ต้นเหตุตรงเป๊ะกับคำเตือนที่ `stock-push-live.mjs` เขียนไว้เองเมื่อ 18 ก.ย.:
      *"เพิ่มเหตุ not_sent ใหม่เมื่อไหร่ **ต้องใส่ `notSentKind` ด้วยทุกครั้ง**
        ไม่ใส่ = ปลายทางจัดลงกองผิด แล้วไม่มีอะไรฟ้อง"*
      ⇒ ด่านนี้กับ `stock-push-common.mjs` **ไม่เคยใส่สักจุด (4 จุด)** ⇒ คำเตือนนั้นเป็นจริงแล้ว
   ✅ จึงคืน `{ ชนิด, why }` — `ชนิด` คือค่าที่เครื่องใช้ตัดสิน · `why` คือคำอธิบายให้คนอ่าน
   🚫 **ห้ามให้ปลายทางเดาชนิดจากข้อความ** (เช่น `/ใบค้างส่ง/.test(why)`) — นั่นคือการให้
      ฟิลด์คำอธิบายมีอำนาจตัดสินใจ ซึ่งเป็นคลาสที่ทีมเจอซ้ำที่สุด [[explain-fields-cant-decide]]
   @returns {null | { ชนิด: "policy_hold"|"policy_down"|"needs_human", why: string }} */
export function ด่านบนชั้น(r, { ค้างส่ง, allowClose = false, confirmReopen = new Set() }) {
  /* อ่านรายการใบค้างส่งไม่ได้ ⇒ **ต้องมีคนดู** (ไม่ใช่นโยบาย) — ระบบต้องพังไปทาง "มีคนมาดู" */
  if (!(ค้างส่ง instanceof Set)) {
    return { ชนิด: "needs_human", why: "ไม่มีรายการใบค้างส่ง — ตรวจด่าน ⑧ ไม่ได้ ไม่ยิง" };
  }
  if (ค้างส่ง.has(String(r.sku))) {
    return { ชนิด: "policy_hold", why: "อยู่ในใบค้างส่ง (ด่าน ⑧) — ของบนชั้นอาจหยิบไม่ได้ ห้ามยิงอัตโนมัติ" };
  }
  if (r.kind === "reopen" && !confirmReopen.has(String(r.sku))) {
    return { ชนิด: "policy_hold", why: `เปิดขายคืน (reopen ${r.from}→${r.to}) ต้องมีคนยืนยันรายรหัสใน confirmReopen (ด่าน ⑥)` };
  }
  if ((r.kind === "close" || r.kind === "down") && !allowClose) {
    return { ชนิด: "policy_down", why: `ทิศลง (${r.kind} ${r.from}→${r.to}) ต้องสั่งแยกด้วย allowClose:true` };
  }
  return null;
}
