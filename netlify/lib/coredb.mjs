...
/** 🧪 **ตัวตัดสินว่า error ของคำสั่งแก้โครง "เป็นเรื่องปกติ" หรือไม่** — แยกออกมาเพื่อ **ทดสอบได้**
 *
 * 🔑 ทำไมต้องแยก: ตัวแก้ที่ฝังอยู่ในลูปที่ต้องมี D1 จริงถึงจะเดิน **ไม่มีทางถูกพิสูจน์**
 *    ⇒ วันนี้ผมแก้ `.catch(() => null)` เป็นการแยกชนิด error แล้ว **เทสยังเท่าเดิม 709**
 *    ⇒ ⇒ แปลว่า **ไม่มีอะไรยืนยันว่าการแยกนั้นถูก** ⇒ จึงยกออกมาเป็นฟังก์ชันที่เทสเรียกได้
 *
 * 🚫 **ปกติมีอย่างเดียว: "คอลัมน์นี้มีอยู่แล้ว"** — เพราะ `ALTER ... ADD COLUMN` ถูกยิงซ้ำทุกครั้งที่ init
 *    อย่างอื่นทั้งหมด (ตารางไม่มี · ชื่อผิด · สิทธิ์ไม่พอ · ฐานล่ม) **ต้องส่งเสียง**
 *    ⇒ ห้ามเผลอเติมรูปแบบใหม่เข้ามาเพราะ "เห็นมันโผล่บ่อย" — บ่อยไม่ได้แปลว่าปกติ */
export function ddlพลาดแบบปกติ(ข้อความ) {
  return /duplicate column name/i.test(String(ข้อความ ?? ""));
}
...
async function d1Fetch(token, sql, params) {
  ...
  /* 🔴 **ถ้ามีผลมากกว่าหนึ่งชุด ต้องโยน ไม่ใช่หยิบชุดแรกแล้วทิ้งที่เหลือเงียบ ๆ** (เพิ่ม 19 ก.ย. เย็น)
      วันนี้ `coreQuery` ส่งประโยคเดียวเสมอ ⇒ `result` มีชุดเดียว ⇒ **ถูกโดยโครงสร้าง ไม่ใช่โดยตรวจ**
      ⇒ วันที่มีคนส่งหลายประโยคผ่านตัวนี้ (D1 รองรับ) **ผลชุดที่ 2..n จะหายเงียบ**
        และผู้เรียกจะได้ข้อมูลของประโยคแรกไปใช้แทน โดยไม่มีอะไรฟ้อง
      🔑 คลาส "ของที่มีมากกว่าหนึ่ง" — มิติที่เราชินป้อน *รูป* ของข้อมูล แต่ลืม *จำนวน* */
  const ชุดผล = Array.isArray(data.result) ? data.result : [];
  if (ชุดผล.length > 1) {
    throw new Error(
      `D1 คืนผล ${ชุดผล.length} ชุด แต่ coreQuery อ่านได้ชุดเดียว ⇒ ผลที่เหลือจะหายเงียบ ` +
      "— ส่งประโยคเดียวต่อครั้ง หรือทำตัวอ่านแบบหลายชุดขึ้นมาใหม่"
    );
  }
  return ชุดผล[0]?.results ?? [];
}
...
export async function coreInit() {
  ...
  /* 🔑 เก็บคำสั่งแก้โครงที่พลาด **ไม่ใช่กลืนทิ้ง** (เพิ่ม 20 ก.ย. 2569) */
  const ddlพลาด = [];
  for (const sql of [
    ...
    /* 🔴 **กลืนได้เฉพาะ "คอลัมน์นี้มีอยู่แล้ว" — อย่างอื่นต้องนับและรายงาน** (20 ก.ย. 2569)
       เดิม `.catch(() => null)` **กลืนทุกชนิด** ⇒ ALTER ซ้ำไม่พัง (ซึ่งเป็นเจตนา)
       **แต่ ALTER ที่ผิดจริงก็เงียบด้วย** (ชื่อคอลัมน์ผิด · ตารางยังไม่มี · สิทธิ์ไม่พอ)
       ⇒ ⇒ คอลัมน์ไม่ถูกเพิ่ม **โดยไม่มีใครรู้** แล้ว `SELECT` ที่อ่านคอลัมน์นั้นตอบ 500 ทีหลัง
          ⇒ **อาการโผล่คนละที่คนละเวลากับต้นเหตุ** — เกิดจริงแล้ว 15 ก.ย. 2569
            (`list=orders` ตอบ 500 "no such column: tag" 3 นาที)
       🔑 `.catch(() => null)` บนของนอกบ้าน = **จุดต้องสงสัยเสมอ**
          ⇒ ที่ถูกคือกลืน **เฉพาะกรณีที่เรารู้ว่าไม่ใช่ปัญหา** แล้วให้ที่เหลือส่งเสียง */
    try {
      await coreQuery(sql, [], { heal: false });
    } catch (e) {
      const ข้อความ = String(e?.message || e);
      if (ddlพลาดแบบปกติ(ข้อความ)) continue;   // ยิงซ้ำ = ปกติ ตามเจตนาเดิม
      ddlพลาด.push({ sql: String(sql).slice(0, 90), เหตุ: ข้อความ.slice(0, 140) });
    }
  }
  ...
  /* 🔑 **ส่งรายการที่พลาดออกไป ไม่ใช่แค่จำนวน** — กติกาที่ตกลงกับฝั่งจอ 20 ก.ย. 2569:
     *ตัวเลขทุกตัวต้องมีรายการที่ขยายได้* ⇒ `ddlพลาด: []` = ไม่มีปัญหา (ไม่ใช่ "ยังไม่รู้") */
  return {
    tables: stmts.filter((s) => /CREATE\s+TABLE/i.test(s)).length,
    ddlพลาด,
    "⚠️ ddlพลาด อ่านยังไง": "คำสั่งแก้โครงที่ **ไม่ใช่** 'คอลัมน์มีอยู่แล้ว' ⇒ โครงตารางอาจไม่ครบ ⇒ SELECT ที่อ่านคอลัมน์ใหม่จะพังทีหลัง",
  };
}
