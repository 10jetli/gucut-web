...
/* 🔴 **20 ก.ย. 2569 — ทางถอยตัวนี้ไม่เคยประกาศตัว**
   ฝั่งจออ่านจอ POS แล้วสรุปว่า *"จำนวนสาขาเป็นข้อมูล ไม่ใช่โค้ด"* (เพราะจอเขียนว่า "ดึงจากเซิร์ฟเวอร์")
   📏 ยิงวัดจริง: `?list=branches` คืน 2 สาขา (KLD · ANJ) **แต่ `POS_BRANCHES` ไม่ได้ตั้งค่า**
      ⇒ ของจริงคือ **ค่าตั้งต้นในโค้ดข้างล่างนี้** ⇒ "2" วันนี้ **เป็นโค้ด ไม่ใช่ข้อมูล**
   🔑 ⇒ คำตอบที่ส่งให้จอ **ต้องบอกที่มาของรายชื่อ** ไม่งั้นปลายทางแยกไม่ออกว่า
      "ร้านตั้งค่าไว้ 2 สาขา" กับ "ไม่มีใครตั้งค่า แล้วโค้ดเดาให้ 2"
      ⇒ วันที่ร้านเปิดสาขาที่สาม จะไม่มีอะไรบอกว่าต้องไปตั้ง env (ทางถอยกลายเป็นทางหลักเงียบ ๆ) */
export function ที่มาของรายชื่อสาขา() {
  return String(process.env.POS_BRANCHES ?? "").trim()
    ? "env POS_BRANCHES"
    : "ค่าตั้งต้นในโค้ด (ไม่มีใครตั้ง POS_BRANCHES) ⇒ จำนวนสาขานี้ยังไม่ได้ถูกยืนยันโดยร้าน";
}
 ...
 *
 * 🔴 **ตัวกรอง channel/status/q เพิ่ม 19 ก.ย. 2569 — ก่อนหน้านี้ปุ่มบนจอเป็นปุ่มหลอก**
 *    จอ POS ส่งสามตัวนี้มาตลอด · ท่อเมินเงียบ ๆ · **และจอไม่ได้กรองในเบราว์เซอร์ด้วย**
 *    ⇒ คนเลือก "Shopee" แล้วเห็นทุกช่องทาง **โดยเชื่อว่ากรองแล้ว**
 *    ⇒ ตารางดูถูกต้องทุกประการ เพราะแถวมีคอลัมน์ channel/status ติดมาอยู่แล้ว (ไว้แสดง)
 *    🔑 ฝั่งจอเป็นคนจับได้ และเลือก **ไม่ตัดสินใจเรื่องปุ่มแทนคนใช้** ⇒ ท่อทำตัวกรองจริงให้
 *       ปุ่มจึงทำงานตามป้ายที่มันเขียนไว้ตั้งแต่แรก (ไม่ต้องถอดปุ่ม ไม่ต้องสอนคนใช้ใหม่)
 * ⚠️ `channel`/`status` เทียบ **ตรงตัว** ไม่ใช่ LIKE — ค่ามาจากตัวเลือกบนจอ ไม่ใช่คำพิมพ์อิสระ
 *    ใช้ LIKE เมื่อไหร่ "Success" จะลากคำอื่นที่มีคำนั้นประกอบมาด้วยเงียบ ๆ (กติกาเดียวกับ list=orders)
 * ⚠️ `q` ต้องใช้ `contains()` **ห้ามเขียน LIKE '%…%'** — D1 จำกัดรูปแบบ LIKE 50 ไบต์
 *    ชื่อไทย ≥17 ตัวทำทั้งคำขอล้ม 500 (บทเรียน 15 ก.ย. 2569)
 * ⚠️ ยอดสรุป (`total`/`totalAmount`/`byPay`) **คิดจากทั้งวัน ไม่ตามตัวกรอง** โดยตั้งใจ
 *    เพราะเป็นตัวเลขปิดยอดสิ้นวัน ⇒ ต้องไม่ขยับเวลาใครกดกรองดู
 *    🔑 ส่ง `rowsScope` บอกความต่างนี้ออกไป ไม่ให้จอเอายอดสรุปไปคู่กับแถวที่กรองแล้วโดยเข้าใจผิด
 */
export async function listSales({ day = "", limit = 50, channel = "", status = "", q = "" } = {}) {
  if (!coreReady()) return { error: "ยังไม่ได้ตั้ง CLOUDFLARE_D1_TOKEN" };
  const d = DAY.test(String(day)) ? day : thaiToday();
  const lim = Math.max(1, Math.min(200, num(limit) || 50));
  const ช่องทาง = String(channel ?? "").trim().slice(0, 40);
  const สถานะ = String(status ?? "").trim().slice(0, 40);
  const คำค้น = String(q ?? "").trim().slice(0, 60);
  const เงื่อนไข = [`source = ${esc(POS_SOURCE)}`, `order_date = ${esc(d)}`];
  const ค่า = [];
  if (ช่องทาง) เงื่อนไข.push(`COALESCE(channel,'') = ${esc(ช่องทาง)}`);
  if (สถานะ) เงื่อนไข.push(`COALESCE(status,'') = ${esc(สถานะ)}`);
  if (คำค้น) {
    เงื่อนไข.push(`(${contains("number")} OR ${contains("customer")})`);
    ค่า.push(คำค้น, คำค้น);
  }
  const rows = await coreQuery(
    `SELECT number, channel, status, amount, customer, order_date, pay_method
     FROM orders WHERE ${เงื่อนไข.join(" AND ")}
  ...
  return {
    ...
    rows,
    /* 📐 **เส้นนี้พิจารณาแค่สองตัวนี้** (ประกาศเอง 19 ก.ย. 2569 · ฝั่งจอขอ)
       🔴 ที่มา: จอยอดขายส่ง `channel` · `q` · `status` มาด้วย แล้ว **ท่อเมินทั้งสามตัวเงียบ ๆ**
          ⇒ แถวที่คืนไปมีคอลัมน์ channel/status ติดมาด้วย (ไว้แสดง) ⇒ ดูเหมือนกรองแล้ว
          🔑 ปุ่มกรองที่ส่งค่าไปแล้วไม่มีผล = ปุ่มหลอก และไม่มีอะไรฟ้องเลยทั้งสองฝั่ง
       ⚠️ `limit` คือค่า **หลังหนีบ** (1–200) ไม่ใช่ค่าที่ผู้เรียกส่งมา ⇒ จอเทียบแล้วรู้ว่าถูกหนีบ
       ⚠️ `day` คือวันที่ **ที่ใช้จริง** — ส่งรูปแบบผิดมา ท่อใช้ "วันนี้ (เวลาไทย)" แทน
          ⇒ ปลายทางต้องอ่านค่านี้ ห้ามสมมติว่าได้วันที่ที่ตัวเองส่งไป */
    applied: { day: d, limit: lim, channel: ช่องทาง || null, status: สถานะ || null, q: คำค้น || null },
    appliedNote:
      "เส้นนี้อ่านจากกระจกใน D1 (ออเดอร์หน้าร้าน POS) **ไม่ได้ยิง ZORT สด** ⇒ ยิงตรวจซ้ำได้ไม่มีค่าใช้จ่ายกับโควตา ZORT",
    /* 🔑 บอกให้ชัดว่าเลขไหนตามตัวกรอง เลขไหนไม่ตาม — ไม่งั้นจอวางคู่กันแล้วคนอ่านว่าขัดแย้ง */
    rowsScope: {
      rows: "ตามตัวกรองที่ส่งมา (channel/status/q) และตัดที่ limit",
      "total/totalAmount/byPay": "**ทั้งวัน ไม่ตามตัวกรอง** (ยกเว้นใบที่ยกเลิก) — เป็นตัวเลขปิดยอดสิ้นวัน",
      "🔑 ถ้าจอจะโชว์ยอดของผลที่กรองแล้ว": "ต้องบวกจาก rows เอง และเขียนกำกับว่าเป็นยอดของที่กรอง ไม่ใช่ยอดวัน",
    },
  };
}
...
/** กรองแถวตามรหัสหมวด — **แยกออกมาเพื่อทดสอบได้โดยไม่ต้องมีฐานข้อมูล**
 *  คืน { picked, unknownCat, guessedRows }
 *  🔴 **หมวดที่ไม่รู้จักต้องบอกว่าไม่รู้จัก ห้ามคืน 0 แถวเงียบ ๆ** — อาการของบั๊กที่เพิ่งแก้
 *     (12 ก.ย. 2569) คือ 0 แถวที่หน้าตาเหมือน "หมวดนี้ไม่มีของ" ทุกประการ ⇒ ไม่มีใครสงสัย
 *  ⚠️ **ตัวเดาจากชื่อต้องประกาศตัวเมื่อถูกใช้** (guessedRows) ไม่งั้นมันกลายเป็นทางหลัก
 *     อีกรอบโดยไม่มีใครรู้ — ซึ่งเป็นต้นตอของบั๊กนี้ตั้งแต่แรก */
export function filterByCategory(rows, group) {
  const list = Array.isArray(rows) ? rows : [];
  if (!group) return { picked: list, unknownCat: false, guessedRows: 0 };
  const known = new Set(list.map((r) => categoryOfRow(r).code));
  const picked = list.filter((r) => categoryOfRow(r).code === group);
  return {
    picked,
    unknownCat: !known.has(group),
    guessedRows: picked.filter((r) => !categoryOfRow(r).zort).length,
  };
}
...
export async function lookup(q, limit = 20, cat = "", offset = 0) {
  ...
  /* ── กรองหมวด ── **ต้องใช้กติกาเดียวกับที่ `posCats()` ใช้สร้างปุ่ม** ──────────────
     🔴 **บั๊กที่แก้ตรงนี้ (12 ก.ย. 2569) — วัดครบทั้ง 57 ปุ่มแล้วจึงแก้**
        ของเดิมกรองด้วย `groupOf(name, sku)` ซึ่ง **เดาหมวดจากชื่อสินค้า** เท่านั้น
        แต่ `posCats()` สร้างรหัสปุ่มจาก **หมวดจริงในทะเบียนสินค้า** (`z:<ชื่อหมวด>`) เป็นหลัก
        ⇒ รหัส `z:` ไม่มีทางตรงกับผลของ groupOf ได้เลย
        ⇒ **ปุ่มหมวดของ ZORT ทั้ง 42 ปุ่ม (ครอบ 2,533 รหัส = 95% ของคลัง) กดแล้วได้ 0 แถว**
           และมันเงียบ: 0 แถวหน้าตาเหมือน "หมวดนี้ไม่มีของ" ทุกประการ
        ⇒ และ 11 จาก 15 ปุ่มที่เดาจากชื่อ เลขบนปุ่มก็ไม่ตรงกับของที่ได้
           (p-8800 ปุ่มบอก 1 แต่คืน 462 — เพราะปุ่มนับเฉพาะตัวที่ไม่มีหมวดจริง
            ส่วนตัวกรองเดาทับทุกตัวแม้ตัวที่มีหมวดจริงอยู่แล้ว)
        🔑 คอมเมนต์ในไฟล์นี้เตือนเรื่องนี้ไว้เองว่า "ตัวเลข items ต้องมาจากกติกาชุดเดียวกับ
           ที่ poslookup ใช้กรอง ห้ามคิดคนละทาง" — เตือนไว้ก่อน แล้วถูกละเมิดตอนเพิ่มหมวด ZORT
     ⇒ กติกาเดียวที่ถูก (ตรงกับ posCats เป๊ะ): มีหมวดจริง ⇒ `z:<หมวดจริง>` ·
        ไม่มีหมวดจริงเท่านั้น ⇒ ค่อยเดาจากชื่อ */
  const { picked, unknownCat, guessedRows } = filterByCategory(rows, group);
  return {
    day,
    cat: group || null,
    ...(unknownCat
      ? {
          unknownCat: true,
          note: `ไม่รู้จักหมวด "${group}" — ไม่มีสินค้าในคลังที่อยู่หมวดนี้ (ไม่ใช่หมวดว่าง)`,
        }
      : {}),
    /* ฐานของหมวดนี้: จากทะเบียน ZORT หรือเดาจากชื่อ — จอเอาไปบอกคนใช้ได้ */
    catBasis: group ? (group.startsWith("z:") ? "zort" : "guessed") : null,
    guessedRows,
...
//
// 📜 **ประวัติของตัวเดาหมวดจากชื่อ — อ่านก่อนคิดจะใช้มันเป็นทางหลักอีก**
//    เคยเขียนไว้ตรงนี้ว่า "ZORT ไม่มีหมวดหมู่ในข้อมูลสินค้าเลย — สุ่มตรวจ 200 ตัว
//    ได้ category = null ทุกตัว" ⇒ **ตอนที่เขียน (ต้นเดือน ก.ย. 2569) น่าจะจริง**
//    และนั่นคือเหตุผลที่ตัวเดาจากชื่อถูกสร้างขึ้นมา — ตอนนั้นถูกต้องแล้ว
//
// 🔴 **วันนี้ไม่จริงอีกแล้ว — วัดของจริง 12 ก.ย. 2569**
//    `list=categories` ตอบ: หมวดจริงใน ZORT **42 หมวด ครอบ 2,672 รหัส**
//    เหลือไม่มีหมวดเพียง **139 ตัว (5%)** · `matchesZort: true` · คัดค่ามาเมื่อ 3 ก.ย. 2569
//    (คอมเมนต์ที่บรรทัด ~501 ในไฟล์นี้เองก็เขียนว่า 2,533 จาก 2,898 ตัวมีหมวด = 87%
//     ⇒ สองบรรทัดในไฟล์เดียวกันขัดกันเอง และบรรทัดเก่าคือฝ่ายที่ตายแล้ว)
//
// 🔑 **บทเรียนที่ต้องเก็บ ไม่ใช่แค่แก้ตัวเลข** (CEO ชี้ 12 ก.ย. 2569)
//    ตัวเดานี้ถูกสร้างเพราะสมมติฐาน "ZORT ไม่มีหมวด" ซึ่งจริงในวันนั้น
//    แล้ววันหนึ่ง ZORT มีหมวด **แต่ไม่มีอะไรไปบอกตัวเดาว่าเหตุผลของมันหมดอายุ**
//    ⇒ คลาส "ทางถอยที่สมมติฐานหายไปใต้เท้า" ([[fallback-outlived-its-assumption]])
//    ⚠️ และการ "สุ่มตรวจ 200 ตัวแล้วสรุปว่าไม่มีเลย" ก็เป็นคลาส
//       "สุ่มแล้วผ่าน ≠ ตรวจแล้วครบ" ซ้อนอีกชั้น ([[sampling-is-not-proof]])
//
// ⇒ **ตัวเดาจากชื่อยังอยู่ และห้ามลบ** แต่สถานะเปลี่ยนเป็น **ทางสำรองของ 139 ตัวที่ไม่มีหมวดจริง**
//    เท่านั้น · หมวดจริงจากทะเบียน ZORT เป็นหลักเสมอ (ดูตัวกรองใน `lookup` และ `posCats`)
//    และ **ตอนใช้ตัวเดา ต้องประกาศตัว** — `lookup` คืน `catBasis` กับ `guessedRows` มาให้จอบอกคนใช้
//
...
/** 🔑 **กติกาหมวดของสินค้าหนึ่งแถว — มีที่เดียวในไฟล์นี้ ใช้ทั้งตัวสร้างปุ่มและตัวกรอง**
 *
 *  🔴 ทำไมต้องรวมเป็นฟังก์ชันเดียว (12 ก.ย. 2569): ก่อนหน้านี้สองฝั่งเขียนกติกาแยกกัน
 *     `posCats()` ใช้หมวดจริงก่อน (`z:<หมวด>`) · `lookup()` เดาจากชื่อล้วน
 *     ⇒ ปุ่ม 42 ปุ่มของหมวด ZORT กดแล้วได้ 0 แถว และ 11 ปุ่มที่เหลือเลขไม่ตรงกับของที่ได้
 *     คอมเมนต์ข้างล่างเตือนเรื่องนี้ไว้เองแล้ว (“ตัวเลข items ต้องมาจากกติกาชุดเดียวกัน”)
 *     แต่การเตือนด้วยคำพูดกันไม่ได้ — **รวมเป็นฟังก์ชันเดียวคือสิ่งที่กันได้จริง**
 *     ⇒ ใครจะเปลี่ยนกติกา แก้ที่นี่ที่เดียว แล้วทั้งสองฝั่งเปลี่ยนพร้อมกันโดยไม่มีทางหลุด
 *
 *  คืน { code, name, zort } · `zort:false` = เดาจากชื่อ (ของ 139 ตัวที่ไม่มีหมวดจริง)
 */
export function categoryOfRow(row) {
  const real = String(row?.cat_real ?? row?.cat ?? row?.category ?? "").trim();
  if (real) return { code: `z:${real}`, name: real, zort: true };
  const g = groupOf(row?.name, row?.sku);
  return { code: g.code, name: g.name, zort: false };
}
...
export async function posCats() {
  ...
  for (const r of rows) {
    ...
    /* ใช้ฟังก์ชันกลางตัวเดียวกับ lookup — ห้ามเขียนกติกาซ้ำที่นี่อีก */
    const g = categoryOfRow(r);
    const { code, name } = g;
    if (g.zort) fromZort += 1; else guessed += 1;
    const cur = count.get(code) ?? { code, name, items: 0, zort: g.zort };
    cur.items += 1;
    count.set(code, cur);
  }
...
export async function listCategories() {
  ...
  return {
    ...
    zortTotalValue: cv.map.size
      ...
      : null,
    /* ⚠️ **จอนี้ไม่มีการเดาหมวดจากชื่อสินค้าเลย** (ฝั่งจอทักมา 5 ก.ย. 2569)
        จอมีตัวนับ "หมวดที่เดาจากชื่อสินค้า" อ่านจาก `r.zort === false` มาตลอด
        แต่ท่อไม่เคยส่งช่องนี้มาสักแถว ⇒ ตัวนับเป็น 0 เสมอ **ป้ายไม่มีวันขึ้น**
        เป็นตาข่ายที่ไม่มีวันทำงาน — คลาสเดียวกับ nets-expire-silently
        ต้นเหตุ: `zort` รายหมวดมีจริง แต่อยู่คนละ endpoint (`list=poscats` → `cats[].zort`)
        ที่นั่นมีการเดาจากชื่อสินค้าจริง ๆ พร้อมยอดรวม `guessed` / `fromZort`
        ⇒ ที่นี่จึงส่ง `zort` มาให้ **ตามความจริงของจอนี้**: ทุกหมวดมาจากทะเบียนสินค้า ZORT
           มีแถวเดียวที่เป็น false คือถังรวมของที่ยังไม่ได้จัดหมวดใน ZORT
        ⚠️ ห้ามแต่งค่านี้ให้ "มีทั้งจริงทั้งเดา" เพื่อให้ตัวนับฝั่งจอขึ้นเลข — จอนี้ไม่ได้เดา
           อยากได้ตัวเลขหมวดที่เดาจากชื่อ ต้องยิง list=poscats ไม่ใช่บิดความหมายที่นี่ */
    zortNote:
      "zort รายแถวที่นี่ = หมวดนี้มาจากทะเบียนสินค้าใน ZORT จริงหรือไม่ · " +
      "จอนี้ไม่มีการเดาหมวดจากชื่อสินค้า จึงมีได้แถวเดียวที่เป็น false " +
      "(ถังของที่ยังไม่ได้จัดหมวด) · ตัวเลขหมวดที่เดาจากชื่อสินค้าอยู่ที่ list=poscats",
    // จอฝั่งหลังร้านอ่านชื่อหมวดจากช่อง `name` — คงชื่อเดิมไว้ให้ใช้ง่าย
    rows: rows.map((r) => ({
      ...r,
      name: r.cat_name,
      zort: !String(r.cat_name).startsWith("("),
      ...(cv.map.get(String(r.cat_name)) || {}),
    })),
  };
}
