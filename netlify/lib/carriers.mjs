...
/* ชื่อจริงของแต่ละเจ้า → รายชื่อสะกดที่ยอมรับ (ตรงตัวหลัง normalize เท่านั้น)
   ...
   📌 **ที่มาของรายชื่อ (CEO สั่งให้เขียนกำกับ 18 ก.ย. 2569)**
      ทุกสะกดในตารางนี้ **ลอกมาจากค่าจริงในช่อง `ship_channel` ของกระจก** ไม่ใช่ชื่อที่นึกเอง
      รอบวัดล่าสุด: 18 ก.ย. 2569 · ขอบเขต `list=logistics` 35,749 ใบ
      (รอบก่อนหน้า: 4 ก.ย. 2569 ตอนพบว่า Flash เจ้าเดียวแตกเป็น 4 สะกด 559 ใบ)

   🚨 **สัญญาณว่าต้องมาไล่รอบใหม่: `carrierUngrouped` โตเกิน ~50 ใบ**
      หลังรอบ 18 ก.ย. เหลือ ungrouped **27 ใบ** ซึ่งเป็นชื่อที่จงใจไม่จัดกลุ่ม (ไม่ใช่ชื่อขนส่ง/กำกวม)
      ⇒ ถ้าวันไหนตัวเลขนี้ขึ้นไปเกิน 50 แปลว่า **มีสะกดใหม่ไหลเข้ามา** ต้องเปิดดู `carrierUngroupedNames`
        แล้วมาเติมที่นี่ · ห้ามปล่อยให้โตเงียบ ๆ เพราะตัวกรองขนส่งจะตกใบไปเรื่อย ๆ โดยไม่มีอะไรฟ้อง
        (ของจริงที่เพิ่งโดน: "ไปรษณีย์ไทย EMS" 780 ใบ หายจากตัวกรองกลุ่มไปรษณีย์ไทยอยู่นาน) */
const CARRIERS = [
