...
export async function listQuotations(limit = 50, page = 1, store = "z1", opts = {}) {
  ...
  return {
    store,
    /* 🔴 **ต้องบอก limit ที่ใช้จริงออกไป** (เพิ่ม 19 ก.ย. 2569)
       ไม่บอก ⇒ ตัวช่วยไล่หน้า (`paging-hint.mjs`) ไม่มีข้อมูลพอ **จึงไม่เติม nextOffset/nextPage เลย**
       ⇒ ปลายทางเห็นคำตอบที่ไม่มีคีย์ไล่หน้า ซึ่ง **แยกไม่ออกจาก "ท่อรุ่นเก่าไม่รองรับ"** ⇒ ต้องเดา
       ของจริง 19 ก.ย. 2569: ฝั่งจอเสียเวลาสองรอบเพราะเดาว่าเส้นหนึ่งไล่ด้วย `page`
       (ของจริงใช้ `offset`) แล้ว **เกือบรายงานว่าท่อไล่หน้าไม่ได้**
       🔑 สัญญาต้องประกาศตัว — ห้ามให้ปลายทางเดาจากความว่างเปล่า */
    limit: n,
    page: p,
...
export async function listReturnOrders(limit = 50, page = 1, q = "", store = "z1", opts = {}) {
  ...
  return {
    ...
    /* 🔴 **ต้องบอก limit ที่ใช้จริง** — ไม่บอก ⇒ ตัวช่วยไล่หน้าไม่เติม nextOffset/nextPage
       ...
       (ของจริง 19 ก.ย.: ฝั่งจอเดาว่าไล่ด้วย `page` แล้วเกือบรายงานว่าท่อพัง) */
    limit: n,
