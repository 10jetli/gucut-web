...
export async function listTransfers(o = {}) {
  ...
  return {
    ...
    /* ⚠️ จอต้องบอกว่ากระจก = สิ่งที่ API ส่งออกมา ไม่ใช่ทั้งหมดที่จอ ZORT นับ
       ...
       ⇒ ส่วนต่างกับจอ ZORT (/Warehouse/Transferlist 12,197 วัด 7 ก.ย. · จอ Adjust 5,317 วัด 3 ก.ย.)
          = **ใบที่จอนับแต่ API ไม่ส่งออกมาเลย** ⇒ ห้ามไล่แก้ที่ตัวซิงก์ · ตัวตนรายใบต้อง export จากจอ ZORT มาเทียบ id
       ⚠️ ตัวเลขฝั่งจอ ZORT วัดคนละวัน — ส่วนต่างที่แน่นอนต้องอ่านจอ ZORT ใหม่ในวันเดียวกับที่ยิง API */
    note: "กระจก = ทุกใบที่ ZORT API ส่งออกมา (ตรงทุกชนิด 14 ก.ย. 2569) · จอ ZORT นับมากกว่า ~195 ใบที่ API ไม่ส่ง — ดูยอดแยกชนิดที่ byKind",
    rows,
  };
}
