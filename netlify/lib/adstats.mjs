...
/** 🔎 **ด่านข้ามช่อง: มียอดแสดงผล แต่ไม่มีช่องค่าใช้จ่ายเลย = ช่องหาย ไม่ใช่ไม่ได้ใช้เงิน**
 *
 *  🔴 ที่มา 19 ก.ย. 2569 (ใบ t_mu7uzpld) — ไฟล์นี้เป็นเคสที่ **นับ "อ่านไม่ได้" ใช้ไม่ได้**
 *     เพราะแคมเปญที่ไม่ได้ยิงจริง ๆ ก็ **ไม่มีช่อง `spend`** และ 0 คือคำตอบที่ถูก
 *     ⇒ ถ้าตั้งด่านว่า "ช่องหาย = เสีย" จะได้แดงลวงทุกวันจนคนปิดด่าน
 *  🔑 ตัวชี้ขาดที่แยกได้จริงคือ **ความสัมพันธ์ระหว่างสองช่อง**:
 *     ยอดแสดงผล > 0 แปลว่าโฆษณาถูกส่งออกไปแล้ว ⇒ **ต้องมีค่าใช้จ่าย**
 *     ⇒ มี impressions แต่ **ไม่มีช่อง spend** (ไม่ใช่ spend เป็น 0) = Facebook เปลี่ยนชื่อช่อง
 *     ⇒ ถ้าปล่อยไว้: ค่าโฆษณาขึ้น ฿0 ทั้งกระดาน ⇒ **ROAS เป็นอนันต์** ⇒ ท่านประธานเร่งงบจากเลขปลอม
 *  ⚠️ ด่านนี้ **รายงาน ไม่หยุด** — ตัวเลขนี้เอาไปดู ไม่ได้เอาไปสั่งงานอะไรอัตโนมัติ
 *     (ต่างจากสต็อกที่ค่าผิดออกไปแตะของจริงบนมาร์เก็ตเพลส ⇒ ที่นั่นต้องหยุด) */
/** แกนกลางของด่านนี้ — **กติกาอยู่ที่เดียว** ใช้ได้ทุกแพลตฟอร์มโฆษณา
 *  @param list      แถวดิบจากแพลตฟอร์ม
 *  @param อ่านเงิน   (row) => ค่าดิบของช่องค่าใช้จ่าย (undefined = ไม่มีช่องนั้น)
 *  @param อ่านแสดงผล (row) => ค่าดิบของยอดแสดงผล
 *  @param เจ้า      ชื่อแพลตฟอร์ม + ไฟล์ที่ต้องไปแก้ ⇒ ข้อความบอกทางต่อได้เอง */
export function ตรวจช่องเงินโฆษณาหาย(list, { อ่านเงิน, อ่านแสดงผล, เจ้า, ไฟล์ }) {
  const rows = Array.isArray(list) ? list : [];
  let เข้าเงื่อนไข = 0;
  for (const x of rows) {
    const ค่าเงิน = อ่านเงิน(x);
    const มีช่องเงิน = ค่าเงิน !== undefined && ค่าเงิน !== null && ค่าเงิน !== "";
    /* ⚠️ ต้องถอดจุลภาคก่อนอ่านตัวชี้ขาด — ถ้าอ่านยอดแสดงผลไม่ออก ด่านจะเงียบ
       = **ตัวชี้ขาดพังไปทางเดียวกับปัญหา** ซึ่งหน้าตาเหมือนไม่มีปัญหา */
    const แสดงผล = Number(String(อ่านแสดงผล(x) ?? "").replace(/,/g, ""));
    if (!มีช่องเงิน && Number.isFinite(แสดงผล) && แสดงผล > 0) เข้าเงื่อนไข++;
  }
  if (!rows.length || !เข้าเงื่อนไข) return null;
  return (
    `⚠️ ${เข้าเงื่อนไข} จาก ${rows.length} แคมเปญ **มียอดแสดงผลแต่ไม่มีช่องค่าใช้จ่าย** ` +
    `⇒ น่าจะเปลี่ยนชื่อช่องฝั่ง ${เจ้า} (ไม่ใช่ว่าไม่ได้ใช้เงิน) ` +
    `⇒ ตัวเลขค่าโฆษณาและ ROAS รอบนี้เชื่อไม่ได้ ต้องแก้ชื่อช่องใน ${ไฟล์} ก่อน`
  );
}

/** ด่านของ Facebook — เปลือกบางบนแกนกลาง (ชื่อช่องของ Facebook อยู่ที่นี่ที่เดียว) */
export function ตรวจช่องค่าโฆษณาหาย(raw) {
  return ตรวจช่องเงินโฆษณาหาย(raw, {
    อ่านเงิน: (x) => x?.spend,
    อ่านแสดงผล: (x) => x?.impressions,
    เจ้า: "Facebook",
    ไฟล์: "adstats.mjs",
  });
}
...
export async function facebookInsights({ accountId, token, since, until }) {
  ...
  /* ⚠️ แนบคำเตือนไปกับผล (ไม่เปลี่ยนสัญญาเดิมที่คืนอาร์เรย์ — ผู้เรียกเดิมยังใช้ได้เหมือนเดิม)
     ⚠️ JSON.stringify ไม่เก็บ property ของอาร์เรย์ ⇒ **ผู้เรียกต้องยกขึ้นคำตอบเอง**
        เขียนกำกับเพราะจุดนี้มองไม่เห็นจากปลายทาง (กติกาเดียวกับ shopee-stock.rows.coverage) */
  const เตือน = ตรวจช่องค่าโฆษณาหาย(body?.data);
