...
export function ตรวจช่องค่าโฆษณาหาย(raw) {
  ...
  let มีแสดงผลแต่ไม่มีช่องเงิน = 0;
  for (const x of list) {
    const มีช่องเงิน = x && x.spend !== undefined && x.spend !== null && x.spend !== "";
    const แสดงผล = Number(String(x?.impressions ?? "").replace(/,/g, ""));
    if (!มีช่องเงิน && Number.isFinite(แสดงผล) && แสดงผล > 0) มีแสดงผลแต่ไม่มีช่องเงิน++;
  }
  if (!list.length || !มีแสดงผลแต่ไม่มีช่องเงิน) return null;
  return (
    `⚠️ ${มีแสดงผลแต่ไม่มีช่องเงิน} จาก ${list.length} แคมเปญ **มียอดแสดงผลแต่ไม่มีช่องค่าใช้จ่าย** ` +
    "⇒ น่าจะเปลี่ยนชื่อช่องฝั่ง Facebook (ไม่ใช่ว่าไม่ได้ใช้เงิน) " +
    "⇒ ตัวเลขค่าโฆษณาและ ROAS รอบนี้เชื่อไม่ได้ ต้องแก้ชื่อช่องใน adstats.mjs ก่อน"
  );
}
