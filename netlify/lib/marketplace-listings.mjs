...
/* 🔑 export ไว้ให้ทดสอบเรียกตรงได้ (19 ก.ย. 2569)
   เดิมเป็นฟังก์ชันภายใน ⇒ ตัวทดสอบต้องเรียกผ่าน marketplaceListings ซึ่งยิงออกนอก
   แล้วจบลงที่ "ข้ามข้อนี้" = เขียวที่แปลว่ายังไม่ได้ทดสอบ ไม่ใช่ผ่าน */
export async function webSkus() {
  ...
  for (const p of base?.list ?? []) {
    ...
    if (code) out.add(code);
    /* 🔴 **ต้องนับรหัสของตัวเลือกด้วย** (แก้ 19 ก.ย. 2569)
       เว็บเราขายหลายแบบในสินค้าเดียว และแต่ละแบบมีรหัสของตัวเอง (ตัดสต็อกด้วยรหัสนั้นจริง)
       เดิมนับแต่รหัสระดับสินค้า ⇒ คอลัมน์ "ช่องทางที่ลงขาย" บอกว่าเว็บเราไม่ได้ขาย
       ทั้งที่ลูกค้ากดซื้อได้อยู่ · วัดจริง: หายไปจากการนับ 270 รหัส
       ⚠️ ฟีดรุ่นเก่ายังไม่มีช่องนี้ ⇒ `?? []` ไว้ ไม่ใช่ถือว่าผิดพลาด */
    for (const v of p?.vsku ?? []) {
