...
/** คีย์ยอดรวมรายวัน — **หนึ่งคีย์ = หนึ่ง (วันไทย × เจ้า)**
 *  ⚠️ ห้ามรวมเป็นก้อนเดียวทั้งวัน: รอบของสามเจ้ายิงห่างกันไม่กี่นาที (05:50 · 05:55 · …)
 *     ⇒ ก้อนเดียว = อ่าน-บวก-เขียนทับกันข้ามเจ้า (กติกาเดียวกับตัวนับคนเข้าเว็บ) */
export const คีย์ยอดรายวัน = (วัน, เจ้า) => `stockpush/daily/${วัน}/${เจ้า}`;

/** บวกผลรอบหนึ่งเข้ายอดรวมรายวัน — **แยกออกมาเพื่อให้เทสได้จริง** (ไม่ต้องมีถัง ไม่ต้องยิงแพลตฟอร์ม)
 *  🔑 ส่วนที่ "คิด" ต้องแยกจากส่วนที่ "ต่อโลกภายนอก" ไม่งั้นเทสได้แค่รูปแบบของซอร์ส
 *  ⚠️ ใช้ `??` ไม่ใช่ `||` — `0 || x` กลืนศูนย์จริง ⇒ รอบที่ยิงออก 0 จะหายไปจากตัวนับรอบ */
export function บวกยอดรายวัน(เดิม, out, วัน) {
  return {
    วัน, เจ้า: out.platform,
    รอบ: Number(เดิม?.รอบ ?? 0) + 1,
    ยิงออก: Number(เดิม?.ยิงออก ?? 0) + Number(out.fired ?? 0),
    สำเร็จ: Number(เดิม?.สำเร็จ ?? 0) + Number(out.pushed ?? 0),
    ถูกปฏิเสธ: Number(เดิม?.ถูกปฏิเสธ ?? 0) + Number(out.rejected ?? 0),
    ไม่ได้ส่ง: Number(เดิม?.ไม่ได้ส่ง ?? 0) + Number(out.notSent ?? 0),
    รอบแรกเมื่อ: เดิม?.รอบแรกเมื่อ ?? out.at,
    รอบล่าสุดเมื่อ: out.at,
    "🔑 อ่านยังไง": "ยอดรวม **รายวันไทย** ต่อเจ้า · `ยิงออก` = จำนวนรหัสที่ส่งออกไปจริง " +
      "(ไม่ใช่จำนวนที่วางแผน) · เก็บถาวร ไม่ถูกเขียนทับเหมือนสมุด 50 รอบ",
  };
}
...
export async function stockPushLive(body, { แผนที่คิดแล้ว = null } = {}) {
  ...
  /* 🔴🔴 **20 ก.ย. 2569 — สมุดเล่มบนเก็บแค่ 50 รอบ ⇒ ที่ความถี่วันนี้คือ ~5 ชั่วโมงครึ่ง**
     (3 เจ้า × ~12 รอบ/ชม.) ⇒ **ประวัติการยิงหายภายในกะเดียว**
     ⇒ คำถามที่ท่านประธานจะถามแน่ ๆ ถ้าสต็อกหน้าร้านเพี้ยน — *"วันนี้มีอะไรออกไปบ้าง"* —
       **ตอบย้อนหลังไม่ได้เลยหลังพ้นห้าชั่วโมง** (ฝั่งจอวัดเจอ 20 ก.ย. · หน้าต่างที่อ่านได้ 05:40→11:10Z)
     🔑 และวันนี้ **ของออกไปแตะสต็อกจริงแล้ว** (shopee 58 รหัส · tiktok 60 รหัส) ⇒ ไม่ใช่เรื่องสมมติ
     ✅ จึงเก็บ **ยอดรวมรายวันต่อเจ้า** แยกคีย์ไว้ถาวร — เล็ก ไม่มีวันเต็ม และตอบคำถามนั้นได้
     ⚠️ **หนึ่งคีย์ = หนึ่ง (วันไทย × เจ้า)** ไม่ใช่ก้อนเดียวทั้งวัน:
        รอบของสามเจ้ายิงห่างกันไม่กี่นาที (05:50 · 05:55 · …) ⇒ ก้อนเดียว = อ่าน-บวก-เขียนทับกัน
        ⇒ แยกคีย์ต่อเจ้าแล้วรอบของ **เจ้าเดียวกัน** ไม่ขนานกันเอง (cron ตัวเดียวต่อเจ้า)
        🚫 วันไหนมีคนยิงมือพร้อม cron ของเจ้าเดียวกัน ยอดอาจขาดหนึ่งรอบ — ยอมรับ เพราะ
           ทางเลือกคือเขียนทับข้ามเจ้า ซึ่งเสียมากกว่า · และสมุดเล่มบนยังเป็นหลักฐานรายรอบอยู่
     ⚠️ จดไม่ได้ **ต้องไม่ทำให้การยิงล้ม** — ของออกไปแล้ว ผลต้องคืนครบ (กติกาเดียวกับสมุดเล่มบน) */
  try {
    const { วันนี้ไทย } = await import("./thaiday.mjs");
    const วัน = วันนี้ไทย();
    const คีย์ = คีย์ยอดรายวัน(วัน, out.platform);
    const เดิม = (await s.get(คีย์, { type: "json" })) || null;
    await s.setJSON(คีย์, บวกยอดรายวัน(เดิม, out, วัน));
  } catch (e) {
    return { ...out, dailySkipped: `จดยอดรวมรายวันไม่ได้: ${String(e?.message || e).slice(0, 120)} (การยิงสำเร็จแล้ว ผลด้านบนยังครบ)` };
  }
