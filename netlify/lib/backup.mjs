...
export const PROTECTED = [
  ...
  /* ⚠️ **ตัวนี้สำคัญที่สุดในรายการ** — ของใน ZORT ที่ไม่มี API ให้ดึงออก
      วันที่ปิดบัญชี ZORT ต้นทางจะหายถาวร ⇒ สำเนานี้จะกลายเป็น "ที่เดียวในโลก" จริง ๆ
      (ตัวอื่นในรายการยังมีต้นทางอยู่ · ตัวนี้จะไม่มี) */
  { store: "gucut-zort-archive", what: "ของใน ZORT ที่ไม่มี API — เก็บก่อนปิดบัญชี" },
];
...
export async function runBackup(budgetMs = 18000) {
  ...
  //    ที่เหลือรอบหน้าเก็บต่อเอง (งานตั้งเวลาวิ่งทุก 6 ชั่วโมง — ลดเมื่อ 18 ก.ย. 2569) ไม่ต้องมีใครมาสั่ง
