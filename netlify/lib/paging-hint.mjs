...
export function withPagingHint(obj, applied, q = {}) {
  ...
  /* 🔴 **ต้องมีทั้งสองคีย์เสมอ ตัวที่ไม่ได้ใช้เป็น null** (แก้ 19 ก.ย. 2569)
     ของเดิมใส่เฉพาะตัวที่ใช้ ⇒ เส้นเดียวกันคืนคีย์ไม่เหมือนกันตามพารามิเตอร์ที่ส่งมา
     (ส่ง page มา = ได้ nextPage ไม่มี nextOffset · ไม่ส่ง = กลับกัน)
     ⇒ ฝั่งจอเจอ `undefined` ซึ่ง **แยกไม่ออกจาก "ท่อรุ่นเก่าไม่รองรับ"**
       ต่างจาก `null` ที่อ่านได้ว่า "รองรับ แต่ทางนี้ไม่ใช้ตัวนี้"
     🔑 คลาสเดียวกับที่สารบัญ "ช่องที่แต่ละเส้นคืน" จับได้ในวันเดียวกันที่ list=returnorders:
        เส้นที่คืนรูปคำตอบไม่สม่ำเสมอ ทำให้ปลายทางต้องเขียนหลายชุดโดยไม่มีใครบอก */
  out.nextPage = null;
  out.nextOffset = null;
