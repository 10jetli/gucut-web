...
const งบเวลา = 18_000;
/** 🔴 **เลขนี้เคยเขียนไว้ว่า "เพดานจริงของฟังก์ชันตามเวลา = 30,000" — ยิงจริงแล้วพบว่าไม่ใช่**
 *  (19 ก.ย. 2569 ค่ำ · ฝั่งจอวัดจาก `?sweeptiming=1` แล้วรายงานว่า lazada live ช้าสุด **30,021 ms**
 *   เกินเลขที่เราประกาศ · เขาสรุปว่า "รอบที่ตายที่เพดานจะไม่ถูกเขียนสมุด ⇒ สถิติเป็นขอบล่าง")
 *  🔑 **แต่หลักฐานชี้ตรงข้ามหนึ่งชั้น**: รอบ 30,021 ms **อยู่ในสมุด** ⇒ มันเขียนสมุดสำเร็จ
 *     ⇒ **รอบนั้นไม่ได้ถูกฆ่า** ⇒ เพดานจริงของฟังก์ชันนี้ **มากกว่า 30,021 ms**
 *     (สอดคล้องกับที่ Netlify รันงานตามเวลาแบบ background ซึ่งได้เวลามากกว่าคำขอปกติ)
 *  ⚠️ **เพดานจริงยังไม่ได้พิสูจน์** — เรารู้แค่ "≥ 30,021" ⇒ ห้ามเอาไปใช้ตัดสินอะไร
 *     ⇒ เลิกใช้คำว่า "เพดาน" เป็นเกณฑ์ และหันไปใช้ **งบเวลาที่เราตั้งเอง** ซึ่งมีความหมายจริง:
 *       รอบที่ใช้เวลาเกินงบ = รอบที่ **ยกงานไปรอบหน้า** (พฤติกรรมที่ควรเฝ้าจริง ๆ)
 *  🔑 บทเรียนของคลาสนี้: **เกณฑ์ต้องคิดจากเลขที่พิสูจน์ได้ ไม่ใช่เลขที่เราเชื่อ**
 *     เกณฑ์ที่วัดระยะถึงเพดานที่ไม่มีอยู่ ให้ตัวเลขที่ดูมีความหมายแต่ตอบคำถามของตัวเอง */
/* 📏 **อัปเดต 21 ก.ย. 2569: ฝั่งจอวัดจากสมุดตัวกวาดได้รอบยาว 56,374 ms**
   ⇒ ขอบล่างที่พิสูจน์แล้วขยับจาก 30,021 → **56,374** (ของจริงที่ทำงานจบและถูกจดสำเร็จ)
   🔑 ชื่อของค่านี้บอกตัวเองว่าเป็น **ขอบล่าง** ไม่ใช่เพดาน ⇒ ทุกครั้งที่เจอรอบยาวกว่านี้ ให้ขยับขึ้น
   📏 เอกสาร Netlify: ฟังก์ชัน sync ~10 วิ · **background/scheduled 15 นาที**
      ⇒ ตัวกวาดวิ่งเป็น scheduled ⇒ เพดานจริงอยู่ระดับนาที **ไม่ใช่ 30 วินาที** */
export const เพดานที่พิสูจน์แล้วว่าเกินได้_ms = 56_374;
/** ⚠️ ชื่อเดิม คงไว้เพื่อไม่ให้ฝั่งจอ/ตัวอ่านสมุดพังทันที (ทั้งสองฝั่ง deploy ไม่พร้อมกัน)
 *  🚫 **ห้ามใช้ตัดสินใหม่** — ค่าเท่ากับขอบล่างที่พิสูจน์แล้ว ไม่ใช่เพดานจริง
 *  📌 ถอดออกได้เมื่อฝั่งจอเลิกอ่านคีย์นี้แล้ว (ต้องบอกกันก่อน) */
export const เพดานเวลาฟังก์ชัน_ms = เพดานที่พิสูจน์แล้วว่าเกินได้_ms;
/** "รอบที่ยาวผิดปกติ" = ใช้เวลาเกิน **งบเวลาที่เราตั้งเอง** (18 วิ) ⇒ รอบนั้นยกงานไปรอบหน้า
 *  🔑 เกณฑ์นี้มีความหมายเพราะ **เราเป็นคนตั้งงบ** และผลของการเกินงบคือพฤติกรรมที่เห็นได้จริง
 *     ต่างจากเกณฑ์ 80% ของ "เพดาน" ที่เลขตั้งต้นพิสูจน์แล้วว่าไม่ตรง */
export const ยาวผิดปกติที่_ms = งบเวลา;
/** ⚠️ ชื่อเดิม + **ค่าเดิม (80% ของ 30,000)** — คงไว้ทั้งคู่โดยตั้งใจ
 *  🔑 เกือบพลาดเอง: ผมกำลังจะให้ชื่อเดิมชี้ไปที่งบเวลา ⇒ **เลขบนจอจะเปลี่ยนความหมายเงียบ ๆ**
 *     (จอเขียนว่า "เกือบชนเพดาน 8 รอบ" แล้ววันหนึ่งกลายเป็นนับคนละเกณฑ์ โดยไม่มีใครรู้)
 *     ⇒ กฎของทีมเอง: เปลี่ยนความหมายของคีย์ ต้องเขียนเส้นทางเต็มและให้ปลายทางสลับเอง
 *  📌 ⇒ ท่อส่ง **ทั้งสองคีย์** · จอสลับมาใช้ `ยาวเกินงบเวลา` เมื่อพร้อม แล้วค่อยถอดตัวนี้ */
export const เกือบชนเพดานที่_ms = 24_000;
...
export async function เวลาต่อรอบ({ ชั่วโมงย้อนหลัง = 24 } = {}) {
  ...
  try {
    ...
    /* 🔴 **คีย์ที่คอมเมนต์ข้างบนสัญญาไว้ว่า "ท่อส่งทั้งสองคีย์" — แต่ไม่เคยถูกส่งจริง** (แก้ 21 ก.ย. 2569)
       ฝั่งจอวัดได้ว่ามีรอบยาว **56,374 ms** ซึ่งเกิน `30,000` ที่เราประกาศเป็น "เพดาน"
       ⇒ ⇒ คอลัมน์ `รอบที่เกือบชนเพดาน` (80% ของ 30,000) **วัดเทียบกับเส้นที่ไม่มีอยู่จริง**
       ⇒ จอไม่มีตัวเลือกอื่นให้แสดง เพราะเราไม่เคยส่งตัวที่มีความหมายไปให้
       🔑 คลาส: **แผนที่เขียนไว้ในคอมเมนต์ แต่ไม่มีใครทำ — และคอมเมนต์อ่านเหมือนทำแล้ว**
          (ฝั่งจอเจอรูปเดียวกันวันนี้เรื่องตัวจับเวลาที่ยังไม่ขึ้นเว็บ) */
    const เกินงบ = new Map();
    for (const [k, a] of กลุ่ม) เกินงบ.set(k, a.filter((ms) => ms >= ยาวผิดปกติที่_ms).length);
    const แถว = (rows || []).map((r) => {
      ...
      return {
        ...
        /* 🟡 **ชื่อเดิม คงไว้เพื่อไม่ให้จอพัง — แต่ห้ามใช้ตัดสิน** (เทียบกับเพดานที่พิสูจน์แล้วว่าไม่จริง) */
        รอบที่เกือบชนเพดาน: เกือบชน.get(`${r.channel}|${r.mode}`) ?? null,
        /* ✅ **ตัวที่มีความหมาย: เกิน "งบเวลาที่เราตั้งเอง"** ⇒ รอบนั้นยกงานไปรอบหน้าจริง */
        รอบที่ยาวเกินงบเวลา: เกินงบ.get(`${r.channel}|${r.mode}`) ?? null,
    ...
    return {
      ...
      /* 🔑 **เลขสองตัวในตารางเทียบกับเส้นคนละเส้น — ต้องเขียนไว้ ไม่ใช่ให้คนอ่านเดา**
         (ฝั่งจอเอา `รอบที่เกือบชนเพดาน` ขึ้นจอโดยไม่มีคำกำกับ แล้วมันวัดกับเพดานที่ไม่มีอยู่จริง) */
      "🔎 สองคอลัมน์นั้นเทียบกับอะไร":
        `รอบที่ยาวเกินงบเวลา = เกิน ${ยาวผิดปกติที่_ms} ms ⇒ **งบที่เราตั้งเอง** · เกินแล้วรอบนั้นยกงานไปรอบหน้า (มีความหมายจริง) · ` +
        `รอบที่เกือบชนเพดาน = เกิน ${เกือบชนเพดานที่_ms} ms ⇒ 80% ของ 30,000 ที่เคยเข้าใจว่าเป็นเพดาน ` +
        `**ซึ่งพิสูจน์แล้วว่าไม่ใช่** (มีรอบยาว ${เพดานที่พิสูจน์แล้วว่าเกินได้_ms} ms ทำงานจบและถูกจดสำเร็จ) ` +
        `⇒ 🚫 **ห้ามใช้ตัวหลังตัดสิน** คงไว้เพื่อไม่ให้จอพังระหว่างสลับ · Netlify: sync ~10 วิ · scheduled/background 15 นาที`,
