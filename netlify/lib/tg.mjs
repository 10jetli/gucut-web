/** ส่งข้อความเข้ากลุ่ม Telegram ของร้าน
 *
 * ⚠️ **ยังไม่ใช่จุดเดียวของทั้งระบบ** (ตรวจ 18 ก.ย. 2569 · `grep -rn api.telegram.org netlify/`)
 *    ยังมีที่ยิงตรงอีก ~20 จุดใน `netlify/functions/*.mjs` ซึ่งเป็นปลายทางของคำขอจริง
 *    **ชุดทดสอบเข้าไม่ถึงจุดพวกนั้น** จึงไม่ใช่ความเสี่ยงสแปมแบบเดียวกัน — ยังไม่ย้าย
 *    ที่ย้ายมาแล้ว = ทุกจุดใน `netlify/lib/` ที่ชุดทดสอบ import ถึง
 *    🔑 เขียนไว้ตรง ๆ เพราะคำว่า "จุดเดียวของทั้งระบบ" ที่ไม่จริง จะทำให้คนรอบหน้า
 *       เชื่อว่าปิดรูครบแล้ว ทั้งที่ยังไม่ครบ
 *
