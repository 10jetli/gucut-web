...
/** 🔑 **ชื่อกองที่เอาไปใช้จริง — กติกามีที่เดียว ห้ามประกอบซ้ำที่อื่น**
 *
 *  ที่มา (12 ก.ย. 2569): กติกานี้เคยอยู่ในตัว `groupsFromCounts` เท่านั้น ⇒ ฝั่ง **รายการสรุป**
 *  ประกาศชื่อ `blank_none_expected` / `blank_source_empty` แต่ฝั่ง **แถว** ส่งชื่อ `blank` เฉย ๆ
 *  ⇒ ชื่อกองสองฝั่งไม่ตรงกัน · จอที่กรองแถวด้วยชื่อจากรายการสรุป **จะไม่เจอแถวกลุ่ม blank เลย**
 *     และจะเงียบ — ได้ 0 แถวที่หน้าตาเหมือน "ไม่มีใบในกองนี้"
 *  ⇒ ต้นตอคือ **กติกาเดียวถูกเขียนที่เดียว แล้วอีกฝั่งไม่ได้เรียกใช้** (รากเดียวกับบั๊กปุ่มหมวด POS
 *     เช้าวันเดียวกัน) ⇒ ดึงออกมาเป็นฟังก์ชันนี้ แล้ว **เรียกทั้งสองทาง**
 *  ⚠️ ใครจะเพิ่มกองใหม่ หรือเปลี่ยนวิธีตั้งชื่อ **แก้ที่นี่ที่เดียว** ห้ามเขียนเงื่อนไขซ้ำฝั่งแถว
 *     (ด่านประจำในสคริปต์ยิงตรวจชุด ⑦ จะแดงทันทีถ้าสองฝั่งหลุดจากกัน)
 */
export function groupKeyOf(group, blankReason) {
  ...
  return group === "blank" && blankReason ? `blank_${blankReason}` : group;
}
...
export function groupsFromCounts(rows = []) {
  ...
  for (const r of rows) {
    ...
    const key = groupKeyOf(s.group, r?.blankReason);   // กติกาเดียว — ดู groupKeyOf
    const g =
      acc.get(key) ||
