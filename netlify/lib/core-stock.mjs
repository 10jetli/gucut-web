...
export async function listStock(o = {}) {
  ...
  return {
    ...
    soldDays,
    freshness,
    /* 🔴 **`applied` — เพิ่ม 19 ก.ย. 2569 · เส้นนี้ไม่มีมาก่อน** (เจอด้วยสารบัญช่องที่แต่ละเส้นคืน)
        วัดแล้วว่าเส้นนี้กรองด้วย q จริง (ยิง q=zz ⇒ 2,674 → 0) แต่คำตอบ **ไม่มีคีย์ q เลย**
        ⇒ จอโชว์ "1 รายการ" โดยไม่มีคำว่า "ที่ตรงกับคำค้น" ⇒ คนอ่านคิดว่าคลังมีของชิ้นเดียว
    ...
    /* 🔴 **สัญญาของคีย์นี้ — เขียนกำกับ 12 ก.ย. 2569 เพราะเดิมไม่มีใครเขียนไว้เลย**
       `total` = จำนวนรหัสที่เข้าเงื่อนไข **q / category / kind** เท่านั้น
       ⇒ **ไม่ถูกกรองด้วย `channel`** เพราะการกรองช่องทางเกิดทีหลังในหน่วยความจำ
         (ป้ายช่องทางไม่ได้อยู่ใน D1 · ดูบล็อก "กรองตามช่องทาง + แบ่งหน้า" ข้างล่าง)
       ⇒ ส่ง channel มาด้วย `total` **ไม่เปลี่ยน** · ตัวที่เปลี่ยนตามช่องทางคือ `rowsMatched`
       🔴 **ใครย้ายการกรองช่องทางไปอยู่ใน SQL (ซึ่งเป็นการ optimize ที่ดูสมเหตุสมผลมาก)
          ต้องกลับมาแก้จอด้วย** — `total` จะเปลี่ยนความหมายทันทีโดยไม่มีอะไรฟ้อง
          แล้วหัวจอมาร์เก็ตเพลสที่เขียนว่า "จำนวน N รหัสในคลัง" จะกลายเป็นยอดของช่องทางเดียว
          (จอเก็บ meta จากคำตอบของแท็บล่าสุด) ⇒ โกหกแบบเดียวกับบั๊กเลขแท็บ 12 ก.ย. เป๊ะ
       ⚠️ สองคีย์นี้อยู่ติดกันและชื่อไม่ได้บอกความต่าง ⇒ ต้องอ่านคอมเมนต์นี้ก่อนใช้ */
    total: num(sum?.skus),
    ...
    /* 🆕 19 ก.ย. 2569 ค่ำ · ฝั่งจอขอว่า "ช่องที่ท่อคิดไม่ได้ต้องเป็น null ไม่ใช่ 0"
       🔑 แต่เคสที่เขายกมา (`valueCost` ต่ำกว่าจริงเพราะรหัสไม่มีราคาทุน) **ไม่ใช่ "คิดไม่ได้"**
          — มันคิดได้ แต่ **ต่ำกว่าความจริง** ⇒ ถ้าส่ง null จอจะขึ้น "—" ทั้งที่มีตัวเลขที่ใช้ได้
          ⇒ ตอบให้ตรงกว่า: ส่งเลขต่อ + **ธงที่คิดจากข้อมูลจริง** ว่ามันต่ำกว่าจริงอยู่
       🚫 ห้ามเขียนธงนี้ฝังไว้ — พอกรอกราคาทุนครบ `noCostSkus` เป็น 0 ธงต้องหายเอง
          (กฎ warnings-computed-not-written) */
    valueCostต่ำกว่าจริง: num(sum?.no_cost) > 0,
    ...mk,
    rows: pageRows.map((r) => ({
      sku: r.sku,
      name: r.name || "",
 ...
 *
 *  🔄 **แก้ข้อความเดิม 6 ก.ย. 2569** — ตรงนี้เคยเขียนว่า *"ZORT ไม่เปิด API ให้ดึงรายการ
 ...
 *     📏 **เหตุผลที่พิสูจน์แล้ว 18 ก.ย. 2569 — ไม่ใช่ "ยังไม่ได้ทำ" แต่เป็น "ดึงมาไม่ได้"**
 *        `GetProducts&warehousecode=KLD` และ `=ANJ` → resCode 100 "Access Denied."
 *        ขณะที่ `=NEW` (โกดัง) ผ่าน ⇒ ZORT อ่านพารามิเตอร์นี้จริง ไม่ได้เมิน
 *        ท่านประธานเปิดสิทธิ์คลังให้แล้ววันเดียวกัน → **ยิงยืนยันว่าไม่มีผล** (ใบโอนยัง 12,005 เท่าเดิม)
 *        ตรวจฝั่งตั้งค่า ZORT: บทบาท Admin ตั้งคลังเป็น "ทั้งหมด" · คีย์ API อยู่ระดับร้าน
 *        ⇒ ฝั่งหน้าเว็บเปิดให้มากกว่านี้ไม่ได้ · เหลือทางเดียวคือถาม ZORT ว่าเส้น API ครอบคลังสาขาไหม
 *     🚫 **ห้ามเขียนว่า "ไม่มีสิทธิ์"** — ใบของ KLD ที่มีจริงตอบ "Invalid ID." ไม่ใช่ "Access Denied."
 *        และ id ที่ไม่มีอยู่จริงก็ตอบเหมือนกันเป๊ะ ⇒ แยกสาเหตุไม่ออก เขียนได้แค่ "เข้าถึงไม่ได้"
 */
