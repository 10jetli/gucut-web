...
async function shopeePlan(full) {
  ...
  /* 🔴 **ส่งต่อคำประกาศ "รายชื่อครบหรือถูกตัด"** (เพิ่ม 19 ก.ย. 2569 · ทั้งสามเจ้า)
     ตัวเทียบคิดค่านี้ให้แล้ว แต่ไม่ส่งต่อ ⇒ **ปลายทางไม่เห็นเลย** แล้วกลับไปเดาจากความยาวเหมือนเดิม
     🔑 ค่าที่คิดแล้วไม่ถูกส่งต่อ เท่ากับไม่ได้คิด — และ **หาไม่เจอด้วยการอ่านไฟล์เดียว**
        (ผมยิงของจริงแล้วเห็น `ประกาศครบ=None` จึงรู้ว่ามันหล่นที่ชั้นนี้ ไม่ใช่ที่ตัวเทียบ)
     ⚠️ ใส่ทั้งสามเจ้าโดยตั้งใจ — เจ้าที่ยังไม่คิดค่านี้ (`lazada`) จะเป็น no-op
        แล้ววันที่มันคิด ค่าจะไหลต่อเองโดยไม่มีใครต้องมาแก้ชั้นนี้อีก */
  for (const k of ["missingSampleครบ", "missingSampleเพดาน"]) if (k in c) p[k] = c[k];
