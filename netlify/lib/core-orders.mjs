...
export async function listOrders(o = {}) {
  ...
  /* 🔴 ตัวกรอง **ร้านเดียวล้วน** — ไม่มีวัน ไม่มีคำค้น ไม่มีช่องทาง ไม่ตัดใบยกเลิก
  ...
  const [
    ...
    /* 🔴 **รายชื่อสถานะที่ "มีอยู่ในระบบ" — ไม่กรองอะไรเลยแม้แต่คำค้น** (เพิ่ม 18 ก.ย. 2569)
       ฝั่งจอเจอตอนใช้จอขายเป็นทางสำรองจริง (ZORT /Sell/list พัง 500 คืนนี้):
         ก่อนค้น 5 แท็บ · ค้น "SO-2026" ⇒ เหลือ **3 แท็บ** · แท็บ "ยกเลิก" กับ "รอส่ง" **หายไปทั้งปุ่ม**
       เหตุ: `byStatus` ข้างบนกรองด้วย `q`/ช่วงวันด้วย (wAll ตัดแค่ตัวกรองสถานะ)
         ⇒ สถานะที่ไม่เจอในผลค้นจะไม่มีแถว ⇒ จอไม่มีทางรู้ว่าควรมีปุ่มนั้น
       ⇒ อาการที่คนใช้เจอ: อ่านได้ว่า **"จอนี้ไม่มีตัวกรองนั้น"** ทั้งที่ความจริงคือ **"กรองแล้วไม่เจอสักใบ"**
         และปุ่มขยับที่ระหว่างกรอง ⇒ คนกดผิดปุ่ม
       🔑 **แท็บคือสารบัญของข้อมูลทั้งหมด ไม่ใช่ผลของตัวกรองที่เลือกอยู่** (กติกาเดิมของเราเอง)
       ⇒ ส่งรายชื่อนี้ให้จอ **เพื่อไม่ต้อง hardcode ชื่อสถานะ** — วันที่ ZORT เพิ่มสถานะใหม่
         แท็บจะโผล่เอง ไม่ต้องรอใครไปเติมชื่อ (รายชื่อที่คนเติมมือจะล้าสมัยเสมอ)
       ⚠️ กรองแค่ร้าน (ขอบเขตเดียวที่จอเลือกแล้วเปลี่ยนความหมายของ "ทั้งระบบ")
       ⚠️ อ่านไม่ได้ ⇒ .catch คืน [] แล้วปลายทางส่ง null ⇒ จอต้องอ่านว่า **ไม่รู้** ไม่ใช่ "ไม่มีสถานะ" */
    coreQuery(
  ...
  return {
    ...
    /* 🔴 **`applied` — เพิ่ม 19 ก.ย. 2569 · เส้นนี้ไม่มีมาก่อนเลยทั้งที่เป็นเส้นที่ใหญ่ที่สุด**
        เจอด้วยสารบัญ "ช่องที่แต่ละเส้นคืน": 11 เส้นส่ง `applied` · เส้นนี้กับ list=stock ไม่ส่ง
        ⇒ **วัดแล้วว่าเส้นนี้กรองด้วย q จริง** (ยิง q=zz ⇒ total 868 → 0)
          แต่ในคำตอบ **ไม่มีคีย์ q เลย** ⇒ จอไม่มีทางรู้ว่าตัวเลขที่ได้เป็นยอดของอะไร
        🔴 ผลกับคนใช้: จอเขียน "ทั้งหมด 0 ใบ" โดยไม่มีคำว่า "ที่ตรงกับคำค้น"
          ⇒ คนอ่านสรุปว่า **ไม่มีออเดอร์เลย** ทั้งที่มี 868 ใบ (เลขต้องมีขอบเขตกำกับ)
        ⚠️ ใช้ชื่อ `applied` ตามที่ 11 เส้นใช้อยู่ **ห้ามคิดชื่อใหม่** · เพิ่มอย่างเดียว ไม่แตะค่าเดิม
        ⚠️ `advancedFilters` ยังอยู่และยังเป็นแหล่งของตัวกรองขั้นสูง — `applied` ไม่ทับมัน
           แต่ชี้ไปหามันด้วย เพื่อให้จอไม่ต้องเดาว่าตัวกรองขั้นสูงอยู่ไหน */
    applied: {
      q, channel, status, store: source, from, to, limit, offset,
      includeCancelled,
      advancedFilters: "ดูคีย์ advancedFilters — ตัวกรองขั้นสูงอยู่ที่นั่น (null = ไม่ได้กรอง)",
    },
    /* 🔴 **ชื่อใน `applied` ไม่ใช่ชื่อพารามิเตอร์ที่ท่อรับ — สองจุด** (เพิ่ม 19 ก.ย. 2569)
        เจอด้วยด่านใหม่ `check-filters-work.mjs`: ส่ง `includeCancelled=1` แล้วยอดไม่ขยับ (868)
        แต่ส่ง `cancelled=1` ได้ 902 ⇒ **ตัวกรองทำงาน แค่ชื่อที่สะท้อนกลับไม่ใช่ชื่อที่รับ**
        ⇒ ใครอ่าน `applied` แล้วเดาชื่อพารามิเตอร์จากคีย์ จะส่งผิดและ **ท่อเมินเงียบ**
          = ปุ่มกรองหลอกที่เกิดจากชื่อ ไม่ใช่จากโค้ดกรอง [[ui-label-is-not-the-param-name]]
        ⇒ ประกาศชื่อจริงออกไปเลย · ช่องที่ไม่อยู่ในนี้ = ชื่อเดียวกับคีย์ */
    /* 🔴 **ถอน `store` ออกจากรายการนี้ — ผมเขียนผิดเอง** (แก้ 19 ก.ย. 2569)
        ผมกวาดด้วย grep แล้วเจอ `source: p.get("store")` ⇒ สรุปว่า "คีย์ store รับด้วยชื่อ source"
        **ของจริงกลับกัน**: `source` เป็นชื่อ**พารามิเตอร์ของฟังก์ชัน** `listOrders({source})`
        ส่วนคีย์ที่ส่งออกใน `applied` คือ `store` และชื่อที่รับก็คือ `store` ⇒ **ตรงกันอยู่แล้ว**
        🔑 บทเรียน: **grep เจอชื่อตัวแปรภายใน ไม่ใช่ชื่อคีย์ที่ส่งออก** — สองอย่างนี้คนละชั้น
          ⇒ จะบอกว่า "ชื่อไม่ตรง" ต้องยิงเทียบสองชื่อจริง ไม่ใช่เทียบจากซอร์ส
        ฝั่งจอจับได้ด้วยการยิงเทียบทีละคู่: `store=z1` ⇒ total 586 ✅ · `source=z1` ⇒ ไม่มีผล
        และเขาไม่แก้จอตามลูกศรของผม ⇒ ถ้าเชื่อผม จอที่ทำงานถูกอยู่จะพัง */
    appliedParamNames: { includeCancelled: "cancelled" },
    appliedNote:
      "ค่าที่เซิร์ฟเวอร์ใช้กรองจริง (null = ไม่ได้กรองด้วยช่องนั้น) · " +
      "⚠️ ชื่อคีย์ในนี้ไม่ใช่ชื่อพารามิเตอร์ทุกตัว — ดู appliedParamNames ก่อนส่งค่า · " +
      "ตัวเลข total/totalAmount เป็นยอด **ของค่าเหล่านี้** ไม่ใช่ยอดทั้งคลัง ⇒ จอต้องเขียนขอบเขตกำกับเลขทุกครั้งที่มีค่าไม่เป็น null",
    total: num(sum?.c),
    ...
    byChannel,
    byStatus,
    /* 🔖 **สารบัญสถานะของทั้งระบบ (กรองแค่ร้าน)** — จอใช้ทำแท็บให้ครบโดยไม่ต้อง hardcode ชื่อ
       🔴 ที่มา 18 ก.ย. 2569: ฝั่งจอใช้จอขายเป็นทางสำรองจริงตอน ZORT /Sell/list พัง 500
          ค้นคำหนึ่งแล้ว **แท็บ "ยกเลิก" กับ "รอส่ง" หายไปทั้งปุ่ม** (5 แท็บ → 3 แท็บ)
          เพราะ `byStatus` กรอง q/วันด้วย ⇒ สถานะที่ผลค้นไม่มี ก็ไม่มีแถว
          ⇒ คนใช้อ่านว่า "จอนี้ไม่มีตัวกรองนั้น" ทั้งที่ความจริงคือ "กรองแล้วไม่เจอสักใบ"
...
export async function listLogistics(o = {}) {
  ...
  return {
    ...
    cod: num(sum?.cod),
    /* 📏 **ตัวนับห้าตัวนี้ไม่ได้อยู่ในขอบเขตเดียวกัน — วัดจริง 19 ก.ย. 2569**
       ไม่กรอง                 total 35,773 · shown 35,773
       `only=unshipped`        total 35,773 · shown **1,182**  ⇒ total ไม่ขยับ
       `carrier=Flash Express` total **32,491** · shown 32,491 ⇒ total ขยับ
       🔑 เหตุ: `total/shipped/unshipped/cod` นับ **หลังตัวกรองที่ทำใน SQL** (วัน · ขนส่ง · คำค้น)
          แต่ **ก่อน** `only` ซึ่งกรองทีหลัง · `shown` นับหลังทุกชั้น
       🔴 ⇒ **ห้ามเขียนบนจอว่า total คือ "ทั้งหมด"** เมื่อมีตัวกรองขนส่ง/วัน/คำค้นอยู่
          และ **ห้ามเอา shown ไปเทียบกับ total ของคำขออื่น**
       ⚠️ ฝั่งจอเคยเกือบกล่าวหาว่า `only` เป็นปุ่มหลอกเพราะดู `total` แทน `shown` (19 ก.ย. 2569)
          ⇒ จึงประกาศติดมากับคำตอบ ไม่ใช่ปล่อยให้เดาจากชื่อคีย์ */
    ขอบเขตตัวนับ: {
      total: "หลังตัวกรองใน SQL (วัน · ขนส่ง · คำค้น) — **ก่อน** only",
      shipped: "เหมือน total",
      unshipped: "เหมือน total",
      cod: "เหมือน total",
      shown: "หลังตัวกรองครบทุกชั้น รวม only ⇒ **ตัวนี้คือเลขที่ตรงกับแถวที่เห็น**",
      "🔑 ใช้ตัวไหน": "อยากรู้ว่าตัวกรองมีผลไหม ⇒ ดู `shown` · `total` ไม่ขยับตาม `only` โดยตั้งใจ",
    },
    limit,
    offset,
    only: usedOnly,
