...
export default async function handler(req, context) {
  ...
  if (req.method === "PATCH") {
    ...
    //
    // 🔴 **เคยมีบรรทัด `if (status === "returned" ...)` หลงมาอยู่ในบล็อกนี้ — แก้ 12 ก.ย. 2569**
    //    `status` ถูกประกาศด้วย const **ข้างล่างบล็อกนี้** ⇒ อ่านก่อนประกาศ = ReferenceError
    //    ⇒ ทุกครั้งที่กดปุ่มส่งซ้ำ: **ส่งเข้า ZORT สำเร็จแล้ว** (บรรทัดถัดขึ้นไป) แต่พังก่อน setJSON
    //    ⇒ ผลที่ไม่ถูกบันทึก ⇒ หน้าจอยังขึ้นว่าไม่สำเร็จ ⇒ คนกดซ้ำ ⇒ **เอกสารซ้ำใน ZORT**
    //    ⚠️ อาการหลอกที่สุดคือ **ฝั่ง ZORT สำเร็จจริง** ความเสียหายจึงไปโผล่ที่ระบบอื่น
    //       ไม่ใช่ที่ปุ่มนี้ ⇒ คนไล่จะไปหาว่า ZORT รับไม่ได้ทำไม ทั้งที่ ZORT รับได้ปกติ
    //    ⚠️ .mjs ไม่มีตัวตรวจชนิดข้อมูล ⇒ ผ่าน build ผ่าน lint ผ่านทุกด่านที่เรามี
    //       ต้องเข้ากิ่งนี้ตอนทำงานจริงเท่านั้นถึงจะเจอ (Codex เจอด้วยการอ่านโค้ด 12 ก.ย. 2569)
    // 🚫 ห้ามอ้างถึง `status` ในบล็อกนี้อีก — บล็อกนี้ไม่ได้เปลี่ยนสถานะ มันส่งเข้า ZORT อย่างเดียว
    if (body.action === "zort") {
      const o = await store.get(`o/${id}`, { type: "json" }).catch(() => null);
      if (!o) return json({ error: "not found" }, 404);
    ...
    // จดเวลาที่เปลี่ยนเป็น "คืนของ" — รายงานต้องนับตามวันที่คืน ไม่ใช่วันที่สั่ง
    // 🔴 บรรทัดนี้เคยอยู่ผิดบล็อก (อยู่ในบล็อกส่งซ้ำเข้า ZORT ข้างบน) ⇒ **ไม่เคยทำงานเลยสักครั้ง**
    //    ⇒ `returnedAt` ไม่เคยถูกเขียน ⇒ `returns-feed.mjs:47` ถอยไปใช้วันที่สั่งซื้อเสมอ
    //    ⇒ รายงานของคืนนับผิดวันมาตลอดโดยไม่มีอะไรฟ้อง (ย้ายมาที่ถูกแล้ว 12 ก.ย. 2569)
    // ⚠️ นี่คือที่เดียวในระบบที่เขียน returnedAt — ย้ายอีกเมื่อไหร่ รายงานของคืนจะเงียบอีกรอบ
...
async function zortAddOrder(order) {
  ...
  try {
    ...
    /* 🔴 **`id: null` ไม่ได้แปลว่าไม่เข้า ZORT — ยิงตรวจของจริงแล้ว 18 ก.ย. 2569**
       เกือบสรุปผิดวันนี้: ออเดอร์ที่จ่ายแล้ว **ทุกใบ** เก็บ `zort:{ok:true, id:null}`
       จึงดูเหมือน ZORT ไม่เคยสร้างใบให้เลยสักใบ **และเกือบใส่ตัวส่งซ้ำอัตโนมัติลงไป**
       ⇒ ยิงค้นกระจก ZORT ทีละใบก่อน พบว่า **ครบทั้ง 5 ใบ สถานะ Success**
       ⇒ ZORT ใช้ **เลขออเดอร์ของเว็บเราเองเป็นเลขเอกสาร** (`number = GCMT…`)
          แล้วไม่คืนค่านั้นกลับมาในฟิลด์ `orderid`/`id` ที่โค้ดนี้อ่าน
       ⇒ ถ้าเชื่อ `id` แล้วสั่งส่งซ้ำ = **เอกสารซ้ำในคลัง พนักงานแพ็กของสองรอบ**

       🔑 ฟิลด์ว่างเป็นหลักฐานว่า "เราอ่านผิดที่" ได้เท่า ๆ กับ "ปลายทางไม่ได้ทำให้"
          แยกสองอย่างนี้ออกจากกันได้ด้วยการยิงถามปลายทางเท่านั้น ห้ามเดาจากฟิลด์
       ⇒ จดเลขที่ค้นต่อได้จริงไว้ · `docNumberIs` บอกตรง ๆ ว่าเลขนั้นมาจากไหน
          กันคนอ่านทีหลังเข้าใจว่าเป็นค่าที่ ZORT คืนมา */
    if (code === "200") {
      return {
        ok: true,
        id: j?.orderid ?? j?.id ?? order.id,
        docNumberIs: (j?.orderid ?? j?.id) ? "zort" : "our-order-id",
      };
    }
    return {
      ok: false,
