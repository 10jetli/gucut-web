// คลังเงา GUCUT Core — งานตามเวลา **รันเองชั่วโมงละครั้ง (นาทีที่ 13)**
// 🔴 **เดิมบรรทัดนี้เขียนว่า "รันเองทุกครึ่งชั่วโมง"** ซึ่งขัดกับ cron ของตัวเอง
//    cron เปลี่ยนจาก `13,43` เป็น `13` เมื่อ 18 ก.ย. 2569 (aacde9e · ลดเครดิต 69%)
//    แต่คำในบรรทัดนี้ไม่ได้แก้ตาม ⇒ ฝั่งจอเขียนตามคำนี้ไป 3 จุด
//    🔑 **cron ถูก คำผิด** — ยืนยันจาก git ว่าตัวเลขเปลี่ยนโดยตั้งใจเพื่อลดเครดิต
//    ⚠️ บรรทัดแรกของไฟล์เป็นที่มาของช่อง desc ใน crontable ⇒ เขียนผิดที่นี่ = สารบัญผิดทั้งระบบ
//    (ตัวที่สามของคลาสนี้ใน 19 ก.ย. ต่อจาก bundle-recipe-sync และ backup-run — ฝั่งจอจับได้ด้วยด่านใหม่)
//
  ...
  try {
    ...
    if (utcH === 18 && utcM < 30) {
      daily = {
        ...
        /* ใบซื้อ (15 ก.ย. 2569 · ใบ t_mu2pfve9) — เดิม syncPurchases **ไม่มีงานตามเวลาเรียกเลย** มีแต่สั่งมือ [[nothing-triggers-it]]
           ร้านละไม่เกิน ~100 ใบ ดึงหน้าเดียวจบ · ตัวอ่านตาราง _v2 ตอบ error ถ้ายังไม่เคยซิงก์ ⇒ ต้องมีตัวจุดชนวน */
        purchasesZ1: await (await import("../lib/core-purchases.mjs")).syncPurchases({ store: "z1" }).catch((e) => ({ error: String(e?.message || e) })),
        purchasesZ2: await (await import("../lib/core-purchases.mjs")).syncPurchases({ store: "z2" }).catch((e) => ({ error: String(e?.message || e) })),
      };
...
export default ครอบงานตามเวลา("core-sync", handler);
...
/* 🔴 ลดจาก 48 → 24 ครั้ง/วัน (18 ก.ย. 2569) — เครดิต Netlify หมด เว็บล่มทั้งเว็บ
   วัดจริงแยกทีละงานที่มันทำในรอบเดียว: ZORT 4.1 + เทียบยอด 3.0 + ถ่ายสต็อก 2.1
   + Shopee 3.0 + เทียบสต็อก 5.3 = 17.5 วิ (+ TikTok/สินค้า/โอนสินค้าที่ยังไม่วัด)
   ⇒ ประมาณ 20–26 วิ/รอบ × 48 รอบ = ~18 นาที/วัน = **ตัวใหญ่สุดในระบบ**

   ⚠️ **ไม่ลดเป็นวันละครั้ง** — ตัวนี้คือกระจกออเดอร์
      ออเดอร์เก่าทั้งวัน = เทียบยอดผิด · แพ็กของผิด ⇒ ทุกชั่วโมงคือจุดที่ยังทันใช้งาน
   🚫 ห้ามลดต่ำกว่านี้โดยไม่ได้สั่ง

   🔑 **ทางที่ดีกว่าการลดความถี่ (ยังไม่ทำ)**: ใส่ทางออกเร็วแบบ stock-push-sweep
      ไม่มีออเดอร์ใหม่ = จบใน 1 วินาที ⇒ เก็บ 48 ครั้งไว้ได้แต่จ่ายแค่เศษเดียว */
