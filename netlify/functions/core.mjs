...
async function route(req, context) {
  ...
  /* ⏳ **คลาสนี้ยังเหลืออีกครึ่ง — ตั้งใจยังไม่แก้วันนี้ (ตกลงกับฝั่งจอ 6 ก.ย. 2569)**
     ...
     จุดที่เป็นแบบนี้ (นับ 6 ก.ย. 2569 — **นับใหม่ทุกครั้งก่อนใช้เลขนี้**):
       🔴 **เดิมมีรายชื่อ 15 ชื่อพิมพ์ไว้ตรงนี้ (นับ 6 ก.ย. 2569) — วัดใหม่ 20 ก.ย. ได้ 18 จุด**
          ⇒ **กองโตขึ้น 3 โดยไม่มีใครรู้** เพราะรายชื่อที่พิมพ์มือไม่มีอะไรทำให้มันตามความจริง
          🔑 กติกาของรีโปนี้ (CLAUDE.md) เขียนไว้แล้วว่า **ห้ามเขียนเลขที่โตเองเป็นตัวเลข
             ไม่ว่าจะมีวันที่กำกับหรือไม่** — รายชื่อที่โตเองก็เข้าข่ายเดียวกัน
          ⇒ จึงเอารายชื่อออก แล้วใส่ **คำสั่งนับ** ที่รันได้แทน:
          node -e "const s=require('node:fs').readFileSync('netlify/functions/core.mjs','utf8');
                   console.log([...s.matchAll(/\\b(?:okJson|json)\\(\\s*\\{\\s*ok:\\s*true,\\s*(\\w+):\\s*await/g)].map(m=>m[1]))"
          ⚠️ คำสั่งนี้นับ **รูปแบบ** ⇒ เป็น "จุดที่ต้องดู" ไม่ใช่ "จุดที่เป็นปัญหา" (ดูย่อหน้าถัดไป)
  ...
  const json = (obj, status = 200) => {
       ...
       ⚠️ ไม่ทับของเดิม — เส้นที่ประกาศ `supportedFilters` เองอยู่แล้วยังใช้ของตัวเอง */
    {
      ...
      /* 🗝️ **`skip` เป็นช่องทางการของทุกเส้น `list=` แล้ว** (19 ก.ย. 2569 ค่ำ)
         🔴 ฝั่งจอขอตรง ๆ: *"`skip` ต้องอยู่ในสัญญาอย่างเป็นทางการ ไม่ใช่ช่องที่บางทีมี"*
            เหตุผลเดียวกับ `warnkeys`: **คีย์ที่มีบางที ทำให้จอแยกสองอย่างนี้ไม่ออก** —
            ① ท่อทำงานได้ปกติ (ไม่มีเหตุ skip)  ② ท่อรุ่นเก่าที่ยังไม่รู้จักการส่ง skip
         ⇒ ส่ง `skip: null` เสมอเมื่อทำงานได้ · สตริงเมื่อทำต่อไม่ได้
         ✅ ปลอดภัยกับโค้ดที่มีอยู่เพราะทุกที่เช็ค **ค่า** (`if (d.skip)`) ไม่ใช่การมีคีย์
            — กวาดทั้งสองรีโปก่อนลงมือ เจอที่เช็คด้วย `"skip" in` **หนึ่งจุด**
            (`scripts/gen-returned-fields.mjs`) แล้วแก้ไปพร้อมกัน
            🔑 ถ้าไม่กวาด ตัวสร้างนั้นจะอ่านทุกคำตอบที่สำเร็จว่าเป็น error **แบบเงียบ ๆ**
         🚫 ห้ามใส่ให้คำตอบที่ไม่ใช่เส้น `list=` (ไม่ยัดคีย์ให้เส้นที่ไม่เกี่ยว) */
      if (ชื่อlist && out && typeof out === "object" && !Array.isArray(out) && !("skip" in out)) {
        out = { ...out, skip: null };
      }
    }
  ...
  try {
    ...
    /* ⏱️ GET ?jobtiming=1[&hours=N] ⇒ เวลาต่อรอบของ **งานตามเวลาที่ไม่ใช่ตัวกวาด**
       (slips-sync · contacts-sync · returns-sync · backup-run — เพิ่มได้เรื่อย ๆ ด้วย `วัดเวลางาน`)
       🔑 ใบ S1 สั่งให้ยิงสี่ตัวนั้นจับเวลา — ฝั่งจอค้านว่านั่นคือ **สั่งงานจริงและกินเวลาที่กำลังจะวัด**
          ⇒ เปลี่ยนเป็นให้งานจดเวลาตัวเองลงสมุด `job_run_log` แล้วเส้นนี้อ่านอย่างเดียว
       ⚠️ สมุดเริ่มจด 19 ก.ย. 2569 ⇒ ก่อนนั้นไม่มีแถว = **ยังไม่มีข้อมูล ไม่ใช่ศูนย์**
       ⚠️ ต้องอ่าน `ครอบคลุม` คู่ทุกครั้ง — รอบที่ถูกตัดตายที่เพดานเวลาไม่มีใครได้จด */
    /* 📝 **POST ?joblog=1 — ช่องรับสมุดเวลาจากงานตามเวลา "ฝั่งจอ"** (เปิด 19 ก.ย. 2569)
       🔴 ฝั่งจอมีงานตามเวลา 4 ตัว (bills-netlify · bills-watch · bills-daily · shelf-report)
          ที่ **ไม่มีตัวไหนจดเวลาเลย** และเขาวัดจากเครื่องตัวเองไม่ได้ (ไม่มี token)
          ⇒ นั่นคือ **"ไม่รู้" ไม่ใช่ "ศูนย์"** ⇒ ถ้าไม่เปิดช่องนี้ **ต้นทุนฝั่งจอจะไม่มีใครเห็นตลอดไป**
       ⇒ งานพวกนั้นเรียก `/api/core` อยู่แล้ว ⇒ ให้จดลงสมุดเล่มเดียวกัน (`job_run_log`)
       ⚠️ ต้องผ่าน `adminGate` เหมือนทุกเส้น (ผู้เรียกมีกุญแจอยู่แล้ว)
       ⚠️ **ผลที่ยอมรับมีสามค่าเท่านั้น** — ค่าอื่นถูกลดเป็น `ไม่ได้ตัดสิน` โดย `แถวที่จะจด()`
          ⇒ ฝั่งจอ **ส่ง `ok` มาเองไม่ได้ถ้าไม่รู้ผล** (กันสมุดเต็มไปด้วย ok ปลอม)
       ⚠️ **บังคับคำนำหน้าชื่องาน `จอ/`** เพื่อไม่ให้ชนกับงานฝั่งท่อ และให้ตัวอ่านแยกที่มาได้
          ⇒ งานฝั่งจอไม่มี cron ในสารบัญของท่อ ⇒ `รอบต่อวันตามตาราง: null` = **ยังไม่รู้** (ถูกต้องแล้ว)
       ⚠️ `ms` ต้องเป็นเลข ≥ 0 · `note` ตัดที่ 300 ตัว (กันสมุดบวม) */
    if (req.method === "POST" && url.searchParams.get("joblog")) {
      let body = null;
      try {
        body = await req.json();
      } catch {
        return json({ error: "อ่าน JSON จาก body ไม่ได้" }, 400);
      }
      const ชื่อ = String(body?.งาน ?? body?.job ?? "").trim().slice(0, 60);
      if (!/^จอ\//.test(ชื่อ)) {
        return json({ error: 'ชื่องานต้องขึ้นต้นด้วย "จอ/" (กันชนกับงานฝั่งท่อ และให้ตัวอ่านแยกที่มาได้)' }, 400);
      }
      const ms = Number(body?.ms);
      if (!Number.isFinite(ms) || ms < 0) return json({ error: "ms ต้องเป็นตัวเลข ≥ 0" }, 400);
      const { จดเวลางาน, ผลที่ยอมรับ } = await import("../lib/job-timing.mjs");
      const ผลที่ส่งมา = body?.ผล ?? body?.outcome;
      const จด = await จดเวลางาน({
        งาน: ชื่อ,
        ms,
        ผล: ผลที่ส่งมา,
        note: body?.note ?? body?.หมายเหตุ ?? null,
      });
      /* 🔑 บอกกลับไปว่า **ผลถูกลดชั้นไหม** — ผู้เรียกต้องรู้ว่าสมุดจดอะไรลงไปจริง
         (ส่งค่าที่ไม่รู้จักมา ⇒ จดเป็น `ไม่ได้ตัดสิน` ไม่ใช่ปฏิเสธ — เพราะการจดเวลายังมีค่า) */
      const ผลที่จด = ผลที่ยอมรับ.includes(ผลที่ส่งมา) ? ผลที่ส่งมา : "ไม่ได้ตัดสิน";
      if (จด !== "ok") return json({ error: `จดสมุดไม่สำเร็จ: ${จด}` }, 502);
      return okJson({
        ok: true,
        จดแล้ว: { งาน: ชื่อ, ms: Math.round(ms), ผล: ผลที่จด },
        ...(ผลที่จด !== ผลที่ส่งมา ? { หมายเหตุ: `ผลที่ส่งมา (${ผลที่ส่งมา ?? "ไม่ได้ส่ง"}) ไม่อยู่ในสามค่าที่ยอมรับ ⇒ จดเป็น "ไม่ได้ตัดสิน"` } : {}),
        "🔑 อ่านผลที่ไหน": "GET /api/core?jobtiming=1 — งานฝั่งจอจะมี `รอบต่อวันตามตาราง: null` เพราะสารบัญ cron ของท่อไม่รู้จัก (= ยังไม่รู้ ไม่ใช่ 0)",
      });
    }
    if (url.searchParams.get("jobtiming")) {
      if (req.method !== "GET") return json({ error: "เส้นนี้ GET เท่านั้น (อ่านอย่างเดียว)" }, 405);
    ...
    if (url.searchParams.get("endpoints")) {
      ...
      /* อ่านสารบัญ "ช่องที่แต่ละเส้นคืน" แยกจาก endpoints.mjs เพราะคนละวงจรชีวิต
         (endpoints สร้างตอน build · ตัวนี้รันมือหลัง deploy) ⇒ อ่านไม่ได้ต้องไม่ลากเส้นนี้ล้ม */
      let rf = null, rfErr = null;
      try {
        const j = await import("../lib/returned-fields.mjs");
        rf = j?.catalogue ?? null;
      } catch (e) {
        rfErr = String(e?.message || e).slice(0, 160);
      }
      return okJson({
        ...
        /* 🔖 **ช่องที่ตัวเขียนของ ZORT รับเข้า** — ฝั่งจอขอ 18 ก.ย. 2569
           ...
           และ **หาไม่เจอด้วยการอ่านโค้ดฝั่งตัวเอง** ต้องเอาสองฝั่งมาวางเทียบ
           ⚠️ **หยาบ** (regex เก็บ o.xxx จากซอร์ส) ⇒ ใช้เป็นรายการที่ต้องดูด้วยตา
              **ห้ามถือเป็นสัญญาของ API** และห้ามเอาจำนวนไปทำตัวเลขบนจอ */
        writeFields: t?.writeFields ?? null,
        /* 🔖 **ช่องที่แต่ละเส้น "คืน"** — ครึ่งที่หายของการเทียบสองทาง (ใบ t_mu78mrcw · 19 ก.ย. 2569)
           `lists`/`paramRoutes` บอกว่าท่อ **มี** เส้นอะไร · เครื่องมือฝั่งจอบอกว่าจอ **ใช้** ช่องอะไร
           ตัวนี้บอกว่าท่อ **คืน** ช่องอะไร ⇒ ด่าน check-pipe-keys ฝั่งจอถอยมาใช้ตัวนี้ได้
           ตอนที่ไม่มีซอร์สท่อในเครื่อง (เช่นตอน Netlify build) แทนที่จะข้ามทั้งด่าน
           🔴 **ได้จากการยิงของจริง ไม่ใช่อ่านซอร์ส** เพราะคีย์จำนวนมากมาจาก spread
           ⚠️ **แคบกว่าความจริง** (เฉพาะพารามิเตอร์ที่สคริปต์ยิง) ⇒ ใช้ชี้เป้าได้
              **ห้ามใช้ตัดสินว่าท่อไม่มีช่องนั้น** — ทิศความผิดกลับกับการอ่านซอร์สทั้งกอง
              (อ่านซอร์ส = กว้างเกิน เขียวลวง · ยิงจริง = แคบเกิน แดงลวง)
           ⚠️ **ไม่ได้สร้างตอน build** — ต้องรันมือหลัง deploy (`node scripts/gen-returned-fields.mjs`)
              เพราะตอน build ยังไม่มีท่อให้ยิง ⇒ **จะล้าสมัยเงียบ ๆ ถ้าไม่มีใครรัน**
              ด้วยเหตุนี้จึงส่ง `ดึงเมื่อ` + `ท่อสร้างเมื่อ` ออกไปด้วย ให้จอเตือนเองได้ */
        /* 🔑 **ของที่รอท่านประธานอนุมัติ — ทะเบียนที่เดียว** (เพิ่ม 19 ก.ย. 2569)
           ที่มา: ฝั่งจอพบข้อความที่อ้างสถานะการอนุมัติแล้ว **เป็นเท็จมา 5 วัน**
           เพราะท่านสั่งเปิดแล้วแต่คอมเมนต์ไม่ได้ผูกกับสวิตช์จริง
           ⇒ ค่าที่เป็น "สถานะการตัดสินใจของคน" ห้ามกระจายเป็นคอมเมนต์ มันจะค้างแน่นอน
        ...
        /* 🔖 **ชื่อคีย์ที่ท่อประกอบขึ้นตอนรัน** — ด่านคีย์ฝั่งจอ grep ไม่เจอชื่อเต็มในซอร์ส
           ⇒ เดิมด่านเขาฟ้องจอที่อ่านคีย์ที่ท่อส่งมาจริง (ธงแดงลวง)
           ⇒ ท่อประกาศออกมาเลย ดีกว่าให้ปลายทางยกเว้นทีละชื่อ (= ฝากความจำไว้กับคน)
           🔑 มีเทสต์บังคับว่ารายชื่อนี้ต้องตรงกับชื่อที่ฟังก์ชันคืนจริง ทั้งขาดและเกิน */
        composedKeys: (await import("../lib/core-freshness.mjs")).คีย์ที่ประกอบขึ้น,
        returnedFields: rf,
        ...
        "⚠️ ขอบเขตของ writeFields":
          "อ่านจากซอร์ส zort-write.mjs ด้วย regex (o.xxx) ⇒ **หยาบ ปนกับตัวช่วยที่ฉีดเข้ามาทดสอบได้** · " +
          "บอกว่า 'ตัวเขียนแตะชื่อนี้' ไม่ได้บอกว่าเป็นช่องที่ต้องส่งหรือส่งได้ · " +
          "ใช้หาช่องที่จอยังไม่ได้ใช้ **ห้ามถือเป็นสัญญาของ API** · null = อ่านไม่ได้ ไม่ใช่ไม่มีช่อง",
      });
    }
    ...
    /* 🗝️ รายชื่อคีย์ที่ท่อ **รุ่นนี้** ใส่แบบมีเงื่อนไข — สมมาตรของ `accepts` ในข้อความ 400
        🔴 ที่มา 19 ก.ย. 2569 ค่ำ · ฝั่งจอตั้งประโยค **"ช่องที่เว้นว่างคือคำกล่าวอ้างที่ไม่มีวันที่
           ไม่มีหลักฐาน และไม่มีใครเคยพิมพ์มันลงไป"** ⇒ "ไม่มีคีย์ `truncated`" อ่านได้สองอย่าง
           ที่จอ **แยกไม่ออก**: ① ตรวจแล้วปกติจริง ② ท่อรุ่นที่ปล่อยอยู่ยังไม่รู้จักคีย์นี้
        📏 เขาวัดมาให้ก่อนผมสร้าง: จออ่าน "ไม่มีคีย์ = ปกติ" **38 จุด** แยกสองสถานะได้ **1 จุด**
           ⇒ ไม่ใช่ของเผื่ออนาคต เป็นหนี้ที่มีอยู่แล้ว 37 จุด (เกณฑ์ `metrics-need-outside-leg`)
        🚫 **ไม่ใส่คีย์นี้ในทุกคำตอบโดยตั้งใจ** — เปลืองไบต์ทุกคำขอ · จอยิงครั้งเดียวตอนเปิดแล้วเก็บไว้เทียบ */
    /* 🚩 "รุ่นที่ขึ้นเว็บอยู่จริง · เมื่อไหร่ · สำเร็จไหม" — ตอบคำถามที่ฝั่งจอตอบไม่ได้เลยสักวิธี
        🔴 ที่มา 19 ก.ย. 2569 ค่ำ · กติกาฝั่งจอข้อ "จดจุดเซฟแจ้งผู้ใช้เสมอ" **ทำตามครบไม่ได้
           โดยโครงสร้าง** (ที่จดอยู่นอกรีโป) · เขาเสนอ `git reflog` ผมค้านว่า **push ≠ deploy สำเร็จ**
           ⇒ วันที่ build ตกคือวันที่บันทึกจาก push จะโกหกที่สุด = วันที่คนต้องพึ่งมันที่สุด
        🔑 ผู้ใช้ปลายทางยืนยันว่าจะอ่านก่อนผมสร้าง (กฎ metrics-need-outside-leg)
        🚫 **ห้ามให้จอเรียกวน** — จอกดเอง (กฎห้ามเช็คอัตโนมัติของร้าน) และตัวนี้ยิง Netlify API
           ซึ่งถ้าเรียกถี่จะกลายเป็นตัวกินทรัพยากรเสียเอง */
    if (url.searchParams.get("deploys")) {
      if (req.method !== "GET") return json({ error: "เส้นนี้ GET เท่านั้น (อ่านอย่างเดียว)" }, 405);
      const { สถานะการปล่อยของ } = await import("../lib/deploy-state.mjs");
      const r = await สถานะการปล่อยของ(CORE_BUILD);
      return okJson({
        ...r,
        "⚠️ ขอบเขต":
          "ถามไซต์นี้ไซต์เดียว 10 ใบล่าสุด · `ที่เสิร์ฟอยู่` เป็น null = **ยังไม่รู้ ไม่ใช่ไม่มี** " +
          "(ไม่เดาจากใบล่าสุด เพราะใบล่าสุดอาจเป็นใบที่ build ตก) · " +
          "`skip` = ถามไม่ได้เลย · `inconclusive` = ถามแล้วแปลผลไม่ได้ ⇒ **ไม่มีคีย์ ok** · " +
          "`ฟิลด์ที่ยังไม่ยืนยัน` = ฟิลด์ที่ยังไม่เคยเห็นค่าจริง (อ่านมาจากเอกสาร) ⇒ null อ่านว่า \"ยังไม่รู้\" · " +
          "`ท่อรุ่นนี้` = ค่าเดียวกับหัว `x-core-build` ของคำตอบนี้ ⇒ เทียบกับ `ที่เสิร์ฟอยู่` ได้ในคำขอเดียว",
      });
    }
    /* 🗝️ **คีย์ `applied` ที่แต่ละเส้นพิจารณา — เสิร์ฟจากผลยิงจริงที่เก็บไว้**
        🔴 ที่มา 19 ก.ย. 2569 ค่ำ: `netlify/lib/list-applied-keys.mjs` ถูกเขียนโดย
           `scripts/probe-applied-keys.mjs --write` (ผลจากการ **ยิงจริงทุกเส้น**) แต่
           **ไม่มีใครอ่านมันเลย** — ท่อไม่ import · จอไม่รู้ว่ามี ⇒ วัดได้แล้วทิ้งระหว่างทาง
           🔑 เจอเพราะไล่ว่า "ไฟล์ไหนไม่มีอะไรจุดชนวน" ⇒ ไฟล์นี้เป็นตัวเดียวในทั้งฝั่งเซิร์ฟเวอร์
        🔑 และฝั่งจอ **รออันนี้อยู่**: เขามีคำกล่าวอ้างเรื่องท่อบนจอ 186 จุด และจะทำด่านเทียบเอง
           เมื่อท่อประกาศว่าแต่ละเส้นพิจารณาคีย์อะไร ⇒ เส้นนี้ทำให้เขาเทียบได้ **โดยไม่ต้องยิง 24 เส้น**
        ⚠️ `พิจารณา: null` = **เส้นนั้นไม่ส่ง applied ⇒ ยังไม่รู้** ไม่ใช่ "ไม่รับตัวกรอง"
        ⚠️ ข้อมูลนี้มาจากการยิง **ณ เวลาที่รัน probe** ไม่ใช่สด ⇒ ต้องส่งเวลาที่วัดไปด้วย */
    if (url.searchParams.get("appliedkeys")) {
      if (req.method !== "GET") return json({ error: "เส้นนี้ GET เท่านั้น (อ่านอย่างเดียว)" }, 405);
      let t = null, readError = null;
      try {
        t = await import("../lib/list-applied-keys.mjs");
      } catch (e) {
        readError = String(e?.message || e).slice(0, 200);
      }
      const เส้น = t?.คีย์ที่เส้นพิจารณา ?? null;
      const ชื่อเส้น = เส้น ? Object.keys(เส้น) : null;
      const วันวัด = เส้น
        ? [...new Set(ชื่อเส้น.map((k) => เส้น[k]?.วัดเมื่อ).filter(Boolean))].sort()
        : [];
      return okJson({
        ok: !!เส้น,
        จำนวนเส้น: ชื่อเส้น ? ชื่อเส้น.length : null,
        ส่งappliedแล้ว: เส้น ? ชื่อเส้น.filter((k) => Array.isArray(เส้น[k]?.พิจารณา)).length : null,
        ยังไม่ส่งapplied: เส้น ? ชื่อเส้น.filter((k) => เส้น[k]?.พิจารณา === null).length : null,
        เส้น,
        /* 🔑 ไฟล์เก็บ `วัดเมื่อ` ไว้ **รายเส้น** (ไม่มีระดับไฟล์) ⇒ คิดช่วงจากของจริงที่มี
           ห้ามส่งค่าเดียวเป็น "เวลาที่วัด" เพราะแต่ละเส้นอาจถูกยิงคนละวัน
           ⇒ ส่งเก่าสุด/ใหม่สุด แล้วให้จอเห็นเองว่าข้อมูลคร่อมกี่วัน */
        วัดเมื่อเก่าสุด: วันวัด.length ? วันวัด[0] : null,
        วัดเมื่อใหม่สุด: วันวัด.length ? วันวัด[วันวัด.length - 1] : null,
        เส้นที่ไม่มีวันวัด: เส้น ? ชื่อเส้น.filter((k) => !เส้น[k]?.วัดเมื่อ).length : null,
        readError,
        "⚠️ ขอบเขต":
          "มาจากการ **ยิงจริงทุกเส้น** ด้วย `scripts/probe-applied-keys.mjs` ⇒ เป็นคำประกาศของเส้นเอง " +
          "ไม่ใช่การสกัดชื่อจากซอร์ส · **ไม่ใช่ข้อมูลสด** — `วัดเมื่อ` คือเวลาที่ยิง (null = ไฟล์รุ่นเก่ายังไม่มีช่องนี้) · " +
          "`พิจารณา: null` = **เส้นนั้นไม่ส่ง applied ⇒ ยังไม่รู้** 🚫 ห้ามอ่านว่า \"ไม่รับตัวกรอง\" · " +
          "และ \"พิจารณาอะไร\" ไม่ได้บอกว่า \"ค่าที่ส่งไปใช้ได้ไหม\" ⇒ อันนั้นดู `ignored` ต่อคำขอ",
      });
    }
    if (url.searchParams.get("warnkeys")) {
      if (req.method !== "GET") return json({ error: "เส้นนี้ GET เท่านั้น (อ่านอย่างเดียว)" }, 405);
      let t = null, readError = null;
      try {
        ({ ตารางคีย์มีเงื่อนไข: t } = await import("../lib/warn-keys.mjs"));
      } catch (e) {
        readError = String(e?.message || e).slice(0, 200);
      }
      const คีย์ = Array.isArray(t?.คีย์) ? t.คีย์ : null;
      return okJson({
        ok: !!คีย์,
        ...(t ?? {}),
        คีย์,
        readError,
        "⚠️ ขอบเขต":
          (t?.ขอบเขต ?? "อ่านตารางไม่ได้ ⇒ ไม่มีขอบเขตให้ประกาศ") +
          " · `คีย์` เป็น null = **อ่านตารางไม่ได้ ไม่ใช่ท่อไม่มีคีย์เลย** " +
          "(ถ้าอ่านเป็น \"ไม่มีคีย์เลย\" จอจะขึ้น \"ไม่รู้\" ทุกจุดพร้อมกันแล้วคนจะปิดตาข่ายทิ้ง) · " +
          "`สร้างเมื่อ` คือเวลาที่ build ไม่ใช่เวลาที่ยิงคำขอ",
      });
    }
    ...
    if (url.searchParams.get("stockpushlog")) {
      const { getStore } = await import("@netlify/blobs");
      const ถัง = getStore({ name: "gucut-coupon", consistency: "strong" });
      const log = await ถัง.get("stockpush/log", { type: "json" }).catch(() => null);
      /* 🔴 **20 ก.ย. 2569 — ยอดรวมรายวันต่อเจ้า (ฝั่งจอชี้ว่าสมุดเล่มนี้เก็บแค่ ~5 ชม.ครึ่ง)**
         สมุดข้างบนถูกตัดที่ 50 รอบ ⇒ ที่ความถี่วันนี้ ประวัติหายภายในกะเดียว
         ⇒ คำถาม *"วันนี้มีอะไรออกไปบ้าง"* ตอบย้อนหลังไม่ได้ ⇒ จึงส่งยอดรวมรายวันมาคู่กัน
         (ตัวจดอยู่ที่ `stock-push-live.mjs` · หนึ่งคีย์ = หนึ่ง (วันไทย × เจ้า) · เก็บถาวร) */
      const { วันนี้ไทย, วันไทยย้อน } = await import("../lib/thaiday.mjs");
      const วันที่จะอ่าน = [วันนี้ไทย(), วันไทยย้อน(1), วันไทยย้อน(2)];
      const เจ้าทั้งหมด = ["lazada", "shopee", "tiktok"];
      const ยอดรายวัน = {};
      for (const ว of วันที่จะอ่าน) {
        for (const ch of เจ้าทั้งหมด) {
          const row = await ถัง.get(`stockpush/daily/${ว}/${ch}`, { type: "json" }).catch(() => null);
          if (row) (ยอดรายวัน[ว] ||= {})[ch] = row;
        }
      }
      return json({
        ok: true,
        log: Array.isArray(log) ? log : [],
        /* 🔑 **บอกความจริงเรื่องเพดาน** — ฝั่งจอขอ 20 ก.ย. 2569
           เดิมส่ง `limitApplied: null` ⇒ ผู้เรียก **แยกไม่ออกว่า "มีแค่ 50 รอบ" หรือ "ถูกตัดเหลือ 50"**
           ⇒ ช่องที่ออกแบบมาเตือนเรื่องการตัด กลับเงียบเสียเอง ซึ่งร้ายกว่าไม่มีช่องเลย
           ⚠️ เส้นนี้ **ไม่ได้อ่าน `limit`** เพราะก้อนถูกตัดที่ **ตัวเขียน** ไม่ใช่ตอนอ่าน */
        limitApplied: 50,
        limitAppliedBy: "ตัวเขียน (`stock-push-live.mjs` → `log.slice(0, 50)`) — ไม่ใช่พารามิเตอร์ของคำขอนี้",
        "⏳ สมุดนี้ครอบได้กี่ชั่วโมง": "50 รอบหารด้วยความถี่รวมทุกเจ้า ⇒ ที่ ~12 รอบ/ชม./เจ้า × 3 เจ้า = **~1.4 ชม.** " +
          "ที่ความถี่ต่ำกว่านั้นก็ยาวขึ้นตาม ⇒ **อย่าใช้สมุดนี้ตอบคำถามย้อนหลังข้ามกะ** ให้ใช้ `ยอดรวมรายวัน` ด้านล่าง",
        ยอดรวมรายวัน: ยอดรายวัน,
        "🔑 ยอดรวมรายวัน — อ่านยังไง": "เก็บถาวร หนึ่งคีย์ต่อ (วันไทย × เจ้า) · `ยิงออก` = จำนวนรหัสที่ส่งออกจริง " +
          "· ไม่มีคีย์ของวัน/เจ้านั้น = **ไม่มีรอบที่ยิงจริงเลย** (ไม่ใช่ข้อมูลหาย) " +
          "· เริ่มจด 20 ก.ย. 2569 ⇒ **ก่อนวันนั้นไม่มีข้อมูล ไม่ใช่ศูนย์**",
    ...
    if (url.searchParams.get("mkpfinancesync")) {
      ...
      const ผล = {};
      const กุญแจรวม = {};   // ตาราง → Set ของคู่กุญแจที่เจอทั้งรอบ (จับการซ้ำข้ามหน้า)
      const ไล่หน้าไม่ครบ = [];
      for (let page = 1; page <= หน้าสุด; page++) {
        ...
        for (const p of r?.results || []) {
          ...
          const ชุดนี้ = w?.คู่กุญแจชุดนี้;
          delete w.คู่กุญแจชุดนี้;   // ไม่ส่งออกทาง HTTP — อาจยาวหลายร้อยรายการ
          if (!ผล[k]) { ผล[k] = { ...w, หน้าที่ไล่: 0 }; กุญแจรวม[k] = new Set(); }
          else {
            ผล[k].รับมา += w.รับมา; ผล[k].ส่งไป += w.ส่งไป || 0;
            ผล[k].ยุบ += w.ยุบ || 0; ผล[k].ขาดกุญแจ += w.ขาดกุญแจ || 0;
            ผล[k].แถวในฐานหลัง = w.แถวในฐานหลัง;
          }
          /* 🔴 รวมคู่กุญแจ **ข้ามหน้า** — `ยุบ` ของแต่ละคำขอมองไม่เห็นการซ้ำข้ามคำขอ
             ของจริง 19 ก.ย. 2569: TikTok เมิน `page` ⇒ หน้า 1/2/3 คืนแถวชุดเดิม
             ⇒ รับมา 300 · ลงฐาน 100 · แต่ `ยุบ` เป็น 0 ทุกหน้า **ตัวนับเงียบสนิท** */
          for (const key of ชุดนี้ || []) กุญแจรวม[k].add(key);
      ...
      /* 🔎 ตัดสินจากยอดรวมทั้งรอบ — ไม่ใช่จากตัวเลขของคำขอเดียว */
      const ซ้ำข้ามหน้า = [];
      for (const [k, set] of Object.entries(กุญแจรวม)) {
        const ส่งไป = ผล[k]?.ส่งไป || 0;
        ผล[k].คู่กุญแจไม่ซ้ำทั้งรอบ = set.size;
        if (ส่งไป > set.size) {
          ผล[k]["🔴 ซ้ำข้ามหน้า"] =
            `ส่งไป ${ส่งไป} แถว แต่คู่กุญแจไม่ซ้ำมีเพียง ${set.size} ` +
            "⇒ **หลายหน้าคืนแถวชุดเดิม** สาเหตุที่พบจริง: แพลตฟอร์มเมินพารามิเตอร์ `page` " +
            "(TikTok ใช้ `pageToken`) ⇒ ไล่หน้าด้วย `page` ไม่ได้ผลกับเจ้านั้น";
          ซ้ำข้ามหน้า.push(k);
        }
      }
      return okJson({
        ...
        ...(ซ้ำข้ามหน้า.length ? { ซ้ำข้ามหน้า } : {}),
        "⚠️ ขอบเขต":
    ...
    if (url.searchParams.get("list") === "logistics") {
      /* 🔑 **ตัวกรองของเส้นนี้ประกาศตัวเองผ่าน `applied` + `ignored` ที่ `listLogistics` คืนมา**
         ⇒ ที่นี่ **ห้ามประกาศซ้ำ** — สองแหล่งเรื่องเดียวกันจะเพี้ยนคนละทางเมื่อแก้ทีละที่
         🔴 บทเรียนราคาแพงวันเดียวกัน (19 ก.ย. 2569): ผมเคยเขียนตรงนี้ว่า
            "วัดแล้ว carrier เมินจริง ⇒ ปุ่มเลือกขนส่งบนจอเป็นปุ่มหลอก" ซึ่ง **ผิด**
            ผมวัดด้วยการส่งชื่อขนส่งที่ไม่มีอยู่แล้วดูว่าจำนวนแถวเท่าเดิม
            ⇒ วิธีนั้นแยกไม่ออกระหว่าง "เมินตัวกรอง" · "เมินเฉพาะค่านี้" · "ค่ามั่วดันไปตรงของจริง"
            ฝั่งจอยิงด้วย `carrier=Flash Express` ⇒ total 35,781 → 32,499 ⇒ **กรองจริง**
            ⇒ ถ้าคำประกาศนั้นขึ้นระบบ จอจะขึ้นเตือนว่าปุ่มที่ทำงานถูกอยู่เป็นปุ่มหลอก
         ⇒ ปลายทางอ่าน `applied.<ชื่อ>` ต่อคำขอ (มีค่า = ค่านี้ถูกใช้ · null + `ignored` = ค่านี้ใช้ไม่ได้) */
      return okJson({
        ...(await listLogistics({
          q: url.searchParams.get("q"),
          only: url.searchParams.get("only"),
        ...
        /* 🔴 **ถอนคำประกาศ `supportedFilters` ออก (19 ก.ย. 2569) — ผมสร้างมันจากวิธีที่กำกวม**
           ผมวัดด้วยการ **เทียบจำนวนแถว** เมื่อส่งค่ามั่ว ⇒ `carrier` ได้แถวเท่าเดิม
           ⇒ ผมสรุปว่า "ท่อเมิน carrier" **ผิด** — ฝั่งจอวัดด้วยชื่อขนส่งจริงแล้วพบว่า
             `carrier=Flash Express` ⇒ total 35,781 → 32,499 ⇒ **กรองจริง**
           🔑 จำนวนแถวเท่าเดิม แยกสามกรณีนี้ไม่ออก: ท่อเมินตัวกรอง · ท่อเมินเฉพาะค่านี้ · ค่ามั่วดันไปตรงของจริง
           ✅ **และเส้นนี้ตอบคำถามนั้นไว้ตรง ๆ อยู่แล้ว**: `applied.carrier` + `ignored`
             (ค่าจริง ⇒ `applied.carrier` มีค่า · ค่ามั่ว ⇒ `applied.carrier: null` + `ignored.carrier`)
           🔑 **ท่อรู้คำตอบและบอกอยู่แล้ว — เราสองคนกลับไปเดาจากอาการ**
           ⇒ ปลายทางต้องอ่าน `applied` ต่อคำขอ **ไม่ใช่รอคำประกาศคงที่จากการเดา** */
      });
    }
    ...
    /* GET ?synccontacts=1[&startpage=&maxpages=] — **เส้นดิบ ไม่ใช่สิ่งที่งานตามเวลาทำ**
       เส้นนี้เรียก `syncContacts()` ตรง ๆ (กวาดตามหน้าที่สั่ง)
       งานตามเวลาเรียก `syncContactsScheduled()` ซึ่งทำ **สองส่วนในรอบเดียว**
       (`recent` หน้า 1–2 จับผู้ติดต่อใหม่ + `sweep` ต่อจาก cursor อีก 6 หน้า ⇒ ~1,600 ราย/รอบ)
       ⇒ ป้ายนี้มีไว้กันคนหยิบผิดเส้น เพราะชื่อต่างกันแค่คำท้าย (`synccontacts` vs `synccontactsnow`)

       ⚠️ **แก้ข้อกล่าวหาที่ผมเขียนผิดไว้ในคอมเมนต์เดิม (19 ก.ย. 2569)**
          ผมเคยเขียนที่นี่ว่า "ฝั่งจอวัดเส้นนี้แล้วสรุปเป็นพฤติกรรมของงานตามเวลา" — **ไม่จริง**
          เขายิง `synccontactsnow=1` ถูกตั้งแต่ต้นทั้งสามรอบ และยืนยันซ้ำให้ดูแล้ว
          ต้นเหตุจริงของข้อสรุปที่คลาดคือ **เครื่องมือเขาตัดคำตอบที่ 120 ตัวอักษร**
          ⇒ ตัดตรงกลางส่วน `recent` พอดี ⇒ ไม่เห็นส่วน `sweep` เลย ⇒ เห็น 400 แล้วคิดว่าคือทั้งรอบ
       🔑 **บทเรียนของผม: อย่าเขียนสาเหตุที่เดาเอาลงโค้ดถาวร โดยเฉพาะสาเหตุที่โยนไปที่คนอื่น**
          บั๊กแก้ได้ แต่ข้อกล่าวหาที่ฝังในคอมเมนต์จะถูกอ่านเป็นข้อเท็จจริงไปอีกนาน
    ...
    if (url.searchParams.get("synccontacts")) {
      return json({
        ...
        "⚠️ เส้นนี้ไม่ใช่งานตามเวลา":
          "เส้นดิบนี้เรียก syncContacts() ตรง ๆ ⇒ กวาดตามหน้าที่สั่งเท่านั้น · " +
          "งานตามเวลาเรียก syncContactsScheduled() ซึ่งทำสองส่วนในรอบเดียว (หน้าแรก ๆ + กวาดต่อจาก cursor) " +
          "⇒ **ห้ามเอาตัวเลขจากเส้นนี้ไปสรุปว่างานตามเวลาทำได้เท่าไร** · " +
          "อยากวัดของจริงให้ยิง ?synccontactsnow=1 (ตัวเดียวกับงานตามเวลา) · " +
          "⚠️ และเวลาอ่านคำตอบของเส้นนั้น **ต้องพิมพ์ทั้งก้อน** — มันมีสองส่วน (recent + sweep) " +
          "ตัดความยาวทิ้งจะเห็นแค่ส่วนแรกแล้วได้ข้อสรุปที่ต่ำกว่าจริงราวสี่เท่า",
      });
    }
    ...
    if (url.searchParams.get("peak")) {
      /* 🔒 **ห้ามเปลี่ยนรหัส HTTP ของ `peak=status` และ `peak=dry` ฝ่ายเดียว** (ตกลงกับฝั่งจอ 19 ก.ย. 2569)
         จอที่ใช้อยู่จริงเรียกแค่สองเส้นนี้ (เขาไล่ตรวจทั้งรีโปแล้ว ไม่มีที่ไหนเรียกเส้นส่งจริง)
         และ **จอยังตัดสินด้วย `res.ok`** ⇒ วันที่เอาแนวคิด "สำเร็จบางส่วน = 502" มาใช้กับสองเส้นนี้
         **จอจะโยน error ใส่ผลซ้อมที่สำเร็จบางส่วน** ซึ่งเป็นผลที่ใช้งานได้ ไม่ใช่ความล้มเหลว
         ⇒ จะทำ **ต้องบอกฝั่งจอก่อน** ให้เขาเปลี่ยนไปอ่านเนื้อในคำตอบ แล้วค่อยเปลี่ยนที่นี่
         🔑 ข้อที่ฝั่งจอชี้และผมเห็นด้วย: **"สำเร็จบางส่วน" ไม่มีรหัส HTTP ที่ถูกต้อง**
            200 โกหก · 502 ก็ยังไม่บอกว่าใบไหนล้ม ⇒ **รหัสสถานะเป็นช่องที่แคบเกินกว่าจะบรรจุความจริงแบบนี้**
            ⇒ ความจริงต้องอยู่ในเนื้อคำตอบ (`สรุป` + `failed`) รหัสมีหน้าที่แค่บอกว่า "ต้องมาดูไหม"
         ⚠️ ที่เปลี่ยนไปแล้วคือ **เส้นส่งจริงเท่านั้น** (ไม่มีจอไหนเรียก · ยังรอคีย์ PEAK จากท่านประธาน) */
    ...
    if (p.get("list") === "stock") {
      ...
      /* 🔴 **ถอนคำประกาศ `supportedFilters` ที่ผมเพิ่งใส่ (19 ก.ย. 2569)**
         สร้างจากการเทียบจำนวนแถวด้วยค่ามั่ว ⇒ วิธีนั้นแยก "เมินตัวกรอง" จาก "เมินเฉพาะค่านี้" ไม่ออก
         ⇒ ดูเหตุผลเต็มที่ `list=logistics` · ของที่ใช้ได้คือ `applied`/`ignored` ต่อคำขอ */
      return okJson(r, r?.error ? 400 : 200);
    }
