  ...
  try {
    ...
    if (url.searchParams.get("pending")) {
      ...
      /* 🔴 **แยกตามช่องทางต้องคิดจาก "ทั้งกอง" ไม่ใช่จากรายการที่ส่งไป** (เพิ่ม 6 ก.ย. 2569)
          ...
          เจอของจริงวันนี้: 50 ใบแรกเป็น Shopify ทั้งหมด (฿184,660) แต่ยอดทั้งกอง ฿721,438
          ⇒ **ยังไม่รู้ที่มาของอีกสามในสี่ของยอด** — คลาส "หน้าแรกไม่ใช่ตัวแทน"
...
export const config = {
  path: "/api/core",
  /* ⚡ **รันที่สิงคโปร์ ไม่ใช่สหรัฐฯ** (6 ก.ย. 2569 — เจ้าของร้านสั่ง "สินค้ากับสินค้าชุด ห้ามโหลดช้า")
      ค่าเริ่มต้นของ Netlify คือ Ohio · แต่คนใช้อยู่ไทย และฐาน D1 อยู่เอเชีย
      ⇒ ทุกคำขอเดินทางข้ามมหาสมุทรสองต่อโดยไม่จำเป็น (เน็ต ~0.3 วิ + คุยฐานอีกรอบละ ~110 มิลลิวินาที)

  ⚠️ **ต้องประกาศตรงนี้ ไม่ใช่ใน netlify.toml** — ลองใส่ `[functions.core] region = "sin"`
     ใน netlify.toml มาแล้ว **deploy ผ่านฉลุยแต่ไม่มีผลอะไรเลย** ไม่มีคำเตือนสักคำ
     รู้ได้เพราะวัดหลัง deploy: เวลาคุยฐานเท่าเดิมเป๊ะ (344 vs 332 มิลลิวินาที)
     ถ้าไปที่สิงคโปร์จริงต้องลดฮวบ เพราะฐานอยู่เอเชียอยู่แล้ว
     ⇒ **ตั้งค่าที่ deploy ผ่านไม่ได้แปลว่าตั้งค่าติด** ต้องวัดของจริงเสมอ

  ⚠️ **ห้ามย้ายฟังก์ชันหน้าร้านตามมา** (ออเดอร์ · แชท · คนเข้าเว็บ · push)
     พวกนั้นใช้ Netlify Blobs ซึ่งอยู่สหรัฐฯ ⇒ ย้ายมาเอเชียจะ**ช้าลง**
     ตัวนี้ย้ายได้เพราะอ่านฐาน D1 (เอเชีย) เป็นหลัก และแตะ Blobs แค่รอบเดียวแบบคู่ขนาน */
  region: "sin",
};
