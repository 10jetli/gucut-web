...
async function route(req, context) {
  /* ⚡ **`?ping=1` — เส้นเบาที่สุด มีไว้ "ปลุกเครื่อง" ล่วงหน้าเท่านั้น** (ฝั่งจอขอมา 6 ก.ย. 2569)
   ...
   ⚠️ **ไม่ต้องมีรหัสหลังร้าน** — ตั้งใจ เพราะมันไม่คืนข้อมูลอะไรเลยสักอย่าง
      และการบังคับรหัสจะทำให้ปลุกจากเบราว์เซอร์ตรง ๆ ไม่ได้ · กันยิงรัวด้วยความที่มันไม่ทำงานอะไร */
  if (new URL(req.url).searchParams.get("ping")) {
    return new Response(JSON.stringify({ ok: true, ping: true, build: CORE_BUILD }), {
      status: 200,
