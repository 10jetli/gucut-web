...
/** 🌡️ **สภาพตอนวัด — ตอบว่า "รู้ได้ยังไง" ไม่ใช่แค่ "เป็นอะไร"** (20 ก.ย. 2569)
 *
 * 🔴 ที่มา: `burnPerDay` ร่วงจาก **3,965/วัน (19 ก.ย.)** เหลือ **215/วัน** ⇒ ดูเหมือนปัญหาหาย
 *    ของจริง: **นี่คือค่าของโลกที่ build ถูกปิด** ⇒ วันที่ปลดธง มันจะกลับขึ้น
 *    ⇒ ⇒ **เลขเดียวกันแปลคนละอย่างเมื่อสภาพเปลี่ยน** ⇒ เลขต้องพาสภาพไปด้วย
 *
 * 🔑 **รูปนี้มาจากข้อค้านของฝั่งจอ และข้อค้านนั้นสำคัญกว่าข้อเสนอเดิมของผม**
 *    ผมเสนอ `buildปิดอยู่: true` (boolean ที่โค้ดพิมพ์เอง)
 *    ⇒ เขาชี้ว่านั่นคือ **เครื่องผลิตคำกล่าวอ้างค้าง**: วันที่ท่านประธานเปิด build กลับ
 *      **ไม่มีอะไรไปแก้ `true` ตัวนั้น** ⇒ จอจะพิมพ์ว่า "วัดในโลกที่ไม่มี build" ตลอดไป
 *      ⇒ ⇒ กลายเป็น **คำอธิบายที่ทำให้เลขผิดปกติดูเหมือนอธิบายได้แล้ว** — อันตรายกว่าไม่มีคำอธิบาย
 *
 * 🚫 **`buildเปิด: null` ไม่ใช่ `false`** — "อ่านไม่ได้" ห้ามกลายเป็น "ปิดอยู่"
 *    (ตาข่ายสามสถานะเดิมของเรา: ไม่รู้ ≠ ไม่มี)
 * ⚠️ วันนี้ยังอ่านจาก Netlify API ไม่ได้ (ยังไม่ได้ตั้ง `NETLIFY_API_TOKEN`)
 *    ⇒ `ที่มา: "อ่านไม่ได้"` ⇒ ปลายทางต้องเขียนว่า **"ยังไม่รู้ว่า build เปิดอยู่ไหม"**
 *    ⇒ ⇒ ไม่ใช่เขียนว่า "วัดในโลกที่ไม่มี build" ซึ่งเป็นคำกล่าวอ้างที่เรายังพิสูจน์ไม่ได้ */
function สภาพตอนวัด(token) {
  /* 🚫 ห้ามเดาจาก "มีคอมมิตค้างเยอะ" หรือจากไฟล์ธงในเครื่อง — ฟังก์ชันนี้รันบนเซิร์ฟเวอร์
     ธงหยุด deploy เป็นไฟล์ใน `$HOME` ของเครื่องที่บ้าน ⇒ **เซิร์ฟเวอร์มองไม่เห็น**
     🔑 และนั่นคือบทเรียนของวันนี้: **ตาข่ายที่อยู่ในเครื่องเดียว กันได้แค่เครื่องนั้น** */
  if (!token) {
    return { buildเปิด: null, ที่มา: "อ่านไม่ได้ (ไม่มี NETLIFY_API_TOKEN)", เมื่อ: new Date().toISOString() };
  }
  return { buildเปิด: null, ที่มา: "ยังไม่ได้ต่อ (มี token แต่ยังไม่มีโค้ดอ่านสถานะ build)", เมื่อ: new Date().toISOString() };
}
...
async function ติดอัตราเผา(s, out) {
  ...
  return Object.assign(out, คิดอัตราเผา(ก่อนหน้า, out), {
    ...
    /* 🌡️ **สภาพที่ใช้วัด — เลขต้องพาบริบทไปเอง** (20 ก.ย. 2569 · รูปนี้มาจากข้อค้านของฝั่งจอ)
       ⚠️ `buildเปิด: null` = **ยังไม่รู้** ⇒ ปลายทางต้องเขียนว่า "ยังไม่รู้ว่า build เปิดอยู่ไหม"
          **ห้ามเขียนว่า "วัดในโลกที่ไม่มี build"** ซึ่งเป็นคำกล่าวอ้างที่เรายังพิสูจน์ไม่ได้ */
    "สภาพตอนวัด": สภาพตอนวัด(process.env.NETLIFY_API_TOKEN || process.env.NETLIFY_AUTH_TOKEN || ""),
