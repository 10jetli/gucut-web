...
export default async function handler(req, context) {
  ...
  const checks = await Promise.all([
    ...
    check("สำรองข้อมูล (Blobs → D1)", async () => {
      ...
      /* ⏱️ **เกณฑ์กับคำพูดคิดจาก cron จริงทั้งคู่** (เปลี่ยน 19 ก.ย. 2569)
         เดิมบรรทัดนี้เขียนเกณฑ์ 12 ชม. กับคำว่า "ทุก 6 ชม." ด้วยมือ พร้อมคอมเมนต์เตือนตัวเองว่า
         "แก้ตาราง cron ที่ไหน ต้องกลับมาแก้เลขนี้ด้วยทุกครั้ง" — **คำเตือนที่ฝากความจำไว้กับคน**
         🔑 วันนี้เจอคำบอกรอบที่ฝังไว้แล้วกลายเป็นเท็จ **4 ครั้งในวันเดียว** ⇒ เลิกฝากความจำ
         ⇒ ให้ที่ว่าง 2 รอบก่อนถือว่าหยุด (พลาดรอบเดียวยังไม่ใช่เรื่อง) โดยคิด "รอบ" จากตารางจริง
         ⚠️ อ่านรอบไม่ออก ⇒ ถอยไปใช้เกณฑ์เดิม 12 ชม. **และบอกออกไปว่าถอย** (ทางถอยต้องประกาศตัว) */
      const { รอบที่คาดหวัง, รอบเป็นคำพูด } = await import("../lib/core-freshness.mjs");
      const รอบ = (await รอบที่คาดหวัง("backup-run", "x")).xExpectedEveryHours;
      const คำ = await รอบเป็นคำพูด("backup-run");
      const เพดาน = รอบ ? รอบ * 2 : 12;
      if (hrs > เพดาน)
        return { warn: true, note:
          `${what} · แต่ไม่มีอะไรใหม่มา ${Math.round(hrs)} ชม. — งานสำรองน่าจะหยุด` +
          (คำ ? ` (ตั้งไว้${คำ} ⇒ เกณฑ์ ${Number(เพดาน.toFixed(1))} ชม. = 2 รอบ)`
              : " (อ่านรอบจากตารางไม่ออก ⇒ ใช้เกณฑ์สำรอง 12 ชม.)") };
    ...
    check("กระจกค่าธรรมเนียม Shopee (ยอดโอนสุทธิ)", async () => {
      ...
      const คำรอบ = await คำของงาน("mkp-fees-sync");
      /* 🔴 เดิมเขียน "ทุกชั่วโมง นาทีที่ 47" ด้วยมือ ⇒ เป็นเท็จทันทีที่ใครแก้ cron
         ⇒ คิดจากตารางจริง · อ่านไม่ออกก็ **ไม่พูดถึงรอบเลย** ดีกว่าพูดผิด */
      if (!beat?.at) return { off: true, note: `ยังไม่เคยวิ่งเลย${คำรอบ ? ` (งานตามเวลา${คำรอบ})` : ""} — ปกติถ้าเพิ่ง deploy` };
      ...
      const รอบชม = (await รอบของงาน("mkp-fees-sync", "x")).xExpectedEveryHours;
      /* ให้ที่ว่าง 3 รอบก่อนเตือน — คิดจากตารางจริง · อ่านไม่ออกถอยไป 180 นาที แล้วบอกว่าถอย */
      const เพดานนาที = รอบชม ? Math.round(รอบชม * 60 * 3) : 180;
      if (mins > เพดานนาที) return { warn: true, note:
