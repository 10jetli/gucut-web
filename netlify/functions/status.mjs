...
export default async function handler(req, context) {
  ...
  const checks = await Promise.all([
    ...
    check("สำรองข้อมูล (Blobs → D1)", async () => {
      ...
      /* ⏱️ **เกณฑ์ต้องตามตารางจริงเสมอ** — งานสำรองลดเป็น **ทุก 6 ชั่วโมง** เมื่อ 18 ก.ย. 2569
         (เดิมทุกชั่วโมง · เกณฑ์เดิม 3 ชม.) ⇒ ถ้าไม่แก้เกณฑ์ตาม หน้านี้จะขึ้นเหลืองทุกวัน
         🔑 คำเตือนที่ดังตอนของปกติ = คำเตือนที่คนเรียนรู้ที่จะมองข้าม แล้ววันที่หยุดจริงก็ถูกมองข้ามด้วย
         ⇒ ให้ที่ว่าง 2 รอบ (12 ชม.) ก่อนถือว่าหยุด — พลาดรอบเดียวยังไม่ใช่เรื่อง
         ⚠️ **แก้ตาราง cron ที่ไหน ต้องกลับมาแก้เลขนี้ด้วยทุกครั้ง** [[nets-expire-silently]] */
      if (hrs > 12)
        return { warn: true, note: `${what} · แต่ไม่มีอะไรใหม่มา ${Math.round(hrs)} ชม. — งานสำรองน่าจะหยุด (ตั้งไว้ทุก 6 ชม.)` };
