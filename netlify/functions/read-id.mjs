...
// ⚠️ หนึ่งการสแกนใช้ได้ถึง 5 คำขอ (บันได 4 มุม + ด่านซูม) — เพดานต้องคิดจาก
//    "จำนวนการสแกน" ไม่ใช่จำนวนคำขอ · 30 ≈ สแกนได้ ~6 รอบ/10 นาที
//    เพดานเดิม 12 ทำเจ้าของร้านทดสอบ 2-3 รอบก็โดนล็อก (เจอจริง 27 ส.ค. 2569)
//    เพดานเงิน: 30 คำขอ × ~4.5 เครดิต ≈ ฿27/IP/10นาที — รับได้ ยังกันคนเผาเครดิตอยู่
const MAX_PER_IP = 30;
...
export default async function handler(req, context) {
  ...
  //
  //    **เหตุการณ์ 25 ส.ค. 2569** — ตอนนั้นร้านมี ANTHROPIC_API_KEY ตั้งไว้เองใน env
  //    Gateway จึงเงียบไปทั้งคู่ ⇒ ได้คีย์ที่ตั้งไว้เองมาโดยไม่มี base url แล้วยิงไป
  //    api.anthropic.com ⇒ ตอบ invalid x-api-key — อาการหลอกตามาก ดูเหมือนคีย์เสีย
  //    ทั้งที่เป็นเรื่องคนละเรื่อง (เสียไปสอง deploy กว่าจะรู้)
  //
  //    **สถานะเมื่อ 4 ก.ย. 2569** — ตรวจ env จริงบนไซต์แล้ว **ไม่มี ANTHROPIC_API_KEY
  //    และ ANTHROPIC_BASE_URL อยู่เลยสักตัว** ⇒ ตอนนี้วิ่งบน Gateway ล้วน
  //    ลำดับนี้ยังต้องคงไว้ กันวันที่มีคนไปตั้งคีย์เองเพิ่มแล้วเจอซ้ำ
  //
  /* ⚠️ **ห้ามเขียนคอมเมนต์ที่เล่า "สภาพปัจจุบันของค่าใน env" แบบไม่มีวันที่กำกับ**
      บรรทัดที่เคยเขียนว่า "ร้านนี้มี ANTHROPIC_API_KEY ตั้งไว้เองอยู่ก่อน" เคยจริงเมื่อ 25 ส.ค.
      แล้วค่านั้นถูกลบไปทีหลัง **แต่ข้อความไม่ได้ถูกลบตาม เพราะไม่มีอะไรบังคับให้อัปเดต**
      4 ก.ย. 2569 มีคนอ่านบรรทัดนี้แล้วรายงานเจ้าของร้านว่าร้านถือคีย์ Anthropic ของตัวเองอยู่
      **เจ้าของร้านจึงสั่งงานจากข้อมูลเท็จ** ("ปิด API") ทั้งที่ไม่มี API ให้ปิด
      ⇒ ของนอกโค้ด (env · ค่าในคอนโซล · การตั้งค่าที่คนแก้ได้) ให้เขียนเป็น
        **"เหตุการณ์ + วันที่"** เสมอ ห้ามเขียนเป็น "สภาพ" — สภาพจะค้าง เหตุการณ์ไม่ค้าง */
  //
