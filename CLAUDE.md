...
> **สถานะ (15 ส.ค. 2026):** R2 เปิดแล้ว · bucket `gucut-video` สร้างแล้ว (Asia Pacific / Standard)
> เหลือเจ้าของร้านรันบน Mac บรรทัดเดียว (สคริปต์พาไปทีละขั้นเป็นภาษาไทย ถามคีย์เอง ลอง 5 ใบก่อน):
>
>     bash <(curl -fsSL https://new78.com/r2-setup.sh)
>
> (ตัวสคริปต์อยู่ที่ `public/r2-setup.sh` · ตัวย้ายจริงคือ `scripts/video-to-r2.mjs` รันซ้ำได้ ข้ามใบที่เสร็จแล้ว)
> ไฟล์ขึ้นครบแล้วเหลือ 2 ขั้นปิดจ๊อบ:
> 1. เปิดทางเข้าไฟล์ให้ bucket — ผูกโดเมน `video.gucut.com` (R2 → Settings → Public access)
>    หรือชั่วคราวใช้ r2.dev ก็ได้ (netlify.toml อนุญาต `pub-*.r2.dev` ใน CSP ไว้แล้ว แต่ r2.dev มี rate limit ไม่เหมาะใช้ยาว)
> 2. แก้ `HOST` ใน `src/lib/videos.ts` เป็น URL นั้น = จบ ไม่เหลือ Shopify แล้ว
> ⚠️ เครื่องที่ Claude ทำงาน **ต่อ cdn.shopify.com ไม่ได้** — การย้ายไฟล์ต้องรันจากเครื่องเจ้าของร้านเท่านั้น
>
...
3. หน้า นโยบายความเป็นส่วนตัว / เงื่อนไขการใช้บริการ
