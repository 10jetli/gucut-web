...
- [x] เช็คเอาต์แบบ Shopee หน้าเดียวจบ → POST `/api/orders`
      ...
      ตัวเลขจัดส่งทั้งหมดอยู่หัวไฟล์ `CheckoutView.tsx` ที่เดียว:
      `SHIPPING_FEE` `SHIPPING_LIST` `COD_FEE` `SHIP_MIN_DAYS` `SHIP_MAX_DAYS` `SHIP_NAME` `CARRIER`
      ⚠️ `SHIPPING_LIST` = ราคาส่งปกติที่ขีดฆ่าโชว์ — ใส่เฉพาะราคาที่เก็บจริง **ห้ามใส่เลขมั่วให้ดูเหมือนลด**
