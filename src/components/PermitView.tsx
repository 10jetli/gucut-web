...
export default function PermitView() {
  ...
  const readCard = useCallback(async (file: File) => {
    ...
    try {
      ...
      // เรียกด่านซูมเมื่อ "ช่องไหนก็ตามที่ควรอ่านได้จากบรรทัดที่อยู่ยังว่าง"
      ...
      // ⚠️ ห้ามยกเว้นบ้านเลขที่/หมู่ — เคยยกเว้นแล้วช่องที่อ่านไม่ได้ขึ้นสีขาวเฉย ๆ
      //    ลูกค้าไม่มีอะไรสะกิดให้เห็นว่าต้องกรอก (ช่องที่ตรวจผ่านเขียวทับเหลืองเองอยู่แล้ว)
      const addrKeys = Object.keys(a);
