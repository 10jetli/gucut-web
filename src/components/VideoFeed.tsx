...
export default function VideoFeed({ first, total, warm = 0 }: { first: FeedItem[]; total: number; warm?: number }) {
    ...
    const tryPlay = (n = 0) => {
      ...
      el.play().catch((err: DOMException) => {
        ...
        if (gestured.current) {
          ...
          // ไม่ปิดเสียงทั้งฟีด ไม่เด้งป้ายใหญ่ (เด้งทุกใบ น่ารำคาญ — เจ้าของร้านทัก)
          // เสียงคืนเองทันทีที่นิ้วแตะจอครั้งถัดไป (ตัวฟัง pointerdown ด้านล่าง)
          // ⚠️ ห้ามสั่ง unmute เองนอก gesture — iOS จะหยุดคลิปทันที กลายเป็นค้าง
          //    ทำได้แค่บอกลูกค้าด้วย toast สั้น ๆ ว่าแตะตรงไหนก็ได้เสียงกลับ
          el.muted = true;
          el.play().catch(() => { again(n); });
          say("🔇 แตะจอเพื่อเปิดเสียง");
          return;
        }
  ...
  }, [active, muted, setMute, shuffled, say]);
...
const Slide = memo(function Slide({
  ...
  return (
    <section data-i={i} className="relative h-full w-full snap-start [scroll-snap-stop:always] mb-[80px]">
      {mode === "video" ? (
        <video
          ref={setNode}
