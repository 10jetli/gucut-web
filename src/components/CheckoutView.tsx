...
export default function CheckoutView() {
  ...
  /* 🔴 **แยก "ถามไม่ได้" ออกจาก "Beam ปิดอยู่"** (6 ก.ย. 2569)
      เดิม `.catch()` ตั้ง `beam:false` ⇒ เน็ตสะดุดครั้งเดียว ลูกค้าจะถูกพาไปทาง
      "แนบสลิปเอง" **โดยไม่มีอะไรบอกว่ามีอะไรผิดปกติ** ทั้งที่ Beam ใช้ได้ตามปกติ
      ⇒ ลูกค้าจ่ายช้าลง และร้านต้องมานั่งตรวจสลิปเอง ซึ่งเจ้าของร้านสั่งไว้ว่าไม่เอา
      ⚠️ ยังคง `beam:false` ไว้เพื่อให้ **มีทางจ่ายเสมอ** (ห้ามเหลือศูนย์ช่องทาง)
         แต่ต้องบอกลูกค้าตรง ๆ ว่าตรวจไม่ได้ ไม่ใช่เงียบแล้วให้เขาเดาเอง */
  const [paysFailed, setPaysFailed] = useState(false);
  ...
  useEffect(() => {
    ...
    fetch("/api/pay-options")
      ...
      .catch(() => { setPaysFailed(true); setPays({ beam: false, cod: COD_ON }); });
    return () => { alive = false; };
  }, []);
  ...
  return (
    <main className="min-h-[100dvh] bg-steel-900 pb-28">
      ...
      <Card>
        ...
        {/* ⚠️ ขึ้นเฉพาะตอน "ถามไม่ได้" — ไม่ใช่ตอน Beam ปิดจริง ๆ (คนละเรื่อง)
            ต้องบอกทางไปต่อด้วย ไม่ใช่บอกแค่ว่าพัง ไม่งั้นลูกค้าปิดหน้าไปเลย */}
        {paysFailed && (
          <p className="mx-3 mt-2.5 rounded-lg bg-[#fff4e5] px-3 py-2 text-[12px] leading-relaxed text-[#7a4a00]">
            ตรวจช่องทางชำระเงินอัตโนมัติไม่สำเร็จ —{" "}
            <b>ไม่ได้แปลว่าจ่ายไม่ได้</b> ลองรีเฟรชหน้านี้อีกครั้งเพื่อใช้การจ่ายแบบสแกนจบในตัว
            หรือจะใช้วิธีโอนแล้วแนบสลิปด้านล่างก็ได้เลย
          </p>
        )}
        ...
        {/* 🔴 **ตอน "ตรวจไม่ได้" ต้องแสดงทั้งสองทาง ห้ามเลือกให้ลูกค้าเงียบ ๆ** (ฝั่งจอเสนอ 6 ก.ย. 2569)
            สองทิศเสียหายไม่เท่ากันและไม่สมมาตร:
            · ถอยไปทาง "ไม่มี Beam" = ลูกค้า **ทุกคน ทุกใบ ตลอดเวลาที่ล่ม** ถูกพาไปโอน+แนบสลิป
              และร้านต้องนั่งตรวจสลิปเอง ⇒ **เสียหายเงียบ ๆ ทุกใบ**
            · ถอยไปทาง "มี Beam" = ลูกค้ากดแล้วอาจเจอ error **แต่เห็นทันทีในหน้านั้น
              และยังเลือกโอนได้อยู่** ⇒ เสียหายเห็นได้และแก้ได้ในนัดเดียว
            ⚠️ แต่ตอนล้มเราไม่มีรายชื่อช่องทางของ Beam (มาจากคำขอเดียวกันที่ล้ม)
               ⇒ แสดงเป็นแถวเดียวด้วยค่าเริ่มต้น QR_PROMPT_PAY ซึ่งเป็นช่องทางหลักของร้าน
               **ห้ามเดารายชื่อทั้งชุด** — เดาผิดคือพาลูกค้าไปช่องทางที่ร้านไม่ได้เปิด */}
        {paysFailed && (
          <PayOption
            on={pay === "beam"}
            onClick={() => { setPay("beam"); setBeamMethod("QR_PROMPT_PAY"); }}
            icon={
              // eslint-disable-next-line @next/next/no-img-element
              <img
            ...
            title="สแกน QR จ่ายจบในตัว (แนะนำ)"
            note="ระบบยืนยันเงินเข้าเอง ไม่ต้องแนบสลิป — ถ้ากดแล้วขึ้นข้อผิดพลาด ใช้วิธีด้านล่างแทนได้"
          />
        )}
        {pays && !pays.beam && (
          <PayOption
