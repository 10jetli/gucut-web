...
export async function buildLz1Pdf(
  ...
  // ⚠️ ขอบช่องทุกตัวมาจากการสแกนพิกเซลหา "จุดไข่ปลา" บนฟอร์มเปล่า (26 ส.ค. 2569)
  //    ห้ามกะด้วยตา — เคยวางช่อง "จำนวน" ไว้ 255–300 ทั้งที่ป้ายคำว่า จำนวน
  //    กินถึง 281pt เลขเลยไปทับตัว "น" (เจ้าของร้านเจอบนมือถือ)
  fit(p2, th(String(total || "")), 355, 398, 659.5);
  // แถว 2.1–2.5 เว้นระยะเท่ากันแถวละ ~39.2pt (วัดจากฟอร์มจริง)
  d.saws.slice(0, 5).forEach((s, i) => {
    const dy = i * 39.2;
    fit(p2, s.engine, 333, 382, 639.5 - dy, 8);
    fit(p2, s.hp, 471, 495, 639.5 - dy, 9);
    fit(p2, th(s.bar), 190, 228, 620 - dy, 9);
    fit(p2, th(s.qty), 285, 320, 620 - dy, 9);
  });
  ...
  // ⚠️ เส้นจุดในวงเล็บอยู่ y≈246.5 (สแกนพิกเซล 26 ส.ค. 2569) ฐานตัวหนังสือ
  //    ต้องอยู่เหนือเส้น ~1.5pt ให้ชื่อ "นั่งบนเส้น" — เคยวางที่ 244 แล้วเส้นจุด
  //    ขีดทะลุกลางตัวหนังสือ (เจ้าของร้านเห็นจากมือถือ)
  fit(p3, d.name, 280, 378, 248, 9);
